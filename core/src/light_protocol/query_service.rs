...
impl QueryService {
    ...
    fn account_key(address: &H160) -> Vec<u8> {
        StorageKey::AccountKey(address).to_key_bytes()
    }

    fn code_key(address: &H160, code_hash: &H256) -> Vec<u8> {
        StorageKey::CodeKey { address, code_hash }.to_key_bytes()
    }
    ...
    fn retrieve_account<'a>(
        ...
        self.retrieve_state_root(epoch)
            .map(move |root| Self::account_key(&address))
    ...
    fn retrieve_code<'a>(
        ...
        // verification in the context?
        self.retrieve_state_root(epoch)
            .map(move |root| Self::code_key(&address, &code_hash))
            .and_then(move |key| self.retrieve_state_entry(epoch, key))
            .map_err(|e| format!("{}", e))
    }
