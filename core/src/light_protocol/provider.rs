...
impl Provider {
    ...
    pub fn register(
        ...
        network
            .register_protocol(self, LIGHT_PROTOCOL_ID, LIGHT_PROTOCOL_VERSION)
            .map_err(|e| {
    ...
    fn dispatch_message(
        ...
        let peer_protocol_version = self.get_existing_peer_state(peer)?.read().protocol_version;

        match msg_id {
            msgid::STATUS_PING => self.on_status(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_STATE_ENTRIES => self.on_get_state_entries(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_STATE_ROOTS => self.on_get_state_roots(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_BLOCK_HASHES_BY_EPOCH => self.on_get_block_hashes_by_epoch(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_BLOCK_HEADERS => self.on_get_block_headers(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::SEND_RAW_TX => self.on_send_raw_tx(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_RECEIPTS => self.on_get_receipts(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_TXS => self.on_get_txs(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_WITNESS_INFO => self.on_get_witness_info(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_BLOOMS => self.on_get_blooms(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_BLOCK_TXS => self.on_get_block_txs(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
            msgid::GET_TX_INFOS => self.on_get_tx_infos(io, peer, decode_rlp_and_check_deprecation(&rlp, peer_protocol_version)?),
    ...
    fn on_status(
        &self, io: &dyn NetworkContext, peer: &NodeId, status: StatusPing,
    ) -> Result<(), Error> {
