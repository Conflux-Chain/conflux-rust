...
impl State {
    ...
    fn commit_staking_state(
        &mut self, debug_record: &mut Option<ComputeEpochDebugRecord>,
    ) -> DbResult<()> {
    ...
    ) -> DbResult<()>
    {
        for address in killed_addresses {
            self.db
                .delete(StorageKey::new_account_key(&address), debug_record)?;
    ...
    ) -> DbResult<StateRootWithAuxInfo>
    {
        ...
        for (address, entry) in self.dirty_accounts_to_commit.iter_mut() {
            ...
            match &mut entry.account {
                ...
                Some(account) => {
                    account.commit(&mut self.db, debug_record)?;
                    self.db.set::<Account>(
