...
use cfx_internal_common::{ChainIdParamsDeprecated, ChainIdParamsInner};
use cfx_types::H256;
...
impl Handleable for StatusDeprecatedV2 {
...
impl Handleable for StatusDeprecatedV3 {
    fn handle(self, ctx: &Context) -> Result<(), Error> {
        debug!("on_status, msg=:{:?}", self);
...
pub struct StatusV4 {
    pub chain_id: ChainIdParamsInner,
    pub genesis_hash: H256,
    pub best_epoch: u64,
    pub node_type: NodeType,
    pub terminal_block_hashes: Vec<H256>,
...
impl Handleable for StatusV4 {
    fn handle(self, ctx: &Context) -> Result<(), Error> {
        debug!("on_status, msg=:{:?}", self);

        let chain_id =
            &*ctx.manager.graph.consensus.get_config().chain_id.read();
        if chain_id.matches(&self.chain_id, self.best_epoch) {
            debug!(
                "Peer {:?} chain_id mismatches (ours: {:?}, theirs: {:?})",
                ctx.node_id, chain_id, self.chain_id,
            );
