...
fn test_admin_contract_sig() {
    ...
    assert_eq!(
        <SetAdminSnake<StorageState>>::default()
            .function_sig()
            .to_vec(),
        SET_ADMIN_SIG.to_vec()
    );
    assert_eq!(
        <Destroy<StorageState>>::default().function_sig().to_vec(),
        DESTROY_SIG.to_vec()
    );
}
...
fn test_admin_contract_sig_v2() {
    ...
    // sdk.
    check_signature!(GetAdmin<StorageState>, "64efb22b");
    check_signature!(SetAdmin<StorageState>, "c55b6bb7");
    check_signature!(Destroy<StorageState>, "00f55d9d");
}
