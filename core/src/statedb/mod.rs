...
impl<'a> StateDb<'a> {
    ...
    pub fn account_key<'k>(&self, address: &'k Address) -> StorageKey<'k> {
        StorageKey::new_account_key(address)
    }

    pub fn code_root_key<'k>(&self, address: &'k Address) -> StorageKey<'k> {
        StorageKey::new_code_root_key(address)
    }

    pub fn code_key<'k>(
        &self, address: &'k Address, code_hash: &'k H256,
    ) -> StorageKey<'k> {
        StorageKey::new_code_key(address, code_hash)
    }

    pub fn storage_root_key<'k>(&self, address: &'k Address) -> StorageKey<'k> {
        StorageKey::new_storage_root_key(address)
    }

    pub fn storage_key<'k>(
        &self, address: &'k Address, key: &'k [u8],
    ) -> StorageKey<'k> {
        StorageKey::new_storage_key(address, key)
    }
