...
mod impls {
    pub struct StateDb {
        ...
        /// modified key values.
        accessed_entries: RwLock<HashMap<Vec<u8>, EntryValue>>,
    ...
    impl StateDb {
        ...
        pub fn get_raw(&self, key: StorageKey) -> Result<Option<Arc<[u8]>>> {
            ...
            let r;
            if let Some(v) = self.accessed_entries.read().get(&key_bytes) {
        ...
        ) -> Result<Option<Vec<MptKeyValue>>>
        {
            ...
            // delete_all will not delete any key which doesn't exist before the
            // operation. Therefore we don't need to check the
            // accessed_entries prior to the operation.
            let deleted =
        ...
        pub fn compute_state_root(&mut self) -> Result<StateRootWithAuxInfo> {
            ...
            for (k, v) in &*self.accessed_entries.get_mut() {
                if v.is_modified() {
                        ...
                        &storage_key
                    {
                        ...
                        &storage_key
                    {
                        // The account is a new contract.
                        if (address_bytes.is_contract_address()
                            // Internal contract
                            || address_bytes.is_builtin_address())
                            && v.original_value.is_none()
                        {
                            // To make sure that we always set StorageLayout for
                            // new contract.
                            Self::load_storage_layout(
                                &mut self.storage_layouts,
                                &self.storage,
            ...
            for (k, v) in &mut storage_layouts {
                // FIXME: debug_record
                self.commit_storage_layout(k, v, None)?;
            }
