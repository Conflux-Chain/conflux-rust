...
use crate::{
    ...
    verification::{compute_block_receipts_root, VerificationConfig},
    vm::{Env, Spec},
...
impl ConsensusExecutionHandler {
    ...
    )
    {
        ...
        warn!("compute block receipts root for epoch {:?}, result: {:?}, receipts: {:?}",
            pivot_block.hash(), compute_block_receipts_root(&epoch_receipts), epoch_receipts,
        );
        self.data_man.insert_epoch_execution_commitment(
            pivot_block.hash(),
            state_root.clone(),
            compute_block_receipts_root(&epoch_receipts),
