...
impl ConsensusExecutionHandler {
            ...
            for (idx, transaction) in block.transactions.iter().enumerate() {
                ...
                match r {
                    ...
                    ExecutionOutcome::ExecutionErrorBumpNonce(
                        ...
                        env.gas_charged += executed.gas_charged;
                        env.gas_fee += executed.fee;
                        warn!(
    ...
    )
    {
        ...
        for (enum_idx, block) in epoch_blocks.iter().enumerate() {
            ...
            let mut accumulated_gas_fee = U256::zero();
            debug_assert!(
                block_receipts.receipts.len() == block.transactions.len()
            );
            for (idx, tx) in block.transactions.iter().enumerate() {
                let fee =
                    block_receipts.receipts[idx].gas_fee - accumulated_gas_fee;
                let info = tx_fee
                    .entry(tx.hash())
                ...
                accumulated_gas_fee = block_receipts.receipts[idx].gas_fee;
            }
        }
