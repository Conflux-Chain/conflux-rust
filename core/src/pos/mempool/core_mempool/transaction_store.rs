...
use crate::pos::mempool::{
    core_mempool::{
        index::{
            AccountTransactions, ParkingLotIndex, PriorityIndex,
...
pub struct TransactionStore {
    ...
    // keeps track of "non-ready" txns (transactions that can't be included in
    // next block)
...
impl TransactionStore {
    ...
    /// updates indexes.
    pub(crate) fn insert(
        &mut self, txn: MempoolTransaction, current_sequence_number: u64,
    ) -> MempoolStatus {
        ...
        // ignores the case transaction hash is same for retrying submit
        // transaction.
        if let Some(txns) = self.transactions.get_mut(&address) {
            if let Some(current_version) = txns.get_mut(&sequence_number) {
                ...
                if current_version.txn.max_gas_amount()
                    == txn.txn.max_gas_amount()
    ...
    fn check_is_full_after_eviction(
        &mut self, txn: &MempoolTransaction, curr_sequence_number: u64,
    ) -> bool {
        if self.system_ttl_index.size() >= self.capacity
            && self.check_txn_ready(txn, curr_sequence_number)
        {
            // try to free some space in Mempool from ParkingLot by evicting a
            // non-ready txn
            if let Some((address, sequence_number)) =
                self.parking_lot_index.get_poppable()
            {
                if let Some(txn) = self
                    .transactions
                    .get_mut(&address)
    ...
    fn check_txn_ready(
        ...
        if tx_sequence_number == curr_sequence_number {
            ...
            // shouldn't really get here because filtering out old txn sequence
            // numbers happens earlier in workflow
    ...
    fn process_ready_transactions(
        &mut self, address: &AccountAddress, current_sequence_number: u64,
    ) {
        if let Some(txns) = self.transactions.get_mut(&address) {
            ...
            for (_, txn) in txns
                .range_mut((Bound::Excluded(sequence_number), Bound::Unbounded))
            {
    ...
    fn clean_committed_transactions(
        &mut self, address: &AccountAddress, sequence_number: u64,
    ) {
        ...
        // the nodes has sent the transaction to consensus but this node
        // still has the transaction sitting in mempool.
        if let Some(txns) = self.transactions.get_mut(&address) {
    ...
    pub(crate) fn commit_transaction(
        &mut self, account: &AccountAddress, account_sequence_number: u64,
    ) {
    ...
    pub(crate) fn reject_transaction(
        &mut self, account: &AccountAddress, _sequence_number: u64,
    ) {
        if let Some(txns) = self.transactions.remove(&account) {
            ...
            diem_debug!(
                LogSchema::new(LogEntry::CleanRejectedTxn).txns(txns_log)
            );
        }
    }
    ...
    pub(crate) fn read_timeline(
        ...
        for (address, sequence_number) in
    ...
    pub(crate) fn gc_by_system_ttl(
        &mut self, metrics_cache: &TtlCache<(AccountAddress, u64), SystemTime>,
    ) {
            ...
            if let Some(txns) = self.transactions.get_mut(&key.address) {
                ...
                if let Some(txn) = txns.remove(&key.sequence_number) {
                    ...
                    if let Some(&creation_time) =
                        metrics_cache.get(&(account, sequence_number))
                    {
    ...
    pub(crate) fn gen_snapshot(
        &self, metrics_cache: &TtlCache<(AccountAddress, u64), SystemTime>,
    ) -> TxnsLog {
        let mut txns_log = TxnsLog::new();
        for (account, txns) in self.transactions.iter() {
            for (seq_num, _txn) in txns.iter() {
                let status =
                    if self.parking_lot_index.contains(account, seq_num) {
                        "parked"
                    } else {
                        "ready"
                    };
                let timestamp =
                    metrics_cache.get(&(*account, *seq_num)).cloned();
                txns_log
                    .add_full_metadata(*account, *seq_num, status, timestamp);
            }
        }
        txns_log
    }
