...
pub static SHARED_MEMPOOL_BROADCAST_LATENCY: Lazy<HistogramVec> =
    Lazy::new(|| {
        register_histogram_vec!(
...
static SHARED_MEMPOOL_PENDING_BROADCASTS_COUNT: Lazy<IntGaugeVec> =
    Lazy::new(|| {
        register_int_gauge_vec!(
...
static SHARED_MEMPOOL_TRANSACTIONS_PROCESSED: Lazy<IntCounterVec> =
    Lazy::new(|| {
        register_int_counter_vec!(
            "diem_shared_mempool_transactions_processed",
            "Number of transactions received and handled by shared mempool",
            &[
                "status", /* state of transaction processing: "received",
                           * "success", status code from failed txn
                           * processing */
                "network", /* state of transaction processing: "received",
                            * "success", status code from failed txn
                            * processing */
...
pub fn shared_mempool_transactions_processed_inc(
    status: &str, network: &str, sender: &str,
) {
...
pub static SHARED_MEMPOOL_TRANSACTION_BROADCAST_SIZE: Lazy<HistogramVec> =
    Lazy::new(|| {
        register_histogram_vec!(
...
pub static SHARED_MEMPOOL_BROADCAST_TYPE_COUNT: Lazy<IntCounterVec> =
    Lazy::new(|| {
        register_int_counter_vec!(
...
pub fn shared_mempool_ack_inc(
    peer: &PeerNetworkId, direction: &str, label: &'static str,
) {
...
pub static CORE_MEMPOOL_INVARIANT_VIOLATION_COUNT: Lazy<IntCounter> =
    Lazy::new(|| {
        register_int_counter!(
