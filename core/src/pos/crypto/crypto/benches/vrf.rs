...
use diem_crypto::{traits::Uniform, VRFPrivateKey};
use rand::{rngs::ThreadRng, thread_rng, random};
use diem_crypto::ec_vrf::EcVrfPrivateKey;
...
fn compute(c: &mut Criterion) {
    ...
    c.bench_function("vrf proof generation", |b| {
        b.iter_batched(|| random::<u64>(), |nonce| priv_key.compute(&nonce.to_be_bytes()), BatchSize::SmallInput)
    });
}
