...
impl ToXDXExchangeRateUpdateEvent {
    /// Exchange rates are represented as 32|32 fixed-point numbers on-chain. So
    /// we divide by the scaling factor (2^32) of the number to arrive at
    /// the floating point representation of the number. The exchange rate
    /// returned is the on-chain rate to two decimal places rounded up (e.g.
    /// 1.3333 would be rounded to 1.34).
    pub fn new_to_xdx_exchange_rate(&self) -> f32 {
        let unrounded =
            (self.new_to_xdx_exchange_rate as f32) / 2f32.powf(32f32);
