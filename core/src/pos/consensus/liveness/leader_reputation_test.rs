...
use crate::liveness::{
    leader_reputation::{
        ActiveInactiveHeuristic, LeaderReputation, MetadataBackend,
        ReputationHeuristic,
    },
...
use diem_types::{
    block_metadata::NewBlockEvent, validator_signer::ValidatorSigner,
};
...
fn create_block(
    proposer: Author, voters: Vec<&ValidatorSigner>,
) -> NewBlockEvent {
    NewBlockEvent::new(
        0,
        proposer,
...
fn test_api() {
    ...
    let proposer_election: Box<dyn ProposerElection> =
        Box::new(leader_reputation);
    ...
    assert!(
        !proposer_election.is_valid_proposer(proposers[unexpected_index], 42)
    );
