...
pub trait MetadataBackend: Send + Sync {
    /// Return a contiguous BlockMetadata window in which last one is at
...
pub trait ReputationHeuristic: Send + Sync {
    ...
    fn get_weights(
        &self, candidates: &[Author], history: &[NewBlockEvent],
    ) -> Vec<u64>;
}

/// If candidate appear in the history, it's assigned active_weight otherwise
/// inactive weight.
...
impl ActiveInactiveHeuristic {
    pub fn new(
        author: Author, active_weight: u64, inactive_weight: u64,
    ) -> Self {
        Self {
            author,
            active_weight,
...
impl LeaderReputation {
    pub fn new(
        proposers: Vec<Author>, backend: Box<dyn MetadataBackend>,
        heuristic: Box<dyn ReputationHeuristic>,
    ) -> Self
    {
        Self {
...
impl ProposerElection for LeaderReputation {
    fn get_valid_proposer(&self, round: Round) -> Author {
        ...
        let mut weights =
            self.heuristic.get_weights(&self.proposers, &sliding_window);
    ...
    /// This function will return true for at most one proposal per valid
    /// proposer for a given round.
