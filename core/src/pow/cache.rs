...
use super::{
    ...
    shared::{
        epoch, get_cache_size, to_hex, Node, ETHASH_CACHE_ROUNDS, NODE_BYTES,
    },
};
...
impl NodeCacheBuilder {
    ...
    pub fn from_file<P: Into<Cow<'static, Path>>>(
        &self, cache_dir: P, block_height: u64,
    ) -> io::Result<NodeCache> {
...
impl NodeCache {
    pub fn cache_path(&self) -> &Path { &self.cache_path }

    pub fn flush(&mut self) -> io::Result<()> {
        if let Some(last) = self.epoch.checked_sub(2).map(|ep| {
            cache_path(
                self.cache_dir.as_ref(),
                &self.builder.epoch_to_ident(ep),
            )
        }) {
...
fn make_memmapped_cache(
    path: &Path, num_nodes: usize, ident: &H256,
) -> io::Result<MmapMut> {
    ...
    unsafe {
        initialize_memory(memmap.as_mut_ptr() as *mut Node, num_nodes, ident)
    };
    ...
    fn as_ref(&self) -> &[Node] {
        match self.cache {
            ...
            Either::Right(ref mmap) => unsafe {
                ...
                // This isn't a safety issue, so we can keep this a debug lint.
                // We don't care about people manually messing
                // with the files unless it can cause unsafety, but if we're
