use super::keccak::{keccak_512, keccak_256, H256};
use super::cache::{NodeCache, NodeCacheBuilder};
...
impl Light {
	pub fn new_with_builder(
		...
		Light { block_height, cache }
	}

	/// Calculate the light boundary data
	/// `header_hash` - The header hash to pack into the mix
	/// `nonce` - The nonce to pack into the mix
	pub fn compute(&self, header_hash: &H256, nonce: u64, block_height: u64) -> H256 {
...
pub fn slow_hash_block_height(block_height: u64) -> H256 {
	SeedHashCompute::resume_compute_seedhash([0u8; 32], 0, block_height / ETHASH_EPOCH_LENGTH)
}
...
fn hash_compute(light: &Light, full_size: usize, header_hash: &H256, nonce: u64) -> H256 {
