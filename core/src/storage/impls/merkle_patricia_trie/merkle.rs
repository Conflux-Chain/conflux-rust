...
///
/// buffer := children_merkles maybe_value;
...
pub fn compute_node_merkle(
    ...
    let mut buffer = Vec::with_capacity(
        std::mem::size_of::<ChildrenMerkleTable>()
...
///
/// compressed_path_info_byte := 128 * (no_first_nibble as bool as int) +
/// compressed_path.path_steps() % 128
...
) -> MerkleHash
{
    if compressed_path.path_slice().len() != 0 {
        ...
        // most significant bit = if the first nibble of the first byte
        // does not belong to the compressed path;
        // least significant 7 bits = number of nibbles in the compressed path %
        // 128.
        // The implementation uses 127 because path_steps doesn't minus 1 for
        // the non-existing first nibble.
        let path_info_byte = (127u8 * (without_first_nibble as u8))
            + (compressed_path.path_steps() as u8) % 128u8;
