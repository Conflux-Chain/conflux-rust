...
fn non_zero_bytes(mut number_of_keys: usize) -> u8 {
...
pub fn make_simple_mpt(mut values: Vec<Box<[u8]>>) -> FakeSnapshotMptDb {
    ...
    let index_byte_len = non_zero_bytes(keys);
    for (index, values) in values.drain(..).enumerate() {
        mpt_kvs.push((to_index_bytes(index, index_byte_len), values));
    }
