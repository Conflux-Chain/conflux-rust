...
impl<ValueType> KvdbSqliteSharded<ValueType> {
    ...
    ) -> Self
    {
        if let Some(connections) = shard_connections.as_ref() {
            assert!(connections.len() <= std::u16::MAX as usize);
...
impl<ValueType> KvdbSqliteSharded<ValueType> {
    ...
    pub fn open_or_create<P: AsRef<Path>>(
        ...
        if dir.as_ref().exists() {
            Ok((true, Self::open(num_shards, dir, statements)?))
        } else {
            Ok((
                false,
...
impl KeyPrefixToMap for i64 {
    fn key_prefix_to_map(&self) -> u32 {
        // We have to xor the first bit to correct the order
        // for negative key.
    ...
    > OwnedReadImplByFamily<KvdbSqliteSharded<ValueType>> for T
{
    fn get_mut_impl(&mut self, key: &[u8]) -> Result<Option<Self::ValueType>> {
        ...
        match maybe_connections {
            None => Ok(None),
            Some(connections) => {
                OwnedReadImplByFamily::<KvdbSqlite<ValueType>>::get_mut_impl(
                    &mut KvdbSqliteBorrowMut::new((
                        connections
                            .get_mut(key_to_shard_id(key, connections.len())),
                        statements,
                    )),
                    key,
                )
    ...
    fn get_mut_with_number_key_impl(
        ...
        match maybe_connections {
            None => Ok(None),
            Some(connections) => {
                OwnedReadImplByFamily::<KvdbSqlite<ValueType>>::get_mut_with_number_key_impl(
                    &mut KvdbSqliteBorrowMut::new((
                        connections.get_mut(number_key_to_shard_id(
                            key,
                            connections.len(),
                        )),
                        statements,
            ...
            + BindValueAppendImpl<<ValueType::Type as SqlBindableValue>::Kind>
{
    fn delete_impl(
        ...
        match maybe_connections {
            ...
            Some(connections) => {
                SingleWriterImplByFamily::<KvdbSqlite<ValueType>>::delete_impl(
                    &mut KvdbSqliteBorrowMut::new((
                        connections
                            .get_mut(key_to_shard_id(key, connections.len())),
                        statements,
    ...
    fn delete_with_number_key_impl(
        ...
        match maybe_connections {
            None => Err(Error::from(ErrorKind::DbNotExist)),
            Some(connections) => SingleWriterImplByFamily::<
                KvdbSqlite<ValueType>,
            >::delete_with_number_key_impl(
                &mut KvdbSqliteBorrowMut::new((
                    connections.get_mut(number_key_to_shard_id(
                        key,
                        connections.len(),
    ...
    fn put_impl(
        ...
        match maybe_connections {
            None => Err(Error::from(ErrorKind::DbNotExist)),
            Some(connections) => {
                SingleWriterImplByFamily::<KvdbSqlite<ValueType>>::put_impl(
                    &mut KvdbSqliteBorrowMut::new((
                        connections
                            .get_mut(key_to_shard_id(key, connections.len())),
                        statements,
