...
impl SnapshotDbManagerSqlite {
    ...
    fn try_make_cow_copy_impl(
        ...
        if cfg!(target_os = "linux") {
            ...
            command.arg("--reflink").arg(old_file).arg(new_file);
        } else if cfg!(target_os = "mac") {
            // APFS
            command = Command::new("cp");
            command.arg("-c").arg(old_file).arg(new_file);
        } else {
            return Ok(false);
        };
    ...
    fn try_make_snapshot_cow_copy_impl(
        ...
        fs::create_dir(new_snapshot_path).ok();
        for (old_db_file, new_db_file) in
            SnapshotDbSqlite::db_file_paths(old_snapshot_path)
                .into_iter()
                .zip(SnapshotDbSqlite::db_file_paths(new_snapshot_path))
        {
            let cow_copy_db_file_result =
                self.try_make_cow_copy_impl(&old_db_file, &new_db_file);
            if cow_copy_db_file_result.is_err()
                || !cow_copy_db_file_result.unwrap()
            {
                success = false;
                break;
            }
            for (old_temp_file, new_temp_file) in
                SqliteConnection::possible_temporary_files(&old_db_file)
                    .into_iter()
                    .zip(SqliteConnection::possible_temporary_files(
                        &new_db_file,
                    ))
            {
                match fs::copy(old_temp_file, new_temp_file) {
                    Err(_) => {
                        success = false;
                        break;
                    }
