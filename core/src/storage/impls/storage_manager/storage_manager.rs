    ...
    pub fn load_persist_state(&self) -> Result<()> {
        ...
        if snapshot_info_map.len() > 0 {
            ...
            for snapshot_epoch_id in missing_delta_db_snapshots {
                ...
                // Do not remove a snapshot which has intermediate delta mpt, because it
                // could be a freshly made snapshot before the previous shutdown.
                // A freshly made snapshot does not have delta db yet.
                if let Some(snapshot_info) = snapshot_info_map.get(&snapshot_epoch_id) {
                    if delta_mpts.contains_key(&snapshot_info.parent_snapshot_epoch_id) {
                        continue;
                    }
                }
                error!(
