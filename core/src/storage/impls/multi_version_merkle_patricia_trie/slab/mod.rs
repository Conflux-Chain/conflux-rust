...
use std::{cell::UnsafeCell, fmt, iter::IntoIterator, marker::PhantomData, mem, ops, ptr, slice};
...
impl<T, E: EntryTrait<T>> Slab<T, E> {
    ...
    pub fn capacity(&self) -> usize { 
        unsafe { self.entries.as_ref().capacity() }
    }
    ...
    pub fn reserve(&mut self, additional: usize) -> Result<()> {
        ...
        unsafe { self.entries.as_mut().reserve(need_add) };
    ...
    pub fn reserve_exact(&mut self, additional: usize) -> Result<()> {
        ...
        unsafe { self.entries.as_mut().reserve_exact(need_add) };
        let capacity = self.capacity();
        self.resize_up(old_capacity, capacity);
        Ok(())
    }
    ...
    pub fn shrink_to_fit(&mut self) {
        ...
        unsafe { self.entries.as_mut().shrink_to_fit() };
    }
    ...
    pub fn iter(&self) -> Iter<T, E> {
        Iter {
            entries: unsafe { self.entries.as_ref().iter() },
            curr: 0,
            value_type: PhantomData,
        }
    }
    ...
    pub fn iter_mut(&mut self) -> IterMut<T, E> {
        IterMut {
            entries: unsafe { self.entries.as_mut().iter_mut() },
            curr: 0,
            value_type: PhantomData,
        }
    }
    ...
    pub unsafe fn get_mut(&self, key: usize) -> Option<&mut T> {
        (*self.entries.get()).get_mut(key).and_then(|entry| entry.into_option_mut())
    }
    ...
    fn get_element_at(&self, key: usize) -> &mut E {
        unsafe { self.entries.as_mut().get_unchecked_mut(key) }
    }
    ...
    pub fn allocate(&self) -> Result<usize> {
        ...
        if key == unsafe { self.entries.as_ref().capacity() } {
            ...
            if key == alloc_fields.size_initialized {
                ...
                alloc_fields.next = unsafe { self.entries.as_mut() }[key].get_next_vacant_index();
            }
            Ok(key)
        }
    }
    ...
    pub fn remove(&self, key: usize) -> Result<T> {
        ...
        let old_value = match unsafe { self.entries.as_mut() }.get_mut(key) {
            Some(val) => {
                alloc_fields.used -= 1;
    ...
    where F: FnMut(usize, &mut T) -> bool {
        for i in 0..unsafe { self.entries.as_ref().len() } {
...
impl<T, E: EntryTrait<T>> ops::Index<usize> for Slab<T, E> {
    ...
    fn index(&self, key: usize) -> &T { 
        unsafe { self.entries.as_mut().get_unchecked(key) }
            .get_occupied_ref()
    }
}
...
/// A private helper trait.
trait UnsafeCellHelper {
    type Type;

    unsafe fn as_ref(&self) -> &Self::Type;
    unsafe fn as_mut(&self) -> &mut Self::Type;
}

impl<T> UnsafeCellHelper for UnsafeCell<T> {
    type Type = T;

    unsafe fn as_ref(&self) -> &T { &*self.get() }
    unsafe fn as_mut(&self) -> &mut T { &mut *self.get() }
}
