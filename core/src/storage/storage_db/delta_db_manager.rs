...
pub trait DeltaDbManagerTrait {
    ...
    fn delta_db_name(snapshot_epoch_id: &EpochId) -> String {
        String::from(snapshot_epoch_id.to_hex())
    }
    ...
    fn scan_persist_state(
        ...
        for (snapshot_epoch_id, snapshot_info) in snapshot_info_map.iter() {
            // Delta MPT
            possible_delta_db_paths.insert(
                self.get_delta_db_path(&Self::delta_db_name(snapshot_epoch_id))
                    .into_bytes(),
                snapshot_epoch_id.clone(),
            );
            // Intermediate Delta MPT
            possible_delta_db_paths.insert(
                self.get_delta_db_path(&Self::delta_db_name(
        ...
        for entry in fs::read_dir(self.get_delta_db_dir())? {
            ...
            if !possible_delta_db_paths.contains_key(dir_name.as_bytes()) {
                ...
                delta_mpts.insert(
                    snapshot_epoch_id,
                    self.get_delta_db(&Self::delta_db_name(
                        &snapshot_epoch_id,
                    ))?
                    .unwrap(),
                );
