...
impl SnapshotDbTrait for Arc<Mutex<FakeSnapshotDb>> {
    ...
    fn open(_snapshot_path: &str) -> Result<Option<Self>> { unreachable!() }

    fn create(_snapshot_path: &str) -> Result<Self> { unreachable!() }

    fn direct_merge(&mut self) -> Result<H256> { unreachable!() }
...
impl SnapshotDbManagerTrait for FakeSnapshotDbManager {
    ...
    fn get_snapshot_db_path(&self, _snapshot_epoch_id: &H256) -> String {
        unreachable!()
    }
