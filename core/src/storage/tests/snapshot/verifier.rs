...
fn test_slice_verifier() {
    ...
    /*
    FullSyncVerifier::new(
        slicer_chunk_proofs.len(), slicer_chunk_bounds,
        slicer_chunk_proofs[0..slicer_chunk_bounds.len()].iter().map(|x| x.clone().unwrap()),
        merkle_root, fake_snapshot_db_manager, EpochId::default(),
    )
    */
}
