...
pub struct VerifiedStateView<'a> {
    ...
    account_to_btree_cache:
        RefCell<HashMap<AccountAddress, BTreeMap<Vec<u8>, Vec<u8>>>>,
    ...
    fn get(&self, access_path: &AccessPath) -> Result<Option<Vec<u8>>> {
        ...
        match self.account_to_btree_cache.borrow_mut().entry(address) {
            ...
            Entry::Vacant(vacant) => {
                ...
                let account_blob_option = match self
                    .speculative_state
                    .get(address_hash)
                {
                    AccountState::ExistsInScratchPad(blob) => Some(blob),
                    AccountState::DoesNotExist => None,
                    // No matter it is in db or unknown, we have to query from
                    // db since even the former case, we
                    // don't have the blob data but only its hash.
                    AccountState::ExistsInDB | AccountState::Unknown => {
                        let (blob, proof) = match self.latest_persistent_version
                        {
                            ...
                            None => {
                                (None, SparseMerkleProof::new(None, vec![]))
                            }
                        };
                        proof
                            .verify(
