...
//!
//! When we commit a transaction, for example T_i, we will first send its write
//! set to storage. Once the writes to storage complete, any node reachable from
//! S_i will be available in storage. Therefore we start from S_i and
//! recursively drop its descendant. For internal or leaf nodes (for example
//! node o in the above example), we do not know if there are other nodes (for
//! example S_{i+1} in the above example) pointing to it, so we replace the node
//! with a subtree node with the same hash. This allows us to clean up memory as
...
/// `AccountState` describes the result of querying an account from this
/// SparseMerkleTree.
...
pub enum AccountState {
    ...
    /// use the value hash to look up the account blob directly.
    ExistsInDB,
...
impl SparseMerkleTree {
    ...
    pub fn update(
        &self, updates: Vec<(HashValue, AccountStateBlob)>,
    ...
    fn update_one(
        root: Arc<SparseMerkleNode>, key: HashValue,
        new_blob: AccountStateBlob, proof_reader: &impl ProofRead,
    ) -> Result<Arc<SparseMerkleNode>, UpdateError>
    {
        ...
        loop {
            let next_node =
                if let Node::Internal(node) = &*current_node.read_lock() {
        ...
        // leaf, a subtree or empty. We construct a new subtree like we
    ...
    fn construct_subtree_at_bottom(
        current_node: Arc<SparseMerkleNode>, key: HashValue,
        new_blob: AccountStateBlob, remaining_bits: HashValueBitIterator,
        proof_reader: &impl ProofRead,
    ) -> Result<Arc<SparseMerkleNode>, UpdateError>
    {
        match &*current_node.read_lock() {
            Node::Internal(_) => unreachable!(
                "Reached an internal node at the bottom of the tree."
            ),
            ...
            Node::Subtree(_) => {
                // When the search reaches an Subtree node, we need proof to to
                // give us more information about this part of
                // the tree.
                let proof = proof_reader
                    .get_proof(key)
                    .ok_or(UpdateError::MissingProof)?;

                // Here the in-memory tree is identical to the tree in storage
                // (we have only the root hash of this subtree
                // in memory). So we need to take into account the leaf
                // in the proof.
                let new_subtree = match proof.leaf() {
                    Some((existing_key, existing_value_hash)) => {
                        let existing_leaf = LeafNode::new(
                            existing_key,
                            LeafValue::BlobHash(existing_value_hash),
                        );
                    ...
                    None => Arc::new(SparseMerkleNode::new_leaf(
                        key,
                ...
                Ok(Self::construct_subtree(
                    ...
                    proof
                        ...
                        .iter()
                        .take(
                            num_remaining_bits + proof_length
                                - HashValue::LENGTH_IN_BITS,
                        )
                        .map(|sibling_hash| {
                            Arc::new(
    ...
    fn construct_subtree_with_new_leaf(
        key: HashValue, new_blob: AccountStateBlob, existing_leaf: &LeafNode,
        distance_from_root_to_existing_leaf: usize,
    ) -> Arc<SparseMerkleNode>
    {
        let new_leaf = Arc::new(SparseMerkleNode::new_leaf(
            key,
            LeafValue::Blob(new_blob),
        ));
        ...
        Self::construct_subtree(
            ...
            .chain(
                std::iter::repeat(Arc::new(SparseMerkleNode::new_empty()))
                    .take(extension_len),
            ),
            new_leaf,
        )
        ...
        let ret = match &*current_node.read_lock() {
            Node::Leaf(node) => {
                if key == node.key() {
                    match node.value() {
                        LeafValue::Blob(blob) => {
                            AccountState::ExistsInScratchPad(blob.clone())
                        }
    ...
    pub fn root_hash(&self) -> HashValue { self.root.read_lock().hash() }

    /// Prunes a tree by replacing every node reachable from root with a subtree
    /// node that has the same hash. If a node is empty or a subtree, we
    /// don't need to do anything. For example in the following case, if we
    /// drop `S_i`, we will replace o with a subtree node, then `o` no
    /// longer has pointers to its children `x` and `A`, so they will be dropped
    /// automatically. ```text
    ///            S_i        S_{i+1}                               S_{i+1}
    ///           /   \      /       \                             /       \
    ///          /     y   /          \          drop(S_i)        o         y'
