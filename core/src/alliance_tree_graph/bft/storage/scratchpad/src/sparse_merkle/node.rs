...
//! This module defines all kinds of nodes in the Sparse Merkle Tree maintained
//! in scratch pad. There are four kinds of nodes:
//!
...
//! is the hash of the corresponding account blob. The difference is that a
//! `LeafNode` does not always have the value, in the case when the leaf was
//! loaded into memory as part of a non-inclusion proof.
//!
//! - A `SubtreeNode` represents a subtree with one or more leaves.
//!   `SubtreeNode`s are generated
//! when we get accounts from storage with proof. It stores the root hash of
//! this subtree.
//!
...
/// We wrap the node in `RwLock`. The only case when we will update the node is
/// when we drop a subtree originated from this node and commit things to
/// storage. In that case we will replace the an `InternalNode` or a `LeafNode`
/// with a `SubtreeNode`.
#[derive(Debug)]
...
impl SparseMerkleNode {
    ...
    pub fn new_internal(
        left_child: Arc<SparseMerkleNode>, right_child: Arc<SparseMerkleNode>,
    ) -> Self {
        SparseMerkleNode {
...
/// The underlying node is either `InternalNode`, `LeafNode`, `SubtreeNode` or
/// `EmptyNode`.
#[derive(Debug)]
pub enum Node {
...
impl Node {
    ...
    pub fn new_empty() -> Self { Node::Empty }
...
impl InternalNode {
    ...
    fn hash(&self) -> HashValue { self.hash }
...
pub struct LeafNode {
    ...
    /// The account blob or its hash. It's possible that we don't know the
    /// value here. For example, this leaf was loaded into memory as part
    /// of an non-inclusion proof. In that case we only know the value's
    /// hash.
    value: LeafValue,
...
impl LeafNode {
    ...
    pub fn value(&self) -> &LeafValue { &self.value }
...
impl SubtreeNode {
    ...
    pub fn hash(&self) -> HashValue { self.hash }
}
