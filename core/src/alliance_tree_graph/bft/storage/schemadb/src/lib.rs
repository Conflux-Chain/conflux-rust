...
//! enforces a set of Libra specific DB options, like custom comparators and
...
use rocksdb::{
    rocksdb_options::ColumnFamilyDescriptor, CFHandle, DBOptions, Writable,
    WriteOptions,
};
use std::{
...
lazy_static! {
    static ref OP_COUNTER: OpMetrics =
        OpMetrics::new_and_registered("schemadb");
}
...
/// Type alias to improve readability.
pub type ColumnFamilyOptionsMap =
    HashMap<ColumnFamilyName, ColumnFamilyOptions>;
...
/// Checks underlying Rocksdb instance existence by checking `CURRENT` file
/// existence, the same way Rocksdb adopts to detect db existence.
...
impl DB {
    ...
    pub fn open<P: AsRef<Path>>(
        path: P, mut cf_opts_map: ColumnFamilyOptionsMap,
    ) -> Result<Self> {
        let mut db_opts = DBOptions::new();

        // For now we set the max total WAL size to be 1G. This config can be
        // useful when column families are updated at non-uniform
        // frequencies.
        ...
        let mut db = DB::open_cf(
            db_opts,
            path,
            vec![cf_opts_map.remove_entry(&DEFAULT_CF_NAME).ok_or_else(
                || format_err!("No \"default\" column family name found"),
            )?],
        )?;
        cf_opts_map
            .into_iter()
    ...
    where T: Into<ColumnFamilyDescriptor<'a>> {
        let _cf_handle =
            self.inner.create_cf(cfd).map_err(convert_rocksdb_err)?;
        Ok(())
    }
    ...
    pub fn get<S: Schema>(
        ...
        OP_COUNTER.observe_duration(
            &format!("db_get_{}", S::COLUMN_FAMILY_NAME),
            time.elapsed(),
        );
        result
            .map(|raw_value| {
    ...
    pub fn get_approximate_sizes_cf(&self) -> Result<BTreeMap<String, u64>> {
        ...
        for cf_name in
            self.inner.cf_names().into_iter().map(ToString::to_string)
        {
            let cf_handle = self.get_cf_handle(&cf_name)?;
            let size = self
                .inner
    ...
    /// Flushes all memtable data. If `sync` is true, the flush will wait until
    /// it's done. This is only used for testing `get_approximate_sizes_cf`
    /// in unit tests.
