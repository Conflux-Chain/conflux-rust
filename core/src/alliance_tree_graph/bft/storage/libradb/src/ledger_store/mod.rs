...
impl LedgerStore {
    ...
    /// Returns the ledger infos reflecting epoch bumps with their 2f+1
    /// signatures in [`start_epoch`, `end_epoch`). If there is no more than
    /// `limit` results, this function returns all of them, otherwise the
    /// first `limit` results are returned and a flag (when true) will be
    /// used to indicate the fact that there is more.
    ...
    pub fn get_latest_ledger_info(&self) -> Result<LedgerInfoWithSignatures> {
        self.get_latest_ledger_info_option().ok_or_else(|| {
            LibraDbError::NotFound(String::from("Genesis LedgerInfo")).into()
        })
    }
    ...
    /// `(LedgerInfoWithSignatures, Option<ValidatorSet>)`: either the first
    /// element carries a validator set and the second element is `None`, or
    /// the second element carries a validator set.
    pub fn get_startup_info(
        &self,
    ) -> Result<Option<(LedgerInfoWithSignatures, Option<ValidatorSet>)>> {
        let latest_ledger_info_with_sigs =
            match self.get_latest_ledger_info_option() {
                Some(x) => x,
                None => return Ok(None),
            };
        ...
        // If the latest LedgerInfo doesn't carry a validator set, we look for
        // the previous LedgerInfo which should always carry a validator
        // set.
        ...
        let prev_ledger_info_with_sigs = self
            ...
            .ok_or_else(|| {
                format_err!("At least one epoch change LedgerInfo must exist.")
            })?;
        ...
        let latest_validator_set = prev_ledger_info_with_sigs
            ...
            .ok_or_else(|| {
                format_err!(
                    "All previous LedgerInfo should have a validator set."
                )
            })?;
        Ok(Some((
    ...
    pub fn get_latest_transaction_info(
        ...
        self.get_latest_transaction_info_option()?.ok_or_else(|| {
            LibraDbError::NotFound(String::from("Genesis TransactionInfo."))
                .into()
        })
    }
