...
//! This crate provides [`LibraDB`] which represents physical storage of the
//! core Libra data structures.
//!
//! It relays read/write operations on the physical storage via [`schemadb`] to
//! the underlying Key-Value storage system, and implements libra data
...
const MAX_NUM_EPOCH_CHANGE_LEDGER_INFO: usize = usize::max_value();
...
/// and provides APIs for access to the core Libra data structures.
pub struct LibraDB {
    db: Arc<DB>,
    ledger_store: LedgerStore,
    transaction_store: TransactionStore,
...
impl LibraDB {
    ...
    pub fn new<P: AsRef<Path> + Clone>(db_root_path: P) -> Self {
        ...
        let db = Arc::new(
            DB::open(path.clone(), cf_opts_map).expect("LibraDB open failed"),
        );
    ...
    /// Returns events specified by `query_path` with sequence number in range
    /// designated by `start_seq_num`, `ascending` and `limit`. If ascending
    /// is true this query will return up to `limit` events that were
    ...
    /// epoch. If there are no more than `MAX_NUM_EPOCH_CHANGE_LEDGER_INFO`
    /// results, this function returns all of them, otherwise the first
    /// `MAX_NUM_EPOCH_CHANGE_LEDGER_INFO` results are returned and a flag
    ...
    pub fn save_ledger_info(
        &self, ledger_info_with_sigs: &Option<LedgerInfoWithSignatures>,
    ) -> Result<()> {
    ...
    ) -> Result<()>
    {
        ...
        // synchronization. In this case txns_to_commit should not be
        // empty. Otherwise it is okay to commit empty blocks.
        ensure!(
        ...
        // If expected ledger info is provided, verify result root hash and save
        // the ledger info.
        if let Some(x) = ledger_info_with_sigs {
        ...
        // Only increment counter if commit succeeds and there are at least one
        // transaction written to the storage. That's also when we'd
        // inform the pruner thread to work.
        if num_txns > 0 {
            ...
            counters
                .expect(
                    "Counters should be bumped with transactions being saved.",
                )
                .bump_op_counters();
    ...
    */
    ...
    /// This backs the `UpdateToLatestLedger` public read API which returns the
    /// latest [`LedgerInfoWithSignatures`] together with items requested
    /// and proofs relative to the same ledger info.
    pub fn update_to_latest_ledger(
        ...
        // TODO: cache last epoch change version to avoid a DB access in most
        // cases.
        ...
        // If the validator change proof in the response is enough for the
        // client to update to latest LedgerInfo, fulfill all request
        // items. Otherwise the client will not be able to verify the
        // latest LedgerInfo, so do not send response items back.
    ...
    fn get_response_items(
        &self, request_items: Vec<RequestItem>, ledger_version: Version,
        ...
        request_items
            ...
            .map(|request_item| match request_item {
                ...
                RequestItem::GetAccountTransactionBySequenceNumber {
                    ...
                    let proof_of_current_sequence_number =
                        match transaction_with_proof {
                            Some(_) => None,
    ...
    /// indicated by the state Merkle tree root hash.
    ///
    ...
    /// Gets information needed from storage during the startup of the executor
    /// or state synchronizer module.
    ///
    /// This is used by the libra core (executor, state synchronizer)
    /// internally.
    pub fn get_startup_info(&self) -> Result<Option<StartupInfo>> {
        ...
        */
        ...
        let startup_info = if latest_tree_state.version != li_version {
            // We synced to some version ahead of the version of the latest
            // ledger info. Thus, we are still in sync mode.
            let committed_version = li_version;
            let committed_txn_info =
                self.ledger_store.get_transaction_info(committed_version)?;
            let committed_account_state_root_hash =
                committed_txn_info.state_root_hash();
            ...
            // The version of the latest ledger info matches other data. So the
            // storage is not in sync mode.
            StartupInfo::new(
                latest_ledger_info,
                latest_validator_set,
                //latest_tree_state,
