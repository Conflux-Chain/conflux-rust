...
use libra_types::{
    ...
    proptest_types::{
        AccountInfoUniverse, LedgerInfoWithSignaturesGen,
        TransactionToCommitGen,
    },
};
...
fn to_blocks_to_commit(
    ...
    // Use temporary LibraDB and STORE LEVEL APIs to calculate hashes on a per
    ...
    let blocks_to_commit = partial_blocks
        ...
        .map(|(txns_to_commit, partial_ledger_info_with_sigs)| {
            ...
            let partial_ledger_info =
                partial_ledger_info_with_sigs.ledger_info();
