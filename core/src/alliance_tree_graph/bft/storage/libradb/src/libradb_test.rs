...
fn verify_epochs(
    db: &LibraDB, ledger_infos_with_sigs: &[LedgerInfoWithSignatures],
) {
...
fn get_events_by_query_path(
    db: &LibraDB, ledger_info: &LedgerInfo, query_path: &AccessPath,
    first_seq_num: u64, last_seq_num: u64, ascending: bool, is_latest: bool,
) -> Result<Vec<ContractEvent>>
{
    ...
    let mut ret = Vec::new();
    loop {
        let (events_with_proof, proof_of_latest_event) = db
...
fn verify_events_by_query_path(
    db: &LibraDB, events: Vec<(AccessPath, Vec<ContractEvent>)>,
...
fn group_events_by_query_path(
    ...
    for txn in txns_to_commit {
        for (address, account_blob) in txn.account_states().iter() {
            ...
            let account = AccountResource::make_from(&account_btree)
                .expect("AccountResource is not found");
    ...
    let mut query_path_to_events: HashMap<AccessPath, Vec<ContractEvent>> =
        HashMap::new();
    for txn in txns_to_commit {
...
fn verify_committed_transactions(
    db: &LibraDB, txns_to_commit: &[TransactionToCommit],
