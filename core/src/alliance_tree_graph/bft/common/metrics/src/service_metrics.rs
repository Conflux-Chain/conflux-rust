...
pub struct ServiceMetrics {
    // This struct is going to define a MetricFamily (per metric, this is how
    // the library works). We'll have a metric per counter, with the
    // methods as dimensions (labels): e.g., calc_service.req{method =
    // "add"} = +1 e.g., calc_service.duration_sum{method="add"} = 6
    num_req: IntCounterVec,
    num_error: IntCounterVec,
    duration: HistogramVec, // we want .avg and .p90 (but really p99)
    message_size: HistogramVec, /* collect the size of messages, up to
                             * max-send-size */
}
    ...
    pub fn default() -> ServiceMetrics {
        ...
        ServiceMetrics {
            num_req: IntCounterVec::new(
                Opts::new("num_req", "Number of requests"),
                &["method"],
            )
            .unwrap(),
            num_error: IntCounterVec::new(
                Opts::new("num_error", "Number of errors"),
                &["method"],
            )
            .unwrap(),
