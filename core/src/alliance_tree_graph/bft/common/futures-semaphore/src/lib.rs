...
//! [`std::sync::Condvar`] style semaphore will block the entire OS thread,
//! which could potentially deadlock the futures runtime if enough tasks are
...
use tokio_sync::semaphore::{
    AcquireError, Permit as TokioPermit, Semaphore as TokioSemaphore,
};

/// The wrapped tokio_sync [`Semaphore`](TokioSemaphore) and total permit
/// capacity.
#[derive(Debug)]
struct Inner {
...
impl Semaphore {
    ...
    pub fn is_full(&self) -> bool { self.available_permits() == 0 }
...
impl Permit {
    fn release(&mut self) { self.permit.release(&self.inner.semaphore); }
}
...
impl Future for PermitFuture {
    ...
    fn poll(
        mut self: Pin<&mut Self>, context: &mut Context,
    ) -> Poll<Self::Output> {
