...
//! libra_channel provides an mpsc channel which has two ends
//! `libra_channel::Receiver` and `libra_channel::Sender` similar to existing
...
struct SharedState<K: Eq + Hash + Clone, M> {
    ...
    /// Waker is needed so that the Sender can notify the task
    /// executor/scheduler that something has been pushed to the
    /// internal_queue and it ready for consumption by the Receiver and
    /// then the executor/scheduler will wake up the Receiver task to
