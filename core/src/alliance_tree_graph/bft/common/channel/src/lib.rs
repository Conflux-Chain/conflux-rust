        ...
        loop {
            match Pin::new(&mut self.inner).poll_next(cx) {
                Poll::Ready(Some(msg)) => {
                    ...
                    if Instant::now().duration_since(msg.entry_time)
                        > self.timeout
                    {
                        warn!(
                            "Message dropped due to timeout: {:?}",
                            msg.value
                        );
                        continue;
                    } else {
...
pub fn new_with_timeout<T>(
    size: usize, gauge: &IntGauge, timeout: Duration,
) -> (Sender<T>, Receiver<T>) {
    gauge.set(0);
...
pub fn new_test_with_timeout<T>(
    size: usize, timeout: Duration,
) -> (Sender<T>, Receiver<T>) {
