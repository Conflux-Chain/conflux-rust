...
use crate::{
    json_log,
    proto::{
        node_debug_interface_server::NodeDebugInterface, Event,
        GetEventsRequest, GetEventsResponse, GetNodeDetailsRequest,
...
impl NodeDebugInterface for NodeDebugService {
    async fn get_node_details(
        &self, _request: Request<GetNodeDetailsRequest>,
    ...
    async fn get_events(
        &self, _request: Request<GetEventsRequest>,
        ...
        for event in json_log::pop_last_entries() {
            ...
            let serialized_event = serde_json::to_string(&event.json)
