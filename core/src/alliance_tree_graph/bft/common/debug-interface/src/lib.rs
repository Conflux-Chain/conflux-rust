...
use crate::proto::{
    node_debug_interface_client::NodeDebugInterfaceClient, GetEventsRequest,
    GetEventsResponse, GetNodeDetailsRequest,
};
...
pub struct NodeDebugClient {
    // Currently the runtime but be ordered before the tonic client to ensure
    // that the runtime is dropped last when this struct is dropped.
...
impl NodeDebugClient {
    ...
    fn client(
        ...
        if self.client.is_none() {
            self.client = Some(self.rt.block_on(
                NodeDebugInterfaceClient::connect(self.addr.clone()),
            )?);
        }
    ...
    pub fn get_node_metric<S: AsRef<str>>(
        &mut self, metric: S,
    ) -> Result<Option<i64>> {
