...
///
/// This function will attempt to interpret `bytes` as the LCS serialized form
/// of `T` and deserialize `T` from `bytes`.
///
/// # Examples
///
/// ```
...
impl<'de> Deserializer<'de> {
    ...
    /// input.
    fn new(input: &'de [u8]) -> Self { Deserializer { input } }
...
impl<'de> Deserializer<'de> {
    ...
    fn parse_u8(&mut self) -> Result<u8> { self.next() }
...
impl<'de, 'a> de::Deserializer<'de> for &'a mut Deserializer<'de> {
    ...
    // LCS is not a self-describing format so we can't implement
    // `deserialize_any`
    ...
    fn deserialize_tuple_struct<V>(
        mut self, _name: &'static str, len: usize, visitor: V,
    ) -> Result<V::Value>
    where V: Visitor<'de> {
        visitor.visit_seq(SeqDeserializer::new(&mut self, len))
    }
    ...
    fn deserialize_struct<V>(
        mut self, _name: &'static str, fields: &'static [&'static str],
        visitor: V,
    ) -> Result<V::Value>
    where
        V: Visitor<'de>,
