...
// For some reason deriving `Arbitrary` results in clippy firing a `unit_arg`
// violation
#![allow(clippy::unit_arg)]

use libra_canonical_serialization::{
    from_bytes, to_bytes, Error, MAX_SEQUENCE_LENGTH,
};
...
fn is_same<T>(t: T)
where T: Serialize + DeserializeOwned + fmt::Debug + PartialEq {
...
fn cow() {
    ...
    {
        let serialized =
            to_bytes(&Message::M1(Cow::Borrowed(&large_object))).unwrap();
    ...
    {
        let serialized =
            to_bytes(&Message::M2(Cow::Borrowed(&large_map))).unwrap();
        let deserialized: Message<'static> = from_bytes(&serialized).unwrap();
...
fn serde_known_vector() {
    ...
    let test_vector = vec![
        ...
        0x64, 0x63, 0x58, 0x4d, 0x42, 0x37, 0x64, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x09, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
        0x06, 0x07, 0x08, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05,
        0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05,
        0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x63,
        0x00, 0x00, 0x00, 0x01, 0x03, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x01, 0x03, 0x00, 0x00, 0x00, 0x16, 0x15, 0x43, 0x03, 0x00, 0x00, 0x00,
        0x00, 0x38, 0x15, 0x03, 0x00, 0x00, 0x00, 0x16, 0x0a, 0x05, 0x04, 0x00,
        0x00, 0x00, 0x14, 0x15, 0x59, 0x69, 0x03, 0x00, 0x00, 0x00, 0xc9, 0x17,
        0x5a,
    ];
