...
/// The `SecurityLog` struct is used to log security-sensitive operations, for
/// instance when an invalid signature is detected or when an unexpected event
/// happens.
///
/// The `security_log()` function should be used to instantiate this struct. It
/// can be decorated with different type of metadata:
...
///
/// All these information can be defined by using the appropriate function with
/// the same name.
///
/// The method `log()` needs to be called to ensure that the event is actually
/// printed.
///
/// # Example:
/// ```rust
...
///     .log();
/// ```
/// In this example, `security_log()` logs an event of type
/// `SecurityEvent::InvalidTransactionAC`, having `TestError::Error` as
/// application error, a `SimpleData` struct and a `String` as additional
/// metadata, and a backtrace that samples 100% of the times.
...
impl SecurityLog {
    ...
    /// Adds additional metadata to the `SecurityLog` struct. The argument needs
    /// to implement the `std::fmt::Debug` trait.
    ...
    /// Adds an application error to the `SecurityLog` struct. The argument
    /// needs to implement the `std::fmt::Debug` trait.
