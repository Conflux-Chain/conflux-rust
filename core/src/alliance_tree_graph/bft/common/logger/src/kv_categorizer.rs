...
/// Structures implementing this trait are being used to categorize the KV
/// values into one of the `KVCategory`.
...
impl KVCategorizer for InlineCategorizer {
    fn categorize(&self, _key: Key) -> KVCategory { KVCategory::Inline }

    fn name(&self, key: Key) -> &'static str { key }
}

/// Used to properly print `error_chain` `Error`s. It displays the error and
/// it's causes in separate log lines as well as backtrace if provided.
/// The `error_chain` `Error` must implement `KV` trait. It is recommended to
/// use `impl_kv_error` macro to generate the implementation.
