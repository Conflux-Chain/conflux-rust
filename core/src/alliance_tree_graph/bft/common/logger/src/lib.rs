...
// This is really annoying. The `error!` and other macros in `slog_scope` depend
// on the `slog_error!` and other macros exported by `slog`. They need to be
// exported into the environment for the `slog_scope` macros to pick them up.
// However if you use `#[macro_use]` then the linter complains about unused
// imports. Ugh.
...
pub use slog::{
    slog_crit, slog_debug, slog_error, slog_info, slog_trace, slog_warn,
};
...
pub fn set_default_global_logger(
    async_drain: bool, chan_size: Option<usize>,
) -> GlobalLoggerGuard {
...
fn create_default_root_logger(
    ...
    let drain = GlogFormat::new(
        PlainDecorator::new(::std::io::stderr()),
        ErrorCategorizer,
    )
    .fuse();
...
fn create_env_logger_with_level<D>(
    drain: D, level: FilterLevel,
) -> EnvLogger<D>
...
/// Caveat: cargo test does not capture output for non main thread. So this
/// logger is not very useful for multithreading scenarios.
fn create_test_root_logger() -> Logger {
    let drain =
        GlogFormat::new(TermDecorator::new().build(), ErrorCategorizer).fuse();
...
where D: Drain<Err = Never, Ok = ()> + Send + 'static {
    if is_async {
        let async_builder = match chan_size {
            Some(chan_size_inner) => {
                Async::new(drain).chan_size(chan_size_inner)
            }
