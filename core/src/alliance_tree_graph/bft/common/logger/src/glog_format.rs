...
impl<D: Decorator, C: KVCategorizer> GlogFormat<D, C> {
    /// Create a glog-formatted `Drain` using the provided `Decorator`, and
    /// `Categorizer`
...
fn write_logline(
    decorator: &mut dyn RecordDecorator, level: Level,
...
fn print_inline_kv<C: KVCategorizer>(
    decorator: &mut dyn RecordDecorator, categorizer: &C,
    kv: Vec<(Key, String)>,
) -> io::Result<()>
{
    for (k, v) in kv {
...
impl<D: Decorator, C: KVCategorizer> Drain for GlogFormat<D, C> {
    ...
    fn log(
        &self, record: &Record<'_>, values: &OwnedKVList,
    ) -> io::Result<Self::Ok> {
        self.decorator.with_record(record, values, |decorator| {
            let (inline_kv, level_kv): (Vec<_>, Vec<_>) = {
                let mut serializer =
                    CollectorSerializer::new(&self.categorizer);
...
mod tests {
    ...
    fn test_inline() {
        ...
        // For the timestamp, it's sufficient to just check it has the right
        // form.
        let captures =
            LOG_REGEX.captures(log_string.as_str().trim_end()).unwrap();
        assert_eq!(
