...
/// This serializer collects all KV pairs into a Vec, converting the values to
/// `String`. It filters out the one that are of `KVCategory::Ignore`
pub struct CollectorSerializer<'a, C: KVCategorizer>(Vec<(Key, String)>, &'a C);

impl<'a, C: KVCategorizer> CollectorSerializer<'a, C> {
    /// Create a collector serializer that will use the given categorizer to
    ...
    pub fn into_inner(self) -> Vec<(Key, String)> { self.0 }
}
...
mod tests {
    ...
    impl KV for TestKv {
        fn serialize(
            &self, _record: &Record<'_>, serializer: &mut dyn Serializer,
        ) -> SlogResult {
            serializer
                .emit_none(self.key)
    ...
    impl KVCategorizer for TestCategorizer {
        ...
        fn ignore(&self, key: Key) -> bool { key.starts_with("ignore") }
    }
