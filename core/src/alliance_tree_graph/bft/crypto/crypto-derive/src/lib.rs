...
//! - the derive macros for `libra_crypto::traits`, namely `ValidKey`,
...
//!   traits.
//! - the derive macro for `libra_crypto::hash::CryptoHasher`, which defines the
//!   domain-separation hasher structures described in `libra_crypto::hash`
//!   (look there for details). This derive macro has for sole difference that
//!   it automatically picks a unique salt for you, using the path of the
//!   structure + its name. I.e. for a Structure Foo defined in bar::baz::quux,
//!   it will define the equivalent of: ```ignore define_hasher! { ( FooHasher,
//!   FOO_HASHER, b"bar::baz::quux::Foo" ) } ```
//!
...
pub fn derive_enum_validkey(input: TokenStream) -> TokenStream {
    ...
    match ast.data {
        ...
        Data::Struct(_) | Data::Union(_) => {
            panic!("#[derive(ValidKey)] is only defined for enums")
        }
    }
}
...
pub fn hasher_dispatch(input: TokenStream) -> TokenStream {
    ...
    let fn_name = get_type_from_attrs(&item.attrs, "CryptoHasherSalt")
        .unwrap_or_else(|_| {
            syn::LitStr::new(&item.ident.to_string(), Span::call_site())
        });
