...
use libra_crypto::hash::{
    CryptoHash, CryptoHasher, HashValue, SparseMerkleInternalHasher,
};
...
fn bench_hasher(c: &mut criterion::Criterion) {
    let expected_digest = HashValue::from_hex(
        "a2a3adfaed90739641d57dc3d0f8e6231b0d038748dad0feaec765c9aa9676a6",
    )
    .unwrap();
    ...
    c.bench_function("hashing", |b| {
        b.iter(|| {
            let digest =
                HashBencher::bench_hash(criterion::black_box(b"someinput"));
