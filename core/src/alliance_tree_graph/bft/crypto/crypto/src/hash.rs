...
//! saying    "I am Alice". naturally, Alice is willing to sign this message,
//! since she    is in fact Alice. However, unbeknownst to Alice, in application
//! Y,    messages beginning with the letter "I" represent transfers. " am "
...
//! ```
//! The output is of type [`HashValue`], which can be used as an input for
//! signing.
//!
...
//! ```
//!
//! The macro will define a hasher automatically called `MyNewStructHasher`, and
//! pick a salt equal to the full module path + "::"  + structure name, i.e. if
//! `MyNewStruct` is defined in `bar::baz::quux`, the salt will be
//! `b"bar::baz::quux::MyNewStruct"`. You can then use it in your implementation
//! of `CryptoHash` (see below).
//!
//! ## The semi-automatic way
//!
//! For any new structure `MyNewStruct` that needs to be hashed, the developer
//! should define a new hasher with:
//!
//! ```
...
//! ```
//!
//! **Note**: The last argument for the `define_hasher` macro must be a unique
//! string.
//!
//! ## The `CryptoHash` implementation (for both automatic and semi-automatic
//! way) Then, the `CryptoHash` trait should be implemented:
//! ```
...
impl HashValue {
    ...
    pub fn is_zero(&self) -> bool { *self == HashValue::zero() }
    ...
    /// Convenience function to compute a sha3-256 HashValue of the buffer. It
    /// will handle hasher creation, data feeding and finalization.
    ...
    pub fn to_hex(&self) -> String { hex::encode(self.hash).to_string() }
...
/// A type that implements `CryptoHash` can be hashed by a cryptographic hash
/// function and produce a `HashValue`. Each type needs to have its own `Hasher`
/// type.
...
///
/// Instances of `CryptoHasher` usually represent state that is changed while
/// hashing data. Similar to `std::hash::Hasher` but not same. CryptoHasher
/// cannot be reused after finish() has been called.
pub trait CryptoHasher: Default {
    ...
    fn write_u8(&mut self, byte: u8) { self.write(&[byte]); }
}
