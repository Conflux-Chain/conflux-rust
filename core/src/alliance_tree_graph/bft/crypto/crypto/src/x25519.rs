...
//! `X25519StaticPublicKey`) as well as short-term keys
//! (`X25519EphemeralPrivateKey`, `X25519PublicKey`).
//!
...
impl X25519StaticPrivateKey {
    ...
    pub fn derive_keypair_from_seed(
        salt: Option<&[u8]>, seed: &[u8], app_info: Option<&[u8]>,
    ) -> (X25519StaticPrivateKey, X25519StaticPublicKey) {
        let derived_bytes = Hkdf::<Sha256>::extract_then_expand(
            salt,
            seed,
            app_info,
        ...
        let secret: x25519_dalek::StaticSecret =
    ...
    /// Generates a random keypair `(PrivateKey, PublicKey)` by combining the
    /// output of `EntropyRng` with a user-provided seed. This concatenated
    /// seed is used as the seed to HKDF (RFC 5869).
    ///
    /// Similarly to `derive_keypair_from_seed` the user provides the following
    /// inputs: a) salt (optional) - denoted as 'salt' in RFC 5869
    /// b) seed - denoted as 'IKM' in RFC 5869
    /// c) application info (optional) - denoted as 'info' in RFC 5869
    ///
    /// Note that this method is not deterministic, but the (random + static
    /// seed) key generation makes it safer against low entropy pools and
    /// weak RNGs.
    pub fn generate_keypair_hybrid(
        ...
        X25519StaticPrivateKey::derive_keypair_from_seed(
            salt,
            &final_seed,
            app_info,
        )
...
impl TryFrom<&[u8]> for X25519StaticPrivateKey {
    ...
    fn try_from(
        bytes: &[u8],
    ) -> Result<X25519StaticPrivateKey, CryptoMaterialError> {
...
impl<'a> From<&'a X25519StaticPrivateKey> for X25519StaticPublicKey {
    fn from(ephemeral: &'a X25519StaticPrivateKey) -> X25519StaticPublicKey {
        X25519StaticPublicKey(X25519PublicKey(x25519_dalek::PublicKey::from(
            &ephemeral.0,
        )))
    }
}
...
impl TryFrom<&[u8]> for X25519StaticPublicKey {
    ...
    fn try_from(
        bytes: &[u8],
    ) -> Result<X25519StaticPublicKey, CryptoMaterialError> {
...
impl ValidKey for X25519StaticPublicKey {
    fn to_bytes(&self) -> Vec<u8> { self.deref().0.as_bytes().to_vec() }
}
