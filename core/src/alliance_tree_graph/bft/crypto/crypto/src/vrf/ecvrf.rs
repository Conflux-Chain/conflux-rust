...
//! This module implements an instantiation of a verifiable random function
//! known as [ECVRF-ED25519-SHA512-TAI](https://tools.ietf.org/html/draft-irtf-cfrg-vrf-04).
//!
//! # Examples
//!
//! ```
...
//!
//! Produce a proof for a message from a `VRFPrivateKey`, and verify the proof
//! and message using a `VRFPublicKey`:
//!
//! ```
//! # use libra_crypto::{traits::Uniform, vrf::ecvrf::*};
//! # use rand::{rngs::StdRng, SeedableRng};
...
use ed25519_dalek::{
    self, Digest, PublicKey as ed25519_PublicKey,
    SecretKey as ed25519_PrivateKey, Sha512,
};
...
///
/// This is similar in structure to ed25519_dalek::ExpandedSecretKey. It can be
/// produced from a VRFPrivateKey.
...
impl VRFPrivateKey {
    ...
    pub fn prove(&self, alpha: &[u8]) -> Proof {
        VRFExpandedPrivateKey::from(self)
            .prove(&VRFPublicKey((&self.0).into()), alpha)
    }
}
...
impl VRFExpandedPrivateKey {
    ...
    pub fn prove(&self, pk: &VRFPublicKey, alpha: &[u8]) -> Proof {
        ...
        let k_scalar = ed25519_Scalar::from_bytes_mod_order_wide(
            &nonce_generation_bytes(self.nonce, h_point),
        );
...
impl TryFrom<&[u8]> for VRFPrivateKey {
    ...
    fn try_from(
        bytes: &[u8],
    ) -> std::result::Result<VRFPrivateKey, CryptoMaterialError> {
        Ok(VRFPrivateKey(
...
impl TryFrom<&[u8]> for VRFPublicKey {
    ...
    fn try_from(
        bytes: &[u8],
    ) -> std::result::Result<VRFPublicKey, CryptoMaterialError> {
...
impl VRFPublicKey {
    /// Given a [`Proof`] and an input, returns whether or not the proof is
    /// valid for the input and public key
    ...
    pub(super) fn hash_to_curve(&self, alpha: &[u8]) -> EdwardsPoint {
        ...
        while wrapped_point.is_none() {
            ...
            wrapped_point =
                CompressedEdwardsY::from_slice(&result).decompress();
            counter += 1;
        }
...
impl Proof {
    ...
    pub fn new(
        gamma: EdwardsPoint, c: ed25519_Scalar, s: ed25519_Scalar,
    ) -> Proof {
        Proof { gamma, c, s }
    }
...
impl TryFrom<&[u8]> for Proof {
    ...
    fn try_from(
        bytes: &[u8],
    ) -> std::result::Result<Proof, CryptoMaterialError> {
...
impl Output {
    ...
    #[inline]
    pub fn to_bytes(&self) -> [u8; OUTPUT_LENGTH] { self.0 }
}
...
pub(super) fn nonce_generation_bytes(
