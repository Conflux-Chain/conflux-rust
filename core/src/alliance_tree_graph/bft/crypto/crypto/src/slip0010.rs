...
//!
//! SLIP-0010 describes how to derive a master private/public key for various
//! curves and how a [BIP-0032](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki) like derivation is used to hierarchically generate keys from an original seed.
//!
//! Note that SLIP-0010 only supports private parent key → private child key
//! hardened key generation for Ed25519. This key derivation protocol should be
//! preferred to HKDF if compatibility with hardware wallets and the BIP32
...
impl ExtendedPrivKey {
    ...
    pub fn new(
        key_path: String, private_key: SecretKey, chain_code: [u8; 32],
    ...
    /// Get public key.
    pub fn get_public(&self) -> PublicKey { (&self.private_key).into() }

    /// Get key path.
    pub fn get_key_path(&self) -> &str { &self.key_path }

    /// Get private key.
    pub fn get_private(&self) -> &SecretKey { &self.private_key }

    /// Get chain code.
    pub fn get_chain_code(&self) -> &[u8; 32] { &self.chain_code }
    ...
    fn child_key(&self, child_number: u32) -> Result<Self, Slip0010Error> {
        ...
        let mut hmac = Hmac::<Sha512>::new_varkey(&self.chain_code)
        ...
        let secret_key = SecretKey::from_bytes(&hmac_output[..32])
            .map_err(|_| Slip0010Error::SecretKeyError)?;
...
impl Slip0010 {
    ...
    pub fn derive_child_key(
        parent_key: ExtendedPrivKey, child_number: u32,
    ) -> Result<ExtendedPrivKey, Slip0010Error> {
        parent_key.child_key(child_number)
    }

    /// Match a valid path of the form "m/A/B.."; each sub-path after m is
...
///
/// This enum reflects there are various causes of SLIP-0010 failures,
/// including: a) invalid key-path.
