...
impl Signature for Ed25519Signature {
    ...
    fn verify(
        &self, message: &HashValue, public_key: &Ed25519PublicKey,
    ) -> Result<()> {
...
pub mod compat {
    ...
    where T: Into<Option<&'a mut StdRng>> + Sized {
        if let Some(rng_mut_ref) = opt_rng.into() {
            <(Ed25519PrivateKey, Ed25519PublicKey)>::generate_for_testing(
                rng_mut_ref,
            )
        } else {
    ...
    ) -> impl Strategy<Value = (Ed25519PrivateKey, Ed25519PublicKey)> {
    ...
    /// Generates a well-known keypair `(Ed25519PrivateKey, Ed25519PublicKey)`
    /// for special use in the genesis block. A genesis block is the first
    /// block of a blockchain and it is hardcoded as it's a special case in
    /// that it does not reference a previous block.
...
impl<'de> de::Visitor<'de> for Ed25519PrivateKeyVisitor {
    ...
    fn visit_bytes<E>(
        self, value: &[u8],
    ) -> std::result::Result<Ed25519PrivateKey, E>
    where E: de::Error {
...
impl<'de> de::Visitor<'de> for Ed25519PublicKeyVisitor {
    ...
    fn visit_bytes<E>(
        self, value: &[u8],
    ) -> std::result::Result<Ed25519PublicKey, E>
    where E: de::Error {
...
impl<'de> de::Visitor<'de> for Ed25519SignatureVisitor {
    ...
    fn visit_bytes<E>(
        self, value: &[u8],
    ) -> std::result::Result<Ed25519Signature, E>
    where E: de::Error {
