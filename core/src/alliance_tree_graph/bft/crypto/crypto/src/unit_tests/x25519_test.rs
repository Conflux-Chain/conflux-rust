...
fn test_default_key_pair() {
    ...
    let public_key2: X25519StaticPublicKey;
    {
        ...
        let private_key1 =
            X25519StaticPrivateKey::generate_for_testing(&mut rng);
...
fn test_hkdf_key_pair() {
    ...
    let info = None;
    let (_, public_key1) =
        X25519StaticPrivateKey::derive_keypair_from_seed(salt, &seed, info);
    let (_, public_key2) =
        X25519StaticPrivateKey::derive_keypair_from_seed(salt, &seed, info);
    assert_eq!(public_key1, public_key2);
...
fn test_generate_key_pair_with_seed() {
    ...
    let (_, public_key1) = X25519StaticPrivateKey::generate_keypair_hybrid(
        Some(salt),
        &seed,
        Some(info),
    );
    let (_, public_key2) = X25519StaticPrivateKey::generate_keypair_hybrid(
        Some(salt),
        &seed,
        Some(info),
    );
    ...
    // Ensure that the deterministic generate_keypair_from_seed returns a
    // completely different key.
    let (_, public_key3) = X25519StaticPrivateKey::derive_keypair_from_seed(
        Some(salt),
        &seed,
        Some(info),
    );
...
fn test_serialize_deserialize() {
    ...
    let deserialized: X25519StaticPrivateKey =
        lcs::from_bytes(&serialized).unwrap();
