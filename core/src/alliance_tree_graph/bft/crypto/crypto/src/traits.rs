    ...
    + ValidKey
{
    ...
    fn verify_signature(
        &self, message: &HashValue, signature: &Self::SignatureMaterial,
    ) -> Result<()> {
    ...
    {
        Self::SignatureMaterial::batch_verify_signatures(
            message,
            keys_and_signatures,
        )
    }
    ...
    + Hash
{
    ...
    fn verify(
        &self, message: &HashValue, public_key: &Self::VerifyingKeyMaterial,
    ) -> Result<()>;
...
pub trait ExchangeKey:
    PrivateKey<PublicKeyMaterial = <Self as ExchangeKey>::DHPublicKeyMaterial>
    + Uniform
{
    ...
    fn dh(
        self, public_key: &Self::DHPublicKeyMaterial,
    ) -> Self::DHSharedKeyMaterial;
}
