...
//! intended only for testing purposes. Production code should generate the key according to the spec [draft-irtf-cfrg-bls-signature-00](https://tools.ietf.org/id/draft-irtf-cfrg-bls-signature-00.html#keygen).
//!
//! This module is not currently used, but could be included in the future for
//! improved performance in consensus.
...
impl Uniform for BLS12381PrivateKey {
    ...
    /// It samples a random 255 bits number and checks whether this number is
    /// smaller than the modulu r, if so, it uses the number to compose a
    /// secret key, otherwise it retries. This method should be used for
    /// testing purposes only, production code should generate the key according to the spec [draft-irtf-cfrg-bls-signature-00](https://tools.ietf.org/id/draft-irtf-cfrg-bls-signature-00.html#keygen).
...
impl TryFrom<&[u8]> for BLS12381PrivateKey {
    ...
    fn try_from(
        ...
        // Note that the underlying implementation of SerdeSecret checks for
        // validation during deserialization. Also, we don't need to
        // check for validity of the derived PublicKey, as a
        // correct SerdeSecret (checked during deserialization that is in field)
        // will always produce a valid PublicKey.
        key_res.or(Err(CryptoMaterialError::DeserializationError))
    }
}
impl ValidKey for BLS12381PrivateKey {
    fn to_bytes(&self) -> Vec<u8> {
        // This is expected to succeed as we just return the bytes of an
        // existing key.
...
impl TryFrom<&[u8]> for BLS12381PublicKey {
    ...
    fn try_from(
        bytes: &[u8],
    ) -> std::result::Result<BLS12381PublicKey, CryptoMaterialError> {
        ...
        // Note that the underlying implementation of
        // threshold_crypto::PublicKey::from_bytes checks for validation
...
impl Signature for BLS12381Signature {
    ...
    fn verify(
        &self, message: &HashValue, public_key: &BLS12381PublicKey,
    ) -> Result<()> {
        self.verify_arbitrary_msg(message.as_ref(), public_key)
    }

    /// Checks that `signature` is valid for an arbitrary &[u8] `message` using
    /// `public_key`.
    fn verify_arbitrary_msg(
        &self, message: &[u8], public_key: &BLS12381PublicKey,
    ) -> Result<()> {
...
impl TryFrom<&[u8]> for BLS12381Signature {
    ...
    fn try_from(
        bytes: &[u8],
    ) -> std::result::Result<BLS12381Signature, CryptoMaterialError> {
        let l = bytes.len();
