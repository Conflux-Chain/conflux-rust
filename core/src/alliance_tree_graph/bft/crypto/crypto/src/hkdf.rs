...
//! Function for the Libra project based on [RFC 5869](https://tools.ietf.org/html/rfc5869).
//!
...
//! recommended approach for generating keys in Libra, especially when a True
...
where
    ...
    D::OutputSize: ArrayLength<u8>,
{
    /// Minimum acceptable output length for the underlying hash function is 32
    /// bytes.
    ...
    pub fn extract(
        ...
        let mut hmac = match salt {
            Some(s) => {
                Hmac::<D>::new_varkey(s).map_err(|_| HkdfError::MACKeyError)?
            }
    ...
    pub fn expand(
        ...
        let mut prev: Option<
            GenericArray<u8, <D as digest::FixedOutput>::OutputSize>,
        > = None;
        let mut hmac =
            Hmac::<D>::new_varkey(prk).map_err(|_| HkdfError::MACKeyError)?;

        for (blocknum, okm_block) in
            okm.chunks_mut(hmac_output_bytes).enumerate()
        {
