...
use crate::{
    ...
    account_config::{
        account_resource_path, association_address,
...
#[derive(
    Serialize, Deserialize, Debug, PartialEq, Hash, Eq, Clone, Ord, PartialOrd,
)]
pub struct Field(Identifier);

impl Field {
    pub fn new(name: Identifier) -> Field { Field(name) }

    pub fn name(&self) -> &IdentStr { &self.0 }
}
...
#[derive(
    Eq, Hash, Serialize, Deserialize, Debug, Clone, PartialEq, Ord, PartialOrd,
)]
pub enum Access {
    Field(Field),
    Index(u64),
...
impl Access {
    pub fn new(name: Identifier) -> Self { Access::Field(Field::new(name)) }
}
...
/// SEPARATOR is used as a delimiter between fields. It should not be a legal
/// part of any identifier in the language
const SEPARATOR: char = '/';

impl Accesses {
    pub fn empty() -> Self { Accesses(vec![]) }

    pub fn new(field: Field) -> Self { Accesses(vec![Access::Field(field)]) }
    ...
    /// Returns the first field in the sequence and reference to the remaining
    /// fields
    ...
    pub fn iter(&self) -> Iter<'_, Access> { self.0.iter() }
...
impl From<Vec<Access>> for Accesses {
    fn from(accesses: Vec<Access>) -> Accesses { Accesses(accesses) }
}
...
impl AccessPath {
    ...
    /// Given an address, returns the corresponding access path that stores the
    /// Account resource.
    ...
    /// That is an AccessPah that uniquely identifies a given event for a
    /// published resource.
    pub fn new_for_event(
        address: AccountAddress, root: &[u8], key: &[u8],
    ) -> Self {
    ...
    /// Create an AccessPath to the event for the sender account in a deposit
    /// operation. The sent counter in LibraAccount.T
    /// (LibraAccount.T.sent_events_count) is used to generate
    /// the AccessPath.
    /// That AccessPath can be used as a key into the event storage to retrieve
    /// all sent events for a given account.
    ...
    /// The received counter in LibraAccount.T
    /// (LibraAccount.T.received_events_count) is used to generate the
    /// AccessPath. That AccessPath can be used as a key into the event
    /// storage to retrieve all received events for a given account.
    pub fn new_for_received_event(address: AccountAddress) -> Self {
        Self::new(address, ACCOUNT_RECEIVED_EVENT_PATH.to_vec())
    }

    pub fn resource_access_vec(
        tag: &StructTag, accesses: &Accesses,
    ) -> Vec<u8> {
        let mut key = vec![];
        ...
        // We don't need accesses in production right now. Accesses are appended
        // here just for passing the old tests.
    ...
    pub fn resource_access_path(
        key: &ResourceKey, accesses: &Accesses,
    ) -> AccessPath {
