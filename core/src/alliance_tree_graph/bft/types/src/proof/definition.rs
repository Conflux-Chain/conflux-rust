...
use libra_crypto::{
    hash::{
        ...
        TransactionAccumulatorHasher, ACCUMULATOR_PLACEHOLDER_HASH,
...
/// Converts sibling nodes from Protobuf format to Rust format, using the fact
/// that empty byte arrays represent placeholder hashes.
fn from_proto_siblings(
    siblings: Vec<Vec<u8>>, placeholder: HashValue,
...
/// Converts sibling nodes from Rust format to Protobuf format. The placeholder
/// hashes are converted to empty byte arrays.
fn into_proto_siblings(
    siblings: Vec<HashValue>, placeholder: HashValue,
) -> Vec<Vec<u8>> {
    debug_assert!(
        placeholder == *ACCUMULATOR_PLACEHOLDER_HASH
            || placeholder == *SPARSE_MERKLE_PLACEHOLDER_HASH,
...
where H: CryptoHasher
{
    ...
    fn try_from(
        proto_proof: crate::proto::types::AccumulatorProof,
    ) -> Result<Self> {
        let siblings = from_proto_siblings(
...
impl<H> From<AccumulatorProof<H>> for crate::proto::types::AccumulatorProof {
    fn from(proof: AccumulatorProof<H>) -> Self {
        ...
        proto_proof.siblings =
            into_proto_siblings(proof.siblings, *ACCUMULATOR_PLACEHOLDER_HASH);
        proto_proof
    }
}
...
pub struct SparseMerkleProof {
    ...
    ///         - If the first `HashValue` equals requested key, this is an
    ///           inclusion proof and the second `HashValue` equals the hash of
    ///           the corresponding account blob.
    ///         - Otherwise this is a non-inclusion proof. The first
    ///           `HashValue` is the only key that exists in the subtree and
    ///           the second `HashValue` equals the hash of the corresponding
    ///           account blob.
...
impl SparseMerkleProof {
    ...
    pub fn leaf(&self) -> Option<(HashValue, HashValue)> { self.leaf }

    /// Returns the list of siblings in this proof.
    pub fn siblings(&self) -> &[HashValue] { &self.siblings }

    /// If `element_blob` is present, verifies an element whose key is
    /// `element_key` and value is `element_blob` exists in the Sparse
...
impl TryFrom<crate::proto::types::SparseMerkleProof> for SparseMerkleProof {
    ...
    fn try_from(
        proto_proof: crate::proto::types::SparseMerkleProof,
    ) -> Result<Self> {
        ...
        let leaf = if proto_leaf.is_empty() {
            ...
            let value_hash =
                HashValue::from_slice(&proto_leaf[HashValue::LENGTH..])?;
            Some((key, value_hash))
        } else {
            bail!(
...
impl From<SparseMerkleProof> for crate::proto::types::SparseMerkleProof {
    fn from(proof: SparseMerkleProof) -> Self {
        let mut proto_proof = Self::default();
        // If a leaf is present, we write the key and value hash as a single
        // byte array of 64 bytes. Otherwise we write an empty byte
        // array.
        ...
        proto_proof.siblings = into_proto_siblings(
            proof.siblings,
            *SPARSE_MERKLE_PLACEHOLDER_HASH,
        );
        proto_proof
...
impl TryFrom<crate::proto::types::AccumulatorConsistencyProof>
    for AccumulatorConsistencyProof
{
    type Error = Error;

    fn try_from(
        proto_proof: crate::proto::types::AccumulatorConsistencyProof,
    ) -> Result<Self> {
...
impl From<AccumulatorConsistencyProof>
    for crate::proto::types::AccumulatorConsistencyProof
{
...
impl<H> TryFrom<crate::proto::types::AccumulatorRangeProof>
    for AccumulatorRangeProof<H>
where H: CryptoHasher
{
    type Error = Error;

    fn try_from(
        proto_proof: crate::proto::types::AccumulatorRangeProof,
    ) -> Result<Self> {
        let left_siblings = from_proto_siblings(
            proto_proof.left_siblings,
            *ACCUMULATOR_PLACEHOLDER_HASH,
        )?;
        let right_siblings = from_proto_siblings(
...
impl<H> From<AccumulatorRangeProof<H>>
    for crate::proto::types::AccumulatorRangeProof
{
...
pub struct SparseMerkleRangeProof {
    /// The vector of siblings. The ones near the bottom are at the beginning
    /// of the vector. In the above example, it's `[X, h]`.
    siblings: Vec<HashValue>,
}

impl SparseMerkleRangeProof {
    /// Constructs a new `SparseMerkleRangeProof`.
    pub fn new(siblings: Vec<HashValue>) -> Self { Self { siblings } }

    /// Returns the siblings.
    pub fn siblings(&self) -> &[HashValue] { &self.siblings }
}

/// The complete proof used to authenticate a `Transaction` object.  This
/// structure consists of an `AccumulatorProof` from `LedgerInfo` to
/// `TransactionInfo` the verifier needs to verify the correctness of the
...
impl TransactionProof {
    ...
    /// Verifies that a `Transaction` with hash value of `transaction_hash` is
    /// the version `transaction_version` transaction in the ledger using
    /// the provided proof.  If `event_root_hash` is provided, it's also
    /// verified against the proof.
    pub fn verify(
        &self, ledger_info: &LedgerInfo, transaction_hash: HashValue,
        event_root_hash: Option<HashValue>, transaction_version: Version,
    ) -> Result<()>
    {
        ensure!(
...
impl TryFrom<crate::proto::types::TransactionProof> for TransactionProof {
    ...
    fn try_from(
        proto_proof: crate::proto::types::TransactionProof,
    ) -> Result<Self> {
        let ledger_info_to_transaction_info_proof = proto_proof
            .ledger_info_to_transaction_info_proof
            .ok_or_else(|| {
                format_err!("Missing ledger_info_to_transaction_info_proof")
            })?
            .try_into()?;
...
impl TryFrom<crate::proto::types::AccountStateProof> for AccountStateProof {
    ...
    fn try_from(
        proto_proof: crate::proto::types::AccountStateProof,
    ) -> Result<Self> {
        let ledger_info_to_transaction_info_proof = proto_proof
            .ledger_info_to_transaction_info_proof
            .ok_or_else(|| {
        ...
        let transaction_info_to_account_proof = proto_proof
            .transaction_info_to_account_proof
            .ok_or_else(|| {
                format_err!("Missing transaction_info_to_account_proof")
            })?
            .try_into()?;
...
impl From<EventProof> for crate::proto::types::EventProof {
    fn from(proof: EventProof) -> Self {
        Self {
            ...
            transaction_info: Some(proof.transaction_info.into()),
            transaction_info_to_event_proof: Some(
                proof.transaction_info_to_event_proof.into(),
            ),
        }
    }
}
...
impl TryFrom<crate::proto::types::TransactionListProof>
    for TransactionListProof
{
    type Error = Error;

    fn try_from(
        proto_proof: crate::proto::types::TransactionListProof,
    ) -> Result<Self> {
        let ledger_info_to_transaction_infos_proof = proto_proof
            .ledger_info_to_transaction_infos_proof
            .ok_or_else(|| {
                format_err!("Missing ledger_info_to_transaction_infos_proof")
            })?
            .try_into()?;
