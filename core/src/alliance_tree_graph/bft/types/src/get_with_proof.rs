...
use crate::{
    ...
    proto::types::{
        ...
        GetEventsByEventAccessPathRequest, GetEventsByEventAccessPathResponse,
...
impl UpdateToLatestLedgerRequest {
    pub fn new(
        client_known_version: u64, requested_items: Vec<RequestItem>,
    ) -> Self {
...
impl TryFrom<crate::proto::types::UpdateToLatestLedgerRequest>
    for UpdateToLatestLedgerRequest
{
    type Error = Error;

    fn try_from(
        proto: crate::proto::types::UpdateToLatestLedgerRequest,
    ) -> Result<Self> {
        Ok(Self {
...
impl From<UpdateToLatestLedgerRequest>
    for crate::proto::types::UpdateToLatestLedgerRequest
{
...
impl TryFrom<crate::proto::types::UpdateToLatestLedgerResponse>
    for UpdateToLatestLedgerResponse
{
    type Error = Error;

    fn try_from(
        proto: crate::proto::types::UpdateToLatestLedgerResponse,
    ) -> Result<Self> {
...
impl From<UpdateToLatestLedgerResponse>
    for crate::proto::types::UpdateToLatestLedgerResponse
{
    fn from(response: UpdateToLatestLedgerResponse) -> Self {
        ...
        let validator_change_proof =
            Some(response.validator_change_proof.into());
        let ledger_consistency_proof =
            Some(response.ledger_consistency_proof.into());
...
impl UpdateToLatestLedgerResponse {
    ...
    /// Verifies that the response has items corresponding to request items and
    /// each of them are supported by proof it carries and is what the
    /// request item asks for.
    ///
    /// After calling this one can trust the info in the response items without
    /// further verification.
    pub fn verify(
        &self, current_epoch_info: &EpochInfo,
...
/// Verifies content of an [`UpdateToLatestLedgerResponse`] against the proofs
/// it carries and the content of the corresponding
/// [`UpdateToLatestLedgerRequest`] Return EpochInfo if there're validator
/// change events.
pub fn verify_update_to_latest_ledger_response(
    current_epoch_info: &EpochInfo, req_client_known_version: u64,
    req_request_items: &[RequestItem], response_items: &[ResponseItem],
    ledger_info_with_sigs: &LedgerInfoWithSignatures,
    validator_change_proof: &ValidatorChangeProof,
) -> Result<Option<EpochInfo>>
{
        ...
        let new_epoch_info = EpochInfo {
            ...
            verifier: Arc::new(
                epoch_change_li
                    ...
                    .next_validator_set()
                    .ok_or_else(|| {
                        format_err!("No ValidatorSet in EpochProof")
                    })?
                    .into(),
            ),
        };
...
fn verify_response_item(
    ledger_info: &LedgerInfo, req: &RequestItem, res: &ResponseItem,
) -> Result<()> {
    match (req, res) {
        // GetAccountState
        (
...
fn verify_get_txn_by_seq_num_resp(
    ledger_info: &LedgerInfo, req_account: AccountAddress,
    req_sequence_number: u64, req_fetch_events: bool,
...
fn verify_get_events_by_access_path_resp(
    ledger_info: &LedgerInfo, req_access_path: &AccessPath,
    req_start_seq_num: u64, req_ascending: bool, req_limit: u64,
    events_with_proof: &[EventWithProof],
    proof_of_latest_event: &AccountStateWithProof,
) -> Result<()>
{
    let account_resource =
        get_account_resource_or_default(&proof_of_latest_event.blob)?;
    let (seq_num_upper_bound, expected_event_key) = {
        ...
        let event_handle = account_resource
            .get_event_handle_by_query_path(&req_access_path.path)?;
        (event_handle.count(), event_handle.key())
    };

    let cursor = if !req_ascending
        && req_start_seq_num == u64::max_value()
...
fn verify_get_txns_resp(
    ledger_info: &LedgerInfo, req_start_version: Version, req_limit: u64,
    req_fetch_events: bool, txn_list_with_proof: &TransactionListWithProof,
) -> Result<()>
{
...
impl ResponseItem {
    pub fn into_get_account_state_response(
        self,
    ...
    ) -> Result<(Option<TransactionWithProof>, Option<AccountStateWithProof>)>
    {
        match self {
            ...
            _ => bail!(
                "Not ResponseItem::GetAccountTransactionBySequenceNumber."
            ),
        }
    }
    ...
    pub fn into_get_transactions_response(
        self,
        ...
        let response = match item {
            GetAccountStateResponse(response) => {
                let account_state_with_proof = response
                    .account_state_with_proof
                    .ok_or_else(|| {
                        format_err!("Missing account_state_with_proof")
                    })?
                    .try_into()?;
                ResponseItem::GetAccountState {
                    account_state_with_proof,
            ...
            GetEventsByEventAccessPathResponse(response) => {
                ...
                let proof_of_latest_event = response
                    .proof_of_latest_event
                    .ok_or_else(|| {
                        format_err!("Missing proof_of_latest_event")
                    })?
                    .try_into()?;
...
impl From<ResponseItem> for crate::proto::types::ResponseItem {
    fn from(response: ResponseItem) -> Self {
        ...
        let res = match response {
            ResponseItem::GetAccountState {
                account_state_with_proof,
            } => ResponseItems::GetAccountStateResponse(
                GetAccountStateResponse {
                    account_state_with_proof: Some(
                        account_state_with_proof.into(),
                    ),
                },
            ),
            ResponseItem::GetAccountTransactionBySequenceNumber {
                ...
                GetAccountTransactionBySequenceNumberResponse {
                    transaction_with_proof: transaction_with_proof
                        .map(Into::into),
                    proof_of_current_sequence_number:
                        proof_of_current_sequence_number.map(Into::into),
                },
            ),
            ResponseItem::GetEventsByEventAccessPath {
                events_with_proof,
                ...
                GetEventsByEventAccessPathResponse {
                    events_with_proof: events_with_proof
                        .into_iter()
                        .map(Into::into)
                        .collect(),
            ...
            ResponseItem::GetTransactions {
                txn_list_with_proof,
            } => ResponseItems::GetTransactionsResponse(
