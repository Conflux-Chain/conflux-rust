...
impl<PrivateKey: SigningKey> ValidatorSigner<PrivateKey> {
    ...
    {
        let public_key: PrivateKey::VerifyingKeyMaterial =
    ...
    ) -> Result<PrivateKey::SignatureMaterial, Error> {
...
pub mod proptests {
    ...
    pub fn arb_signing_key<
        PrivateKey: SigningKey + Uniform + Genesis + 'static,
    >() -> impl Strategy<Value = PrivateKey> {
        prop_oneof![
            ...
            LazyJust::new(|| PrivateKey::generate_for_testing(
                &mut StdRng::from_seed(TEST_SEED)
            )),
    ...
    pub fn signer_strategy<PrivateKey: SigningKey + Uniform + Genesis>(
        ...
        signing_key_strategy
            .prop_map(|signing_key| ValidatorSigner::new(None, signing_key))
    }
    ...
    fn select_keypair<
        PrivateKey: SigningKey + Uniform + Genesis + Clone + 'static,
    >(
        keys: Vec<PrivateKey>,
    ) -> impl Strategy<Value = PrivateKey> {
        sample::select(keys)
