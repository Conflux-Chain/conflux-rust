...
//! - help make sure that any consensus data that uses cryptographic material is
//!   defined generically in terms of libra_crypto::traits, rather than
//!   accessing the implementation details of a particular scheme (a.k.a.
//!   encapsulation),
//! - contribute to a single-location place for instantiations of these
//!   polymorphic types (`crate::chained_bft::consensus_types`)
//! - help the consensus crate avoid generic parameters within the code, by
//!   avoiding propagating generic signature type parameters throughout the
//!   crate's code (as prototyped in https://github.com/libra/libra/pull/522),
...
use crate::{
    ...
    validator_verifier::{
        ValidatorInfo as RawValidatorInfo,
        ValidatorVerifier as RawValidatorVerifier, VerifyError,
    },
};
...
impl<Sig: RawSignature> SignatureWrapper<Sig> {
    pub fn verify(
        ...
        author: AccountAddress, message: HashValue,
    ...
    pub fn try_from(
        bytes: &[u8],
    ) -> Result<Self, libra_crypto::traits::CryptoMaterialError> {
...
impl<Sig: RawSignature> From<Sig> for SignatureWrapper<Sig> {
    fn from(s: Sig) -> Self { SignatureWrapper(s) }
}
...
#[derive(Clone)]
/// EpochInfo represents a trusted validator set to validate messages from the
/// specific epoch, it could be updated with ValidatorChangeProof.
