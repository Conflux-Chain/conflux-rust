...
pub struct VMStatus {
    ...
    /// The optional sub status. Used e.g. for things such as the abort code,
    /// or the arithmetic error type for an ARITHMETIC_ERROR major status.
    pub sub_status: Option<u64>,

    /// The optional message. Useful for verification errors, and for returning
    /// information in validation.
...
impl fmt::Display for VMStatus {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let mut status =
            format!("status {:#?} of type {}", self.major_status, status_type);
...
impl VMStatus {
    ...
    /// Mutates the VMStatus sub status field to be the new `sub_status` passed
    /// in.
    ...
    pub fn append_message_with_separator(
        mut self, separator: char, message: String,
    ) -> Self {
    ...
    /// Append two VMStatuses together. The major status is kept from the
    /// caller.
...
impl TryFrom<crate::proto::types::VmStatus> for VMStatus {
    ...
    fn try_from(proto: crate::proto::types::VmStatus) -> Result<Self> {
        let mut status = VMStatus::new(
            StatusCode::try_from(proto.major_status)
...
#[repr(u64)]
/// We don't derive Arbitrary on this enum because it is too large and breaks
/// proptest. It is written for a subset of these in proptest_types. We test
/// conversion between this and protobuf with a hand-written test.
pub enum StatusCode {
    ...
    // The self address of a module the transaction is publishing is not the
    // sender address
    MODULE_ADDRESS_DOES_NOT_MATCH_SENDER = 1067,
    // The module does not have any module handles. Each module or script must
    // have at least one module handle.
    ...
    // The sender is trying to publish a module named `M`, but the sender's
    // account already contains a module with this name.
