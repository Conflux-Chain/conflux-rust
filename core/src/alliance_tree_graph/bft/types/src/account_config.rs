...
pub fn coin_module_name() -> &'static IdentStr { &*COIN_MODULE_NAME }

pub fn coin_struct_name() -> &'static IdentStr { &*COIN_STRUCT_NAME }

pub fn account_module_name() -> &'static IdentStr { &*ACCOUNT_MODULE_NAME }

pub fn account_struct_name() -> &'static IdentStr { &*ACCOUNT_STRUCT_NAME }

pub fn core_code_address() -> AccountAddress { AccountAddress::default() }
...
impl AccountResource {
    ...
    pub fn new(
        balance: u64, sequence_number: u64, authentication_key: ByteArray,
        delegated_key_rotation_capability: bool,
        delegated_withdrawal_capability: bool, sent_events: EventHandle,
        received_events: EventHandle, event_generator: u64,
    ) -> Self
    {
        AccountResource {
            balance,
    ...
    /// Given an account map (typically from storage) retrieves the Account
    /// resource associated.
    ...
    pub fn authentication_key(&self) -> &ByteArray { &self.authentication_key }
...
/// Return the path to the Account resource. It can be used to create an
/// AccessPath for an Account resource.
...
/// Both SentPaymentEvent and ReceivedPaymentEvent are representable with this
/// struct. They have an AccountAddress for the sender or receiver, the amount
/// transferred, and metadata.
#[derive(Debug, Default, Serialize, Deserialize)]
...
impl AccountEvent {
    // TODO: should only be used for libra client testing and be removed
    // eventually
    pub fn new(
        amount: u64, account: AccountAddress, metadata: Vec<u8>,
    ) -> Self {
        Self {
            amount,
            account,
