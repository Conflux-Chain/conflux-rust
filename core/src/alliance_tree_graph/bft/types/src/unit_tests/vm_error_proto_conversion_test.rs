...
fn status_roundtrip() {
    ...
    for status_number in 0..max_error_number {
        let status = StatusCode::try_from(status_number)
            .unwrap_or_else(|_| StatusCode::UNKNOWN_STATUS);
        ...
        assert_protobuf_encode_decode::<crate::proto::types::VmStatus, VMStatus>(
            &status,
        );
    }
}
