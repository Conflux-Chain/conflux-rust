...
pub struct ValidatorVerifier<PublicKey> {
    ...
    address_to_validator_info:
        BTreeMap<AccountAddress, ValidatorInfo<PublicKey>>,
...
impl<PublicKey: VerifyingKey> ValidatorVerifier<PublicKey> {
    ...
    ) -> Result<Self>
    {
        let total_voting_power =
            address_to_validator_info.values().fold(0, |sum, x| {
                // The voting power of any node is assumed to be small relative
                // to u64::max_value()
        ...
        T: Into<PublicKey::SignatureMaterial> + Clone,
    {
        self.check_num_of_signatures(aggregated_signature)?;
        self.check_voting_power(aggregated_signature.keys())?;
        let keys_and_signatures: Vec<(
            PublicKey,
            PublicKey::SignatureMaterial,
        )> = aggregated_signature
            .iter()
            .flat_map(|(address, signature)| {
                let sig: PublicKey::SignatureMaterial =
        ...
        // batching fails.
        if PublicKey::batch_verify_signatures(&hash, keys_and_signatures)
            .is_err()
        {
    ...
    fn check_num_of_signatures<T>(
        &self, aggregated_signature: &BTreeMap<AccountAddress, T>,
    ) -> std::result::Result<(), VerifyError>
    where T: Into<PublicKey::SignatureMaterial> + Clone {
...
impl<PublicKey: VerifyingKey> From<&ValidatorSet<PublicKey>>
    for ValidatorVerifier<PublicKey>
{
    fn from(validator_set: &ValidatorSet<PublicKey>) -> Self {
        ValidatorVerifier::new(validator_set.iter().fold(
            BTreeMap::new(),
            |mut map, key| {
                map.insert(
...
impl<PublicKey: VerifyingKey> From<&ValidatorVerifier<PublicKey>>
...
mod tests {
    use crate::{
        ...
        validator_verifier::{
            ValidatorInfo, ValidatorVerifier,
    ...
    fn test_equal_vote_quorum_validators() {
        ...
        assert_eq!(
            validator_verifier.batch_verify_aggregated_signature(
                random_hash,
