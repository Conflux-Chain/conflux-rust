        ...
        self.ledger_info_with_sigs.iter().try_fold(
            ...
            |(epoch, validator_verifier), ledger_info| {
                ...
                ledger_info
                    ...
                    .ok_or_else(|| {
                        format_err!("LedgerInfo doesn't carry ValidatorSet")
                    })
            },
        )?;
...
impl TryFrom<crate::proto::types::ValidatorChangeProof>
    for ValidatorChangeProof
{
    type Error = Error;

    fn try_from(
        proto: crate::proto::types::ValidatorChangeProof,
    ) -> Result<Self> {
...
mod tests {
    ...
    fn verify_validator_set_change_proof() {
        use crate::{
            crypto_proxies::random_validator_verifier, ledger_info::LedgerInfo,
        };
        ...
        // the current validator set and carries next validator set.
        ...
        for epoch in &all_epoch {
            ...
            let signatures = current_signers
                .iter()
                .map(|s| {
                    (s.author(), s.sign_message(ledger_info.hash()).unwrap())
                })
                .collect();
            valid_ledger_info
        ...
        let proof_1 = ValidatorChangeProof::new(
