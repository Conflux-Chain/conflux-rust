...
impl LedgerInfo {
    ...
    /// To bootstrap the system until we execute and commit the genesis txn
    /// before start.
...
impl TryFrom<crate::proto::types::LedgerInfo> for LedgerInfo {
    ...
    fn try_from(proto: crate::proto::types::LedgerInfo) -> Result<Self> {
        ...
        let consensus_data_hash =
            HashValue::from_slice(&proto.consensus_data_hash)?;
        let consensus_block_id =
            HashValue::from_slice(&proto.consensus_block_id)?;
        let epoch = proto.epoch;
        let round = proto.round;
        let timestamp_usecs = proto.timestamp_usecs;

        let next_validator_set =
            if let Some(validator_set_proto) = proto.next_validator_set {
...
impl From<LedgerInfo> for crate::proto::types::LedgerInfo {
    fn from(ledger_info: LedgerInfo) -> Self {
        Self {
            ...
            transaction_accumulator_hash: ledger_info
                .transaction_accumulator_hash()
                .to_vec(),
...
impl<Sig: Signature> LedgerInfoWithSignatures<Sig> {
    pub fn new(
        ledger_info: LedgerInfo, signatures: BTreeMap<AccountAddress, Sig>,
    ) -> Self {
    ...
    pub fn verify(
        &self, validator: &ValidatorVerifier<Sig::VerifyingKeyMaterial>,
    ) -> ::std::result::Result<(), VerifyError> {
        let ledger_hash = self.ledger_info().hash();
        validator
            .batch_verify_aggregated_signature(ledger_hash, self.signatures())
    }
}
    ...
    for LedgerInfoWithSignatures<Sig>
{
    ...
    fn try_from(
        proto: crate::proto::types::LedgerInfoWithSignatures,
    ) -> Result<Self> {
        ...
        let signatures = signatures_proto
            ...
            .map(|proto| {
                let validator_id =
                    AccountAddress::try_from(proto.validator_id)?;
    ...
    fn from(ledger_info_with_sigs: LedgerInfoWithSignatures<Sig>) -> Self {
        ...
        let signatures = ledger_info_with_sigs
            ...
            .map(|(validator_id, signature)| {
                crate::proto::types::ValidatorSignature {
...
mod tests {
    ...
    fn test_signatures_hash() {
        ...
        let ledger_info_with_signatures = LedgerInfoWithSignatures::new(
            ledger_info.clone(),
        ...
        let ledger_info_with_signatures_bytes =
            lcs::to_bytes(&ledger_info_with_signatures)
