...
pub fn parse_as_address(s: &str) -> Result<TransactionArgument> {
    ...
    if addr.len() > 32 {
        return Err(ErrorKind::ParseError(
            "address must be 32 bytes or less".to_string(),
        )
        .into());
    }
    if addr.len() < 32 {
...
pub fn parse_as_byte_array(s: &str) -> Result<TransactionArgument> {
    if s.starts_with("b\"") && s.ends_with('"') && s.len() >= 3 {
        ...
        if s.is_empty() {
            return Err(ErrorKind::ParseError(
                "byte array cannot be empty".to_string(),
            )
            .into());
        }
        let s = if s.len() % 2 == 0 {
        ...
        Err(
            ErrorKind::ParseError(format!("\"{}\" is not a byte array", s))
                .into(),
        )
    }
}
...
pub fn parse_as_transaction_argument(s: &str) -> Result<TransactionArgument> {
    ...
    Err(ErrorKind::ParseError(format!(
        "cannot parse \"{}\" as transaction argument",
        s
    ))
    .into())
}
