...
use crate::{
    ...
    proof::{
        accumulator::InMemoryAccumulator, TransactionListProof,
        TransactionProof,
    },
...
#[derive(
    Clone, Debug, Hash, Eq, PartialEq, Serialize, Deserialize, CryptoHasher,
)]
pub struct RawTransaction {
    ...
    // never be included.
    // A transaction that doesn't expire is represented by a very large value
    // like u64::max_value().
...
impl RawTransaction {
    ...
    pub fn new_write_set(
        ...
        RawTransaction {
            sender,
            sequence_number,
            payload: TransactionPayload::WriteSet(ChangeSet::new(
                write_set,
                vec![],
            )),
    ...
    pub fn format_for_client(
        ...
        let (code, args) = match &self.payload {
            ...
            TransactionPayload::WriteSet(_) => {
                ("genesis".to_string(), &empty_vec[..])
            }
            TransactionPayload::Script(script) => {
                (get_transaction_name(script.code()), script.args())
            }
            TransactionPayload::Module(_) => {
                ("module publishing".to_string(), &empty_vec[..])
            }
        };
...
impl SignedTransaction {
    ...
    pub fn expiration_time(&self) -> Duration { self.raw_txn.expiration_time }
...
impl From<SignedTransaction> for crate::proto::types::SignedTransaction {
    fn from(txn: SignedTransaction) -> Self {
        let txn_bytes =
            lcs::to_bytes(&txn).expect("Unable to serialize SignedTransaction");
        Self { txn_bytes }
    }
}

impl From<SignatureCheckedTransaction>
    for crate::proto::types::SignedTransaction
{
...
impl TryFrom<crate::proto::types::TransactionWithProof>
    for TransactionWithProof
{
    type Error = Error;

    fn try_from(
        mut proto: crate::proto::types::TransactionWithProof,
    ) -> Result<Self> {
...
impl From<TransactionWithProof> for crate::proto::types::TransactionWithProof {
    fn from(mut txn: TransactionWithProof) -> Self {
        Self {
            ...
            events: txn.events.take().map(|list| {
                crate::proto::types::EventsList {
...
impl TransactionStatus {
    pub fn vm_status(&self) -> &VMStatus {
        match self {
            ...
            | TransactionStatus::Keep(vm_status) => vm_status,
        }
    }
}
...
impl From<VMStatus> for TransactionStatus {
    fn from(vm_status: VMStatus) -> Self {
        let should_discard = match vm_status.status_type() {
            ...
            // Any error that is a validation status (i.e. an error arising from
            // the prologue) causes the transaction to not be
            // included.
            StatusType::Validation => true,
            // If the VM encountered an invalid internal state, we should
            // discard the transaction.
            StatusType::InvariantViolation => true,
            // A transaction that publishes code that cannot be verified is
            // currently not charged. Therefore the transaction can
            // be excluded.
            //
            // The original plan was to charge for verification, but the code
            // didn't implement it properly. The decision of whether
            // to charge or not will be made based on data (if
            // verification checks are too expensive then yes, otherwise no).
            StatusType::Verification => true,
            // Even if we are unable to decode the transaction, there should be
            // a charge made to that user's account for the gas fees
            // related to decoding, running the prologue etc.
            StatusType::Deserialization => false,
            // Any error encountered during the execution of the transaction
            // will charge gas.
...
impl TryFrom<crate::proto::types::TransactionInfo> for TransactionInfo {
    ...
    fn try_from(
        proto_txn_info: crate::proto::types::TransactionInfo,
    ) -> Result<Self> {
        let transaction_hash =
            HashValue::from_slice(&proto_txn_info.transaction_hash)?;
        let state_root_hash =
            HashValue::from_slice(&proto_txn_info.state_root_hash)?;
        let event_root_hash =
            HashValue::from_slice(&proto_txn_info.event_root_hash)?;
        let gas_used = proto_txn_info.gas_used;
        let major_status = StatusCode::try_from(proto_txn_info.major_status)
...
pub struct TransactionInfo {
    ...
    /// The major status. This will provide the general error class. Note that
    /// this is not particularly high fidelity in the presence of sub
    /// statuses but, the major status does determine whether or not the
    /// transaction is applied to the global state or not.
...
impl TransactionInfo {
    ...
    pub fn new(
        ...
        event_root_hash: HashValue, gas_used: u64, major_status: StatusCode,
    ) -> TransactionInfo
    {
        TransactionInfo {
            transaction_hash,
    ...
    pub fn major_status(&self) -> StatusCode { self.major_status }
}
...
impl TransactionToCommit {
    pub fn new(
        transaction: Transaction,
        account_states: HashMap<AccountAddress, AccountStateBlob>,
        events: Vec<ContractEvent>, gas_used: u64, major_status: StatusCode,
    ) -> Self
    {
        TransactionToCommit {
            transaction,
...
impl TryFrom<crate::proto::types::TransactionToCommit> for TransactionToCommit {
    ...
    fn try_from(
        proto: crate::proto::types::TransactionToCommit,
    ) -> Result<Self> {
        let transaction = proto
            .transaction
        ...
        let major_status = StatusCode::try_from(proto.major_status)
            .unwrap_or(StatusCode::UNKNOWN_STATUS);
...
impl TryFrom<crate::proto::types::TransactionListWithProof>
    for TransactionListWithProof
{
    type Error = Error;

    fn try_from(
        mut proto: crate::proto::types::TransactionListWithProof,
    ) -> Result<Self> {
...
impl From<TransactionListWithProof>
    for crate::proto::types::TransactionListWithProof
{
    fn from(txn: TransactionListWithProof) -> Self {
        let transactions =
            txn.transactions.into_iter().map(Into::into).collect();

        let events_for_versions = txn.events.map(|all_events| {
            crate::proto::types::EventsForVersions {
...
/// `Transaction` will be the transaction type used internally in the libra node
...
pub enum Transaction {
    ...
    /// Transaction that applies a WriteSet to the current storage. This should
    /// be used for ONLY for genesis right now.
    WriteSet(ChangeSet),
...
impl CryptoHash for Transaction {
    ...
    fn hash(&self) -> HashValue {
        let mut state = Self::Hasher::default();
        state.write(
            &lcs::to_bytes(self).expect("Failed to serialize Transaction."),
        );
        state.finish()
    }
}
...
impl From<Transaction> for crate::proto::types::Transaction {
    fn from(txn: Transaction) -> Self {
        let bytes =
            lcs::to_bytes(&txn).expect("Serialization should not fail.");
