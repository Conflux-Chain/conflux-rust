...
impl Arbitrary for ByteArray {
    ...
    #[inline]
    fn arbitrary_with(_args: ()) -> Self::Strategy { arb_byte_array().boxed() }
}
...
fn new_raw_transaction(
    ...
    max_gas_amount: u64, gas_unit_price: u64, expiration_time_secs: u64,
) -> RawTransaction
{
    match payload {
...
impl SignatureCheckedTransaction {
    ...
    pub fn script_strategy(
        keypair_strategy: impl Strategy<
            Value = (Ed25519PrivateKey, Ed25519PublicKey),
        >,
    ) -> impl Strategy<Value = Self>
    {
        Self::strategy_impl(
...
impl Arbitrary for SignatureCheckedTransaction {
    ...
    fn arbitrary_with(_args: ()) -> Self::Strategy {
        Self::strategy_impl(keypair_strategy(), any::<TransactionPayload>())
            .boxed()
    }
}
...
impl TransactionPayload {
    ...
    pub fn write_set_strategy() -> impl Strategy<Value = Self> {
        any::<WriteSet>().prop_map(|ws| {
            TransactionPayload::WriteSet(ChangeSet::new(ws, vec![]))
        })
    }
...
impl Arbitrary for BlockMetadata {
    ...
    fn arbitrary_with(
        ...
        let signature_strategy =
            (any::<HashValue>(), Just(num_validators_range)).prop_flat_map(
...
impl BlockInfoGen {
    pub fn materialize(
        ...
        let (epoch, next_validator_set) =
            if current_epoch == 0 || self.new_epoch {
                (
...
impl Arbitrary for BlockInfoGen {
    ...
    fn arbitrary_with(_args: Self::Parameters) -> Self::Strategy {
        ...
        (
            ...
            .prop_map(|(id, executed_state_id, timestamp_usecs, new_epoch)| {
                Self {
                    id,
                    executed_state_id,
                    timestamp_usecs,
...
pub struct LedgerInfoWithSignaturesGen {
    ...
    // TODO: To make it more real, we can let the universe carry the current
    // validator set.
    signers: Vec<Index>,
}
...
impl Arbitrary for LedgerInfoWithSignaturesGen {
    ...
    fn arbitrary_with(_args: Self::Parameters) -> Self::Strategy {
        (any::<LedgerInfoGen>(), vec(any::<Index>(), 3))
            .prop_map(|(ledger_info_gen, signers)| {
                LedgerInfoWithSignaturesGen {
                    ledger_info_gen,
                    signers,
                }
            })
    ...
    pub fn materialize(
        ...
        let signatures = self
            ...
            .map(|signer_index| {
                ...
                let signature =
                    account.private_key.sign_message(&ledger_info_hash);
