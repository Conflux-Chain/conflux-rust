...
/// Trivial mock: generates MockTransactions on the fly. Each next transaction
/// is the next value.
#[derive(Clone)]
...
impl TxnManager for MockTransactionManager {
    ...
    async fn pull_txns(
        &mut self, max_size: u64, _exclude_txns: Vec<&Self::Payload>,
    ) -> Result<Self::Payload> {
    ...
    async fn commit_txns(
        &mut self, txns: &Self::Payload, _compute_result: &StateComputeResult,
