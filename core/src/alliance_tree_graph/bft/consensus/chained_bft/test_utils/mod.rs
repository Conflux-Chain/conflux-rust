...
impl TreeInserter {
    ...
    pub fn new_with_store(
        signer: ValidatorSigner, block_store: Arc<BlockStore<TestPayload>>,
    ) -> Self {
        Self {
            signer,
            payload_val: 0,
    ...
    pub fn insert_block(
        &mut self, parent: &ExecutedBlock<TestPayload>, round: Round,
    ...
    pub fn insert_block_with_qc(
        &mut self, parent_qc: QuorumCert, parent: &ExecutedBlock<TestPayload>,
        round: Round,
    ) -> Arc<ExecutedBlock<TestPayload>>
    {
        self.payload_val += 1;
    ...
    pub fn create_qc_for_block(
        &self, block: &ExecutedBlock<TestPayload>,
        committed_block: Option<BlockInfo>,
    ) -> QuorumCert
    {
...
pub fn with_smr_id(id: String) -> impl Fn() {
    move || {
        set_simple_logger_prefix(format!(
            "{}[{}]{}",
            Fg(LightBlack),
            id,
            Fg(Reset)
