...
impl StateComputer for MockStateComputer {
    ...
    async fn commit(
        &self, _blocks: Vec<&ExecutedBlock<Self::Payload>>,
        commit: LedgerInfoWithSignatures, _synced_trees: &ExecutedTrees,
    ) -> Result<()>
    {
        self.consensus_db
...
impl StateComputer for EmptyStateComputer {
    ...
    async fn commit(
        &self, _blocks: Vec<&ExecutedBlock<Self::Payload>>,
        _commit: LedgerInfoWithSignatures, _synced_trees: &ExecutedTrees,
    ) -> Result<()>
    {
        Ok(())
    }
