...
/// all nodes crash, safety is guaranteed.  This trait also also supports
/// liveness aspects (i.e. highest timeout certificate) and supports clean up
/// (i.e. tree pruning). Blocks persisted are proposed but not yet committed.
/// The committed state is persisted via StateComputer.
pub trait PersistentStorage<T>: Send + Sync {
    /// Persist the blocks and quorum certs into storage atomically.
    fn save_tree(
        &self, blocks: Vec<Block<T>>, quorum_certs: Vec<QuorumCert>,
    ) -> Result<()>;
...
impl<T: Payload> RecoveryData<T> {
    ...
    {
        let root =
            Self::find_root(&mut blocks, &mut quorum_certs, storage_ledger)
                .with_context(|| {
    ...
    pub fn root_block(&self) -> &Block<T> { &self.root.0 }
    ...
    /// info always exists. In the case of an epoch boundary ledger
    /// info(i.e. it has validator set), we generate the virtual genesis block.
    fn find_root(
        blocks: &mut Vec<Block<T>>, quorum_certs: &mut Vec<QuorumCert>,
        storage_ledger: &LedgerInfo,
    ...
    fn find_blocks_to_prune(
        root_id: HashValue, blocks: &mut Vec<Block<T>>,
...
impl<T: Payload> PersistentStorage<T> for StorageWriteProxy {
    ...
    fn save_highest_timeout_cert(
        ...
        self.db.save_highest_timeout_certificate(lcs::to_bytes(
