...
use crate::{
    chained_bft::{
        ...
        test_utils::{
            consensus_runtime, with_smr_id, MockStateComputer, MockStorage,
...
use libra_types::crypto_proxies::{
    ...
    ValidatorSet, ValidatorSigner, ValidatorVerifier,
};
use network::{
...
impl SMRNode {
    fn start(
        playground: &mut NetworkPlayground, signer: ValidatorSigner,
        smr_id: usize, storage: Arc<MockStorage<TestPayload>>,
    ...
    ) -> Self
    {
        ...
        let safety_rules_manager_config =
            SafetyRulesManagerConfig::new_with_signer(
                signer.clone(),
    ...
    fn start_num_nodes(
        num_nodes: usize, playground: &mut NetworkPlayground,
        proposer_type: ConsensusProposerType, executor_with_reconfig: bool,
    ) -> Vec<Self>
    {
        let (mut signers, validators) =
            random_validator_verifier(num_nodes, None, true);
...
fn basic_start_test() {
    ...
    let nodes =
        SMRNode::start_num_nodes(2, &mut playground, RotatingProposer, false);
    let genesis = nodes[0]
        .smr
        .block_store()
...
#[test]
/// Upon startup, the first proposal is sent, delivered and voted by all the
/// participants.
fn start_with_proposal_test() {
    ...
    block_on(async move {
        ...
        // Verify that the proposed block id is indeed present in the block
        // store.
        assert!(nodes[0]
            .smr
            .block_store()
...
fn basic_full_round(num_nodes: usize, proposer_type: ConsensusProposerType) {
    ...
    // In case we're using multi-proposer, every proposal and vote is sent to
    // two participants.
    ...
    block_on(async move {
        ...
        let next_proposal: ProposalMsg<Vec<u64>> =
            ProposalUncheckedSignatures::<Vec<u64>>::try_from(
                broadcast_proposals_2[0].1.clone(),
            )
            .unwrap()
            .into();
...
#[test]
/// Upon startup, the first proposal is sent, voted by all the participants, QC
/// is formed and then the next proposal is sent.
fn basic_full_round_test() { basic_full_round(2, FixedProposer); }
...
/// Verify the basic e2e flow: blocks are committed, txn manager is notified,
/// block tree is pruned, restart the node and we can still continue.
#[test]
fn basic_commit_and_restart() {
    ...
    block_on(async {
        ...
        for round in 0..num_rounds {
            ...
            if round >= 3 {
                ...
                let commit_v1 =
                    nodes[0].commit_cb_receiver.next().await.unwrap();
                let commit_v2 =
                    nodes[1].commit_cb_receiver.next().await.unwrap();
                assert_eq!(
        ...
        // This message is for proposal with round 11 to delivery the QC, but
        // not gather the QC so after restart, proposer will propose
        // round 11 again.
        playground
            .wait_for_messages(1, NetworkPlayground::exclude_timeout_msg)
            .await;
    });
    // create a new playground to avoid polling potential vote messages in
    // previous one.
    ...
    block_on(async {
        ...
        while round < 10 {
            // The loop is to ensure that we collect a network vote(enough for
            // QC with 2 nodes) then move the round forward because
            // there's a race that node1 may or may not reject round
            // 11 depends on whether it voted for before restart.
            loop {
                let msg = playground
                    .wait_for_messages(
                        1,
                        NetworkPlayground::exclude_timeout_msg,
                    )
                    .await;
        ...
        // Because of the race, we can't assert the commit reliably, instead we
        // assert both nodes commit to at least round 17.
        // We cannot reliable wait for the event of "commit & prune": the only
        // thing that we know is that after receiving the vote for round
        // 20, the root should be at least height 16.
        assert!(
...
fn basic_block_retrieval() {
    ...
    let mut nodes =
        SMRNode::start_num_nodes(4, &mut playground, FixedProposer, false);
    block_on(async move {
        ...
        // The next proposal is delivered to all: as a result nodes[2] should
        // retrieve the missing blocks from nodes[0] and vote for the
        // 3th proposal.
        playground.stop_drop_message_for(
            &nodes[0].signer.author(),
            &nodes[3].signer.author(),
        );
        ...
        playground
            .wait_for_messages(2, NetworkPlayground::votes_only)
            .await;
        // The first two proposals should be present at nodes[3] via block
        // retrieval
        ...
        // 4th proposal will get quorum and verify that nodes[3] commits the
        // first proposal.
        playground
            .wait_for_messages(2, NetworkPlayground::proposals_only)
            .await;
...
#[test]
/// Verify that a node that is lagging behind can catch up by state sync some
/// blocks have been pruned by the others.
fn basic_state_sync() {
    let runtime = consensus_runtime();
    let mut playground = NetworkPlayground::new(runtime.handle().clone());
    // This test depends on the fixed proposer on nodes[0]
    ...
    block_on(async move {
        ...
        // The first ten proposals are delivered just to nodes[0..2], which
        // should commit the first seven blocks.
        playground.drop_message_for(
            &nodes[0].signer.author(),
            nodes[3].signer.author(),
        );
        ...
        // Next proposal is delivered to all: as a result nodes[3] should be
        // able to retrieve the missing blocks from nodes[0] and commit
        // the first eight proposals as well.
        playground.stop_drop_message_for(
            &nodes[0].signer.author(),
            &nodes[3].signer.author(),
        );
        ...
        playground
            .wait_for_messages(3, NetworkPlayground::proposals_only)
            .await;
        // Verify that node 3 has notified its mempool about the committed txn
        // of next block.
        nodes[3]
...
#[test]
/// Verify that a node syncs up when receiving a timeout message with a relevant
/// ledger info
fn state_sync_on_timeout() {
    ...
    block_on(async move {
        // The first ten proposals are delivered just to nodes[0..2], which
        // should commit the first seven blocks.
        // nodes[2] should be fully disconnected from the others s.t. its
        // timeouts would not trigger SyncInfo delivery ahead of time.
        playground.drop_message_for(
            &nodes[0].signer.author(),
            nodes[3].signer.author(),
        );
        ...
        // Stop dropping messages from node 1 to node 0: next time node 0 sends
        // a timeout to node 1, node 1 responds with a SyncInfo that
        // carries a LedgerInfo for commit at round >= 7.
        playground.stop_drop_message_for(
            &nodes[1].signer.author(),
            &nodes[3].signer.author(),
        );
        ...
        playground
            ...
            .await;
        // In the end of the state synchronization node 3 should have commit at
        // round >= 7.
        assert!(
            nodes[3]
                .commit_cb_receiver
...
#[test]
/// Verify that in case a node receives timeout message from a remote peer that
/// is lagging behind, then this node sends a sync info, which helps the remote
/// to properly catch up.
fn sync_info_sent_if_remote_stale() {
    let runtime = consensus_runtime();
    let mut playground = NetworkPlayground::new(runtime.handle().clone());
    // This test depends on the fixed proposer on nodes[0]
    // We're going to drop messages from 0 to 2: as a result we expect node 2 to
    // broadcast timeout messages, for which node 1 should respond with
    // sync_info, which should eventually help node 2 to catch up.
    let mut nodes =
        SMRNode::start_num_nodes(4, &mut playground, FixedProposer, false);
    block_on(async move {
        playground.drop_message_for(
        ...
        // Don't want to receive timeout messages from 2 until 1 has some real
        // stuff to contribute.
        playground.drop_message_for(
            &nodes[2].signer.author(),
            nodes[1].signer.author(),
        ...
        // Close node 1 channel for new commit callbacks and iterate over all
        // its commits: we should find the node 2 commit there.
        let mut found = false;
        nodes[1].commit_cb_receiver.close();
        while let Ok(Some(node1_commit)) =
            nodes[1].commit_cb_receiver.try_next()
        {
            let node1_commit_id =
                node1_commit.ledger_info().consensus_block_id();
...
#[test]
/// Verify that a QC can be formed by aggregating the votes piggybacked by
/// TimeoutMsgs
fn aggregate_timeout_votes() {
    let runtime = consensus_runtime();
    let mut playground = NetworkPlayground::new(runtime.handle().clone());

    // The proposer node[0] sends its proposal to nodes 1 and 2, which cannot
    // respond back, because their messages are dropped.
    // Upon timeout nodes 1 and 2 are sending timeout messages with attached
    // votes for the original proposal: both can then aggregate the QC for
    // the first proposal.
    let nodes =
        SMRNode::start_num_nodes(3, &mut playground, FixedProposer, false);
    block_on(async move {
        ...
        // Now when the nodes 1 and 2 have the votes from 0, enable
        // communication between them. As a result they should get the
        // votes from each other and thus be able to form a QC.
        playground.stop_drop_message_for(
            &nodes[2].signer.author(),
            &nodes[1].signer.author(),
        );
...
#[test]
/// Verify that the NIL blocks formed during timeouts can be used to form commit
/// chains.
fn chain_with_nil_blocks() {
    let runtime = consensus_runtime();
    let mut playground = NetworkPlayground::new(runtime.handle().clone());

    // The proposer node[0] sends 3 proposals, after that its proposals are
    // dropped and it cannot communicate with nodes 1, 2, 3. Nodes 1, 2, 3
    // should be able to commit the 3 proposal via NIL blocks commit chain.
    let num_nodes = 4;
    let nodes = SMRNode::start_num_nodes(
        num_nodes,
        &mut playground,
        FixedProposer,
        false,
    ...
    block_on(async move {
        ...
        // After the first timeout nodes 1, 2, 3 should have last_proposal votes
        // and they can generate its QC independently.
        // Upon the second timeout nodes 1, 2, 3 send NIL block_1 with a QC to
        // last_proposal. Upon the third timeout nodes 1, 2, 3 send NIL
        // block_2 with a QC to NIL block_1. G <- p1 <- p2 <- p3 <- NIL1
        // <- NIL2
        let num_timeout = 3;
        playground
            .wait_for_messages(
                // all-to-all broadcast except nodes 0's messages are dropped
                // and self messages don't count
                (num_nodes - 1) * (num_nodes - 1) * num_timeout,
                NetworkPlayground::timeout_votes_only,
            )
            .await;
        // We can't guarantee the timing of the last timeout processing, the
        // only thing we can look at is that HQC round is at least 4.
        assert!(
            nodes[2]
                .smr
                .block_store()
...
fn secondary_proposers() {
    ...
    let num_nodes = 4;
    let mut nodes = SMRNode::start_num_nodes(
        num_nodes,
        &mut playground,
    ...
    block_on(async move {
        ...
        // Run a system until node 0 is a designated primary proposer. In this
        // round the secondary proposal should be voted for and attached
        ...
        for msg in timeout_votes {
            ...
            secondary_proposal_ids
                .push(vote_msg.vote().vote_data().proposed().id());
        }
        assert_eq!(
        ...
        // The secondary proposal id should get committed at some point in the
        // future: 10 rounds should be more than enough. Note that it's
        // hard to say what round is going to have 2 proposals and what
        // round is going to have just one proposal because we don't want
        // to predict the rounds with proposer 0 being a leader.
        for _ in 0..10 {
            playground
                .wait_for_messages(num_nodes - 1, NetworkPlayground::votes_only)
                .await;
            // Retrieve all the ids committed by the node to check whether
            // secondary_proposal_id has been committed.
            while let Ok(Some(li)) = nodes[1].commit_cb_receiver.try_next() {
                if li.ledger_info().consensus_block_id()
...
fn reconfiguration_test() {
    ...
    // This quorum size needs to be 2f+1 because we derive the ValidatorVerifier
    // from ValidatorSet at network.rs which doesn't support specializing
    // quorum power
    let _nodes = SMRNode::start_num_nodes(
        4,
        &mut playground,
    ...
    block_on(async move {
        ...
        loop {
            let mut msg = playground
                ...
                .await;
            if let Some(ConsensusMsg_oneof::EpochChange(proof)) =
                msg.pop().unwrap().1.message
            {
