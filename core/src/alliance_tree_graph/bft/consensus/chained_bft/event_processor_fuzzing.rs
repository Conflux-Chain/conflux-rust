...
use crate::{
    chained_bft::{
        ...
        test_utils::{
            EmptyStateComputer, MockStorage, MockTransactionManager,
            TestPayload,
        },
    },
...
fn create_node_for_fuzzing(
) -> EventProcessor<MockTransactionManager, TestPayload> {
    ...
    let (initial_data, storage) =
        MockStorage::<TestPayload>::start_for_testing(validator_set);
...
pub fn fuzz_proposal(data: &[u8]) {
    ...
    let proposal =
        match ProposalUncheckedSignatures::<TestPayload>::try_from(proposal) {
            Ok(xx) => xx,
            Err(_) => {
                if cfg!(test) {
                    panic!();
