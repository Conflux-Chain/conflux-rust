...
/// The primary proposer is determined by an index of hash(round) %
/// num_proposers. The secondary proposer is determined by hash(hash(round)) %
/// (num_proposers - 1), etc. In order to ensure the required number of
/// proposers, a set of the proposers to choose from is updated after each hash:
/// a chosen candidate is removed to avoid duplication.
///
/// Note the hash doesn't have to be cryptographic. The goal is to make sure
/// that different combinations of consecutive leaders are going to appear with
/// equal probability.

/// While each round has more than a single valid proposer, only the primary
/// proposer is considered for `process_proposal`. The best backup proposer is
/// returned in `get_best_backup_proposal()`.
pub struct MultiProposer<T> {
    ...
    // Keeps the highest received backup (non-primary) proposal of the highest
    // round. When a proposal from a higher round is received, the previous
    // proposals are discarded: `process_proposal()` is supposed to be
    // called only if the proposal round matches the current round in the
    // system, hence, proposals from previous rounds can be discarded.
...
impl<T> MultiProposer<T> {
    pub fn new(
        epoch: u64, proposers: Vec<Author>, mut num_proposers_per_round: usize,
    ) -> Self {
...
impl<T: Payload> ProposerElection<T> for MultiProposer<T> {
    fn is_valid_proposer(
        &self, author: Author, round: Round,
    ) -> Option<Author> {
    ...
    fn process_proposal(&mut self, proposal: Block<T>) -> Option<Block<T>> {
        ...
        for (rank, candidate) in candidates.iter().enumerate() {
            ...
            if author == *candidate {
                // This is a valid non-primary proposal, add it to
                // backup_proposals.
                debug!(
                ...
                if round > self.backup_proposal_round {
                    ...
                    // Already have some backup for the given round: choose the
                    // best (lowest) rank.
