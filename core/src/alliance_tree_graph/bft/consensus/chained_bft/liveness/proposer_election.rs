...
pub trait ProposerElection<T> {
    ...
    fn is_valid_proposer(&self, author: Author, round: Round)
        -> Option<Author>;

    /// Return all the possible valid proposers for a given round (this
    /// information can be used by e.g., voters for choosing the
    /// destinations for sending their votes to).
    fn get_valid_proposers(&self, round: Round) -> Vec<Author>;

    /// Notify proposer election about a new proposal. The function doesn't
    /// return any information: proposer election is going to notify the
    /// client about the chosen proposal via a dedicated channel (to be
    ...
    /// A backup proposal is a valid proposal that was not chosen immediately in
    /// the `process_proposal()` return value.
    ///
    /// Note that once the backup proposal is taken and no other proposals are
    /// submitted, the following take requests are going to return None.
