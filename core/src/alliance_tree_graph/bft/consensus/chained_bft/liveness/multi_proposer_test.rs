...
fn test_multi_proposer_take_all() {
    // In case num of proposers per round is equal to the overall num of
    // proposers all the proposers are valid candidates
    let epoch = 0u64;
    let mut signers = vec![];
    let mut proposers = vec![];
    for i in 0..8 {
...
fn test_multi_proposer_hash() {
    // Verify that the hash distributes primary proposers in a "reasonable"
    // fashion
    ...
    for round in 0..10000 {
        ...
        let idx =
            multi_proposer_election::next(&mut state) % (counts.len() as u64);
