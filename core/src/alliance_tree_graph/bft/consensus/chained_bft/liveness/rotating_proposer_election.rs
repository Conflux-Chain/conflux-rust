...
impl<T: Payload> ProposerElection<T> for RotatingProposer {
    ...
    fn process_proposal(&mut self, proposal: Block<T>) -> Option<Block<T>> {
        // This is a simple rotating proposer, the proposal is processed in the
        // context of the caller task, no synchronization required
        // because there is no mutable state.
