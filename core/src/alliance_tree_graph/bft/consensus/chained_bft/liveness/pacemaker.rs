...
/// NewRoundEvents produced by Pacemaker are guaranteed to be monotonically
...
/// `Pacemaker` is a Pacemaker implementation that is responsible for generating
/// the new round and local timeout events.
///
...
/// PacemakerTimeoutInterval trait. It depends on the delta between the current
...
pub struct Pacemaker {
    ...
    // might choose not to inform the Pacemaker about certain committed
    // rounds (e.g., NIL blocks): in this case the committed round in
    // Pacemaker might lag behind the committed round of a block tree.
...
impl Pacemaker {
    ...
    {
        // Our counters are initialized via lazy_static, so they're not going to
        // appear in Prometheus if some conditions never happen.
        // Invoking get() function enforces creation.
    ...
    /// Notify the Pacemaker about the potentially new QC, TC, and highest
    /// committed round. Note that some of these values might not be
    /// available by the caller.
    pub fn process_certificates(
        &mut self, hqc_round: Option<Round>, htc_round: Option<Round>,
