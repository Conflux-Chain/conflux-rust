...
pub struct NetworkPlayground {
    ...
    node_consensus_txs:
        Arc<Mutex<HashMap<Author, channel::Sender<NetworkNotification>>>>,
...
impl NetworkPlayground {
    ...
    async fn start_node_outbound_handler(
        drop_config: Arc<RwLock<DropConfig>>, src: Author,
        ...
        node_consensus_txs: Arc<
            Mutex<HashMap<Author, channel::Sender<NetworkNotification>>>,
        >,
    )
    {
    ...
    async fn deliver_message(
        &mut self, src: Author, msg: NetworkRequest,
    ) -> (Author, ConsensusMsg) {
        // extract destination peer
        let dst = match &msg {
            NetworkRequest::SendMessage(dst, _) => *dst,
            msg => panic!(
                "[network playground] Unexpected NetworkRequest: {:?}",
                msg
            ),
        };
    ...
    /// While all the sent messages are delivered, only the messages that
    /// satisfy the given msg inspector are counted.
    ...
    fn is_message_dropped(
        &self, src: &Author, net_req: &NetworkRequest,
    ) -> bool {
        self.drop_config
            .read()
            .unwrap()
    ...
    pub fn stop_drop_message_for(
        &mut self, src: &Author, dst: &Author,
    ) -> bool {
        self.drop_config
            .write()
            .unwrap()
...
impl DropConfig {
    pub fn is_message_dropped(
        &self, src: &Author, net_req: &NetworkRequest,
    ) -> bool {
        match net_req {
            NetworkRequest::SendMessage(dst, _) => {
                self.0.get(src).unwrap().contains(&dst)
            }
            NetworkRequest::SendRpc(dst, _) => {
                self.0.get(src).unwrap().contains(&dst)
            }
            _ => true,
        }
    ...
    fn add_node(&mut self, src: Author) { self.0.insert(src, HashSet::new()); }
}
...
fn test_rpc() {
    ...
    let on_request_block = async move {
        while let Some(request) = block_retrieval.next().await {
            ...
            // blocked, we can not process 2 votes and the test will
            // timeout
