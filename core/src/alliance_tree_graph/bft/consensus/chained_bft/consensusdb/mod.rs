...
use schemadb::{
    ColumnFamilyOptions, ColumnFamilyOptionsMap, ReadOptions, SchemaBatch, DB,
    DEFAULT_CF_NAME,
};
...
impl ConsensusDB {
    pub fn new<P: AsRef<Path> + Clone>(db_root_path: P) -> Self {
        ...
        info!("ConsensusDB path: {:?}", path.clone());
    ...
    pub fn save_highest_timeout_certificate(
        &self, highest_timeout_certificate: HighestTimeoutCertificate,
    ) -> Result<()> {
    ...
    pub fn save_blocks_and_quorum_certificates<T: Payload>(
        &self, block_data: Vec<Block<T>>, qc_data: Vec<QuorumCert>,
    ) -> Result<()> {
        ensure!(
    ...
    fn get_blocks<T: Payload>(&self) -> Result<HashMap<HashValue, Block<T>>> {
        ...
        iter.seek_to_first();
        iter.map(|value| {
            value.and_then(|(k, v)| Ok((k, v.borrow_into_block().clone())))
        })
    ...
    fn get_quorum_certificates(
        &self,
    ) -> Result<HashMap<HashValue, QuorumCert>> {
