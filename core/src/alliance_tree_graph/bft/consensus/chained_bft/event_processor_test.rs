...
use crate::{
    chained_bft::{
        ...
        liveness::{
            pacemaker::{
                ExponentialTimeInterval, NewRoundEvent, NewRoundReason,
                Pacemaker,
            },
        ...
        test_utils::{
            self, consensus_runtime, MockStateComputer, MockStorage,
...
use libra_types::{
    ...
    crypto_proxies::{
        random_validator_verifier, EpochInfo, LedgerInfoWithSignatures,
...
fn basic_new_rank_event_test() {
    ...
    let mut inserter = TreeInserter::new_with_store(
        node.signer.clone(),
    ...
    block_on(async move {
        ...
        let pending_proposals: Vec<ProposalMsg<TestPayload>> = pending_messages
            ...
            .filter_map(|m| match m.1.message {
                Some(ConsensusMsg_oneof::Proposal(proposal)) => Some(
                    ProposalUncheckedSignatures::<TestPayload>::try_from(
                        proposal,
                    )
                    .unwrap()
                    .into(),
        ...
        // Simulate a case with a1 receiving enough votes for a QC: a new
        // proposal should be a child of a1 and carry its QC.
        let vote = Vote::new(
            VoteData::new(
...
fn process_old_proposal_test() {
    ...
    let genesis_qc = certificate_for_genesis();
    let new_block =
        Block::new_proposal(vec![1], 1, 1, genesis_qc.clone(), &node.signer);
    let new_block_id = new_block.id();
    let old_block =
        Block::new_proposal(vec![1], 1, 2, genesis_qc.clone(), &node.signer);
...
fn process_round_mismatch_test() {
    ...
    let block_skip_round =
        Block::new_proposal(vec![1], 2, 2, genesis_qc.clone(), &node.signer);
    block_on(async move {
...
fn process_vote_timeout_msg_test() {
    ...
    // As the static proposer processes the the vote message it should learn
    // about the block_0_quorum_cert at round 1.
    let dummy_vote_data =
