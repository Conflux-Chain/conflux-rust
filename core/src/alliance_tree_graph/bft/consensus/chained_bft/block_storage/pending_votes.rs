...
struct LastVoteInfo {
    li_digest: HashValue,
    round: Round,
    is_timeout: bool, /* true if a vote includes a round signature that can
                       * be aggregated to TC */
}

/// Last pending votes of the authors. Should be cleared upon reconfiguration.
pub struct PendingVotes {
    /// `li_digest_to_votes` might keep multiple LedgerInfos per proposed block
    /// in order to tolerate non-determinism in execution: given a
    /// proposal, a QuorumCertificate is going to be collected only for all
    /// the votes that carry identical LedgerInfo. LedgerInfo digest covers
    /// the potential commit ids, as well as the vote information
    ...
    /// succeed to aggregate 2f+1 signatures for the same round a
    /// TimeoutCertificate is formed. Note that QuorumCert has higher
    /// priority than TimeoutCertificate (in case 2f+1 votes are
    /// gathered for the same ledger info we are going to generate QuorumCert
    /// and not the TimeoutCertificate).
    round_to_tc: HashMap<Round, TimeoutCertificate>,
    /// Map of Author to last vote info. Any pending vote from Author is
    /// cleaned up whenever a new vote is added by same Author
...
impl PendingVotes {
    ...
    fn aggregate_qc(
        ...
        // Note that the digest covers the ledger info information, which is
        // also indirectly covering vote data hash (in its
        // `consensus_data_hash` field).
        ...
        // TODO: we'd prefer to use LedgerInfoWithSignatures::add_signature
        // instead, but the CryptoProxy types should be properly updated
        // first.
        vote.signature()
            .clone()
        ...
        {
            Ok(_) => VoteReceptionResult::NewQuorumCertificate(Arc::new(
                QuorumCert::new(vote.vote_data().clone(), li_with_sig.clone()),
            )),
    ...
    /// In case a timeout certificate is formed (there are 2f+1 votes in the
    /// same round) return the new TimeoutCertificate, otherwise, return
    /// None.
    fn aggregate_tc(
        &mut self, vote: &Vote, validator_verifier: &ValidatorVerifier,
        ...
        let tc = self.round_to_tc.entry(timeout.round()).or_insert_with(|| {
            TimeoutCertificate::new(timeout, HashMap::new())
        });
        tc.add_signature(vote.author(), timeout_signature);
        match validator_verifier.check_voting_power(tc.signatures().keys()) {
            Ok(_) => Some(VoteReceptionResult::NewTimeoutCertificate(
                Arc::new(tc.clone()),
            )),
    ...
    /// already voted on same block then return DuplicateVote error. If Author
    /// has already voted on some other result, prune the last vote and
    /// insert new one in map.
    fn replace_prev_vote(
        &mut self, vote: &Vote,
        ...
        let last_voted_info =
            match self.author_to_last_voted_info.insert(author, vote_info) {
                None => {
        ...
        if li_digest == last_voted_info.li_digest {
            if is_timeout == last_voted_info.is_timeout {
                ...
                // Author has already voted for this LedgerInfo, but this time
                // the Vote's round signature is different.
        ...
        if let Some(li_pending_votes) =
            self.li_digest_to_votes.get_mut(&last_voted_info.li_digest)
        {
        ...
        if round == last_voted_info.round {
            // The same author has already sent a vote for this round but a
            // different LedgerInfo digest: this is not a valid
            // behavior.
            error!(
        ...
        if let Some(pending_tc) =
            self.round_to_tc.get_mut(&last_voted_info.round)
        {
