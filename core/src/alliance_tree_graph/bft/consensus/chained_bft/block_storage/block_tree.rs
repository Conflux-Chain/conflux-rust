...
{
    pub(super) fn new(
        root: ExecutedBlock<T>, root_quorum_cert: QuorumCert,
    ...
    pub(super) fn insert_block(
        &mut self, block: ExecutedBlock<T>,
        ...
        if let Some(existing_block) = self.get_block(&block_id) {
            debug!("Already had block {:?} for id {:?} when trying to add another block {:?} for the same id",
                       ...
                       block_id,
                       block);
            //checked_verify_eq!(existing_block.compute_result(),
    ...
    pub(super) fn insert_vote(
        ...
        if let Some(old_qc) = self.id_to_quorum_cert.get(&block_id) {
            return VoteReceptionResult::OldQuorumCertificate(Arc::clone(
                old_qc,
            ));
        }
        let res = self.pending_votes.insert_vote(vote, validator_verifier);
        if let VoteReceptionResult::NewQuorumCertificate(_) = res {
            // Note that the block might not be present locally, in which case
            // we cannot calculate time between block creation and
            // qc
            if let Some(time_to_qc) =
                self.get_block(&block_id).and_then(|block| {
                    duration_since_epoch().checked_sub(Duration::from_micros(
