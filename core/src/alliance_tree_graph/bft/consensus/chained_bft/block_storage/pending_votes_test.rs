...
#[test]
/// Verify that votes are properly aggregated to QC based on their LedgerInfo
/// digest
fn test_qc_aggregation() {
    ...
    // A different author voting for a different result in the same round but
    // without a round signature: VoteAdded
...
fn test_qc_aggregation_keep_last_only() {
    ...
    // another author voting for round 1 cannot form a QC because the old vote
    // is gone
...
fn test_tc_aggregation() {
    ...
    // another vote for the same round with a different value cannot form a TC
    // if it doesn't have a round signature
    let li2 = random_ledger_info();
