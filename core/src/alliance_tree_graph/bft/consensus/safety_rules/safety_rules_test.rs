...
use super::{test_utils, Error, InMemoryStorage, SafetyRules, TSafetyRules};
use consensus_types::{
...
fn make_proposal_with_qc_and_proof(
    round: Round, proof: Proof, qc: QuorumCert,
...
fn make_proposal_with_parent(
    round: Round, parent: &VoteProposal<Round>,
...
fn test_voting_potential_commit_id() {
    ...
    let a4 =
        make_proposal_with_parent(round + 5, &a3, Some(&a2), &validator_signer);
    let a5 =
        make_proposal_with_parent(round + 6, &a4, Some(&a3), &validator_signer);
