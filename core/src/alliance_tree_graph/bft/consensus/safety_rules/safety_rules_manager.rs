...
impl SafetyRulesManagerConfig {
    pub fn new(config: &mut NodeConfig) -> Self {
        let private_key = config
            ...
            .take_private()
            .expect(
            "Failed to take Consensus private key, key absent or already read",
        );
    ...
    pub fn new_with_signer(
        validator_signer: ValidatorSigner, config: &SafetyRulesConfig,
    ) -> Self {
...
impl<T: Payload> SafetyRulesManager<T> {
    ...
    pub fn new_direct(
        storage: Box<dyn PersistentStorage>, validator_signer: ValidatorSigner,
    ) -> Self {
        let safety_rules =
            SafetyRules::new(storage, Arc::new(validator_signer));
