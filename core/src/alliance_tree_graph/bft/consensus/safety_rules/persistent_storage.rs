...
/// before returning. @TODO add access to private key from persistent store
...
impl InMemoryStorage {
    pub fn new(
        epoch: u64, last_voted_round: Round, preferred_round: Round,
    ) -> Self {
        Self {
            epoch,
            last_voted_round,
...
impl PersistentStorage for InMemoryStorage {
    ...
    fn preferred_round(&self) -> Round { self.preferred_round }
    ...
    fn last_voted_round(&self) -> Round { self.last_voted_round }
...
impl OnDiskStorage {
    pub fn new_storage(
        file_path: PathBuf,
    ) -> Result<Box<dyn PersistentStorage>> {
...
impl PersistentStorage for OnDiskStorage {
    fn epoch(&self) -> u64 { self.internal_data.epoch() }
    ...
    fn preferred_round(&self) -> Round { self.internal_data.preferred_round() }
