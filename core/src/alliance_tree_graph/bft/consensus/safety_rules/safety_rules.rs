...
/// @TODO bootstrap with a hash of a ledger info (waypoint) that includes a
/// validator set @TODO update storage with hash of ledger info (waypoint)
/// during epoch changes (includes a new validator set)
...
impl<T: Payload> SafetyRules<T> {
    ...
    /// rule is: B0 (as well as its prefix) can be committed if there exist
    ...
    pub fn signer(&self) -> &ValidatorSigner { &self.validator_signer }
}
...
impl<T: Payload> TSafetyRules<T> for SafetyRules<T> {
    ...
    /// @TODO if public key does not match private key in validator set, access
    /// persistent storage to identify new key
    ...
    /// @TODO only sign blocks that are later than last_voted_round and match
    /// the current epoch @TODO verify QC correctness
    /// @TODO verify QC matches preferred round
    fn sign_proposal(
        &self, block_data: BlockData<T>,
    ...
    /// @TODO only sign a timeout if it matches last_voted_round or
    /// last_voted_round + 1 @TODO update last_voted_round
