...
pub struct ExecutedBlock<T> {
    ...
    block: Block<T>,
    /* The execution output is calculated for all the pending blocks prior
     * to insertion to the tree. The execution results are not
     * persisted: they're recalculated again for the pending blocks upon
     * restart. output: Arc<ProcessedVMOutput>, */
}

impl<T: PartialEq> PartialEq for ExecutedBlock<T> {
    fn eq(&self, other: &Self) -> bool {
        self.block == other.block
        //&& self.compute_result() == other.compute_result()
    }
}
...
impl<T: PartialEq> Display for ExecutedBlock<T> {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result { self.block().fmt(f) }
}
...
impl<T> ExecutedBlock<T> {
    ...
    pub fn block(&self) -> &Block<T> { &self.block }
    ...
    pub fn payload(&self) -> Option<&T> { self.block().payload() }
    ...
    */
    ...
    pub fn block_info(&self) -> BlockInfo {
        ...
        self.block().gen_block_info(
            ...
            None,              /* executed_state.validators */
        )
    }
}
...
where T: PartialEq
{
    pub fn is_nil_block(&self) -> bool { self.block().is_nil_block() }
}
