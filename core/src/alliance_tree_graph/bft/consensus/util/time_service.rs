...
impl TimeService for ClockTimeService {
    ...
    fn get_current_timestamp(&self) -> Duration { duration_since_epoch() }
...
pub enum WaitingSuccess {
    /// No waiting to complete and includes the current duration since epoch
    /// and the difference between the current duration since epoch and
    /// min_duration_since_epoch
    NoWaitRequired {
        current_duration_since_epoch: Duration,
        early_duration: Duration,
    },
    /// Waiting was required and includes the current duration since epoch and
    /// the duration slept to finish waiting
    WaitWasRequired {
        current_duration_since_epoch: Duration,
...
pub enum WaitingError {
    /// The waiting period exceeds the maximum allowed duration, returning
    /// immediately
    MaxWaitExceeded,
    /// Waiting to ensure the current time exceeds min_duration_since_epoch
    /// failed
    WaitFailed {
        current_duration_since_epoch: Duration,
        wait_duration: Duration,
...
/// Attempt to wait until the current time exceeds the min_duration_since_epoch
/// if possible
///
/// If the waiting time exceeds max_instant then fail immediately.
/// There are 4 potential outcomes, 2 successful and 2 errors, each represented
/// by WaitingSuccess and WaitingError.
pub async fn wait_if_possible(
    time_service: &dyn TimeService, min_duration_since_epoch: Duration,
    max_instant: Instant,
) -> Result<WaitingSuccess, WaitingError>
{
    // Fail early if waiting for min_duration_since_epoch would exceed
    // max_instant Ideally, comparing min_duration_since_epoch and
    // max_instant would be straightforward, but min_duration_since_epoch is
    // relative to UNIX_EPOCH and Instant is not comparable.  Therefore,
    ...
    if current_instant <= max_instant {
        ...
        if current_duration_since_epoch <= min_duration_since_epoch {
            let duration_to_min_time =
                min_duration_since_epoch - current_duration_since_epoch;
    ...
    if current_duration_since_epoch <= min_duration_since_epoch {
        // Delay has millisecond granularity, add 1 millisecond to ensure a
        // higher timestamp
        let sleep_duration = min_duration_since_epoch
        ...
        Ok(WaitingSuccess::NoWaitRequired {
            current_duration_since_epoch,
            early_duration: current_duration_since_epoch
                - min_duration_since_epoch,
        })
    }
}
