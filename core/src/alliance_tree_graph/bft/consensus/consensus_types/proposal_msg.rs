...
impl<T: Payload> From<ProposalUncheckedSignatures<T>> for ProposalMsg<T> {
    fn from(proposal: ProposalUncheckedSignatures<T>) -> Self { proposal.0 }
}

impl<T: Payload> ProposalUncheckedSignatures<T> {
    /// Validates the signatures of the proposal. This includes the leader's
    /// signature over the block and the QC, the timeout certificate
    /// signatures.
    pub fn validate_signatures(
        self, validator: &ValidatorVerifier,
    ) -> Result<ProposalMsg<T>> {
...
impl<T: Payload> ProposalMsg<T> {
    ...
    pub fn proposal(&self) -> &Block<T> { &self.proposal }

    pub fn take_proposal(self) -> Block<T> { self.proposal }

    pub fn sync_info(&self) -> &SyncInfo { &self.sync_info }

    pub fn round(&self) -> Round { self.proposal.round() }
