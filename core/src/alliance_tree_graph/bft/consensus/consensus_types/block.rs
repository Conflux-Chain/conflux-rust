...
impl<T> Block<T> {
    ...
    pub fn payload(&self) -> Option<&T> { self.block_data.payload() }
...
where T: Default + PartialEq + Serialize
{
    ...
    pub fn make_genesis_block_from_ledger_info(
        ...
        let block_data = BlockData::new_genesis(
            ledger_info.timestamp_usecs(),
            genesis_quorum_cert,
        );
    ...
    pub fn new_proposal(
        payload: T, round: Round, timestamp_usecs: u64,
    ...
    pub fn new_proposal_from_block_data(
        block_data: BlockData<T>, validator_signer: &ValidatorSigner,
    ) -> Self {
