...
pub struct BlockData<T> {
    ...
    /// least f+1    honest replicas think that T is in the past.  An
    /// honest replica will only vote    on a block when its own clock >=
    /// timestamp T. 3. If a block of transactions B is agreed on with
    /// timestamp T, then at least f+1 honest    replicas saw the contents
    /// of B no later than T + delta for some delta.    If T = 3:00 PM and
    /// delta is 10 minutes, then an honest replica would not have    voted
    /// for B unless its clock was between 3:00 PM to 3:10 PM at the time the
    ///    proposal was received.  After 3:10 PM, an honest replica would no
    /// longer vote    on B, noting it was too far in the past.
    timestamp_usecs: u64,
...
impl<T> BlockData<T> {
    ...
    pub fn block_type(&self) -> &BlockType<T> { &self.block_type }
...
where T: Default + Serialize
{
    ...
    pub fn new_proposal(
        payload: T, author: Author, round: Round, timestamp_usecs: u64,
        quorum_cert: QuorumCert,
    ) -> Self
    {
        Self {
...
where T: Serialize
{
    ...
    fn hash(&self) -> HashValue {
        let bytes =
            lcs::to_bytes(self).expect("BlockData serialization failed");
