...
use libra_crypto::hash::{
    CryptoHash, TestOnlyHasher, ACCUMULATOR_PLACEHOLDER_HASH,
};
use libra_crypto::hash::{CryptoHasher, HashValue};
#[cfg(test)]
use libra_types::proof::TestAccumulatorInternalNode;
use libra_types::proof::{
    accumulator::InMemoryAccumulator, definition::LeafCount,
};
use serde::{Deserialize, Serialize};
use std::marker::PhantomData;

/// A proof that first verifies that establishes correct computation of the root
/// and then returns the new tree to acquire a new root and version. Note: this
/// is used internally by VoteProposal hence why it exists within
/// consensus-types andd not libra-types. @TODO This should contain
/// AccumulatorConsistencyProof once that is code complete
