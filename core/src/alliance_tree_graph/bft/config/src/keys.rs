...
use libra_crypto::{
    test_utils::TEST_SEED, PrivateKey, Uniform, ValidKeyStringExt,
};
...
where T: Serialize + ValidKeyStringExt
{
    fn serialize<S>(
        &self, serializer: S,
...
where T: ValidKeyStringExt + DeserializeOwned + 'static
{
    fn deserialize<D>(
        deserializer: D,
    ) -> std::result::Result<PrivateKeyContainer<T>, D::Error>
    where D: Deserializer<'de> {
        // Note: Any error in parsing is assumed to be due to the private key
        // being absent.
...
where
    ...
    T::PublicKeyMaterial:
        DeserializeOwned + 'static + Serialize + ValidKeyStringExt,
{
        ...
        DeserializeOwned + 'static + Serialize + ValidKeyStringExt,
{
    ...
    pub fn public(&self) -> &T::PublicKeyMaterial { &self.public_key }

    /// Beware, this destroys the private key from this NodeConfig
    pub fn take_private(&mut self) -> Option<T> { self.private_key.take() }
}
