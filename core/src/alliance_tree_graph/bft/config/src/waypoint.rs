...
use libra_types::{
    crypto_proxies::ValidatorSet, ledger_info::LedgerInfo, transaction::Version,
};
...
/// which provides an off-chain mechanism to verify the sync process right after
/// the restart. At high level, a trusted waypoint verifies the LedgerInfo for a
...
pub struct Waypoint {
    ...
    /// by this waypoint.
    version: Version,
    /// The hash of the chosen fields of LedgerInfo (including the next
    /// validator set).
    value: HashValue,
}
    ...
    pub fn new(ledger_info: &LedgerInfo) -> Result<Self> {
        Ok(Self {
            ...
            next_validator_set: ledger_info
                ...
                .ok_or_else(|| {
                    format_err!(
                        "Cannot create a waypoint without validator set"
                    )
                })?
                .clone(),
        })
...
impl CryptoHash for Ledger2WaypointConverter {
    ...
    fn hash(&self) -> HashValue {
        let bytes = lcs::to_bytes(self)
            .expect("Ledger2WaypointConverter serialization failed");
...
mod test {
    ...
    use libra_types::{
        block_info::BlockInfo, crypto_proxies::ValidatorSet,
