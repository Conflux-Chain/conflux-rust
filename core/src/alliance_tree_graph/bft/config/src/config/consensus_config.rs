...
impl ConsensusConfig {
    ...
    fn default_peers(
        keypair: &ConsensusKeyPair, peer_id: PeerId,
...
impl ConsensusPeersConfig {
    ...
    ) -> ValidatorSet
    {
        let mut keys: Vec<ValidatorPublicKeys> = self
            ...
            .collect();
        // self.peers is a HashMap, so iterating over it produces a differently
        // ordered vector each time. Sort by account address to produce
...
mod test {
    ...
    fn test_with_defaults() {
        ...
        // Assert default loading doesn't affect paths and default remain in
        // place
