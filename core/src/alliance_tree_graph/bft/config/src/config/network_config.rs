...
pub struct NetworkConfig {
    ...
    // The address that this node advertises to other nodes for the discovery
    // protocol.
    ...
    // If the network is permissioned, only trusted peers are allowed to
    // connect. Otherwise, any node can connect. If this flag is set to
    // true, the `enable_encryption_and_authentication` must also be set to
    // true.
    ...
    // network peers are the nodes allowed to connect when the network is
    // started in permissioned mode.
    #[serde(skip)]
...
impl NetworkConfig {
    ...
    pub fn clone_for_template(&self) -> Self {
        Self {
            ...
            enable_encryption_and_authentication: self
    ...
    */
    ...
    pub fn random_with_peer_id(
        ...
        self.peer_id = if let Some(peer_id) = peer_id {
            ...
            PeerId::try_from(network_keypairs.identity_keys.public().to_bytes())
                .unwrap()
        };
...
pub struct SeedPeersConfig {
    // All peers config. Key:a unique peer id, will be PK in future, Value:
    // peer discovery info
    pub seed_peers: HashMap<PeerId, Vec<Multiaddr>>,
}

// Leveraged to store the network keypairs together on disk separate from this
// config
#[cfg_attr(any(test, feature = "fuzzing"), derive(Clone))]
...
mod test {
    ...
    fn test_with_defaults() {
        ...
        // Assert default loading doesn't affect paths and defaults remain in
        // place
