...
/// Holds the VM configuration, currently this is only the publishing options
/// for scripts and modules, but in the future this may need to be expanded to
...
impl VMConfig {
    /// Creates a new `VMConfig` where the whitelist is empty. This should only
    /// be used for testing.
...
/// possible configurations: 1. No module publishing, only whitelisted scripts
/// are allowed. 2. No module publishing, custom scripts are allowed.
/// 3. Both module publishing and custom scripts are allowed.
/// We represent these as an enum instead of a struct since whitelisting and
...
impl VMPublishingOption {
    ...
    pub fn get_whitelist_set(
        &self,
    ) -> Option<&HashSet<[u8; SCRIPT_HASH_LENGTH]>> {
        match self {
...
fn string_to_script_hash(input: &str) -> [u8; SCRIPT_HASH_LENGTH] {
    ...
    let decoded_hash = hex::decode(input)
        .expect("Unable to decode script hash from configuration file.");
...
where D: Deserializer<'de> {
    let whitelisted_scripts: Vec<String> =
...
fn serialize_whitelist<S, H>(
    whitelist: &HashSet<[u8; SCRIPT_HASH_LENGTH], H>, serializer: S,
) -> Result<S::Ok, S::Error>
where
    S: Serializer,
    H: BuildHasher,
{
    let encoded_whitelist: Vec<String> =
        whitelist.iter().map(hex::encode).collect();
