...
mod test {
    ...
    const RANDOM_DEFAULT: &str =
        "src/config/test_data/random.default.node.config.toml";
    const RANDOM_COMPLETE: &str =
        "src/config/test_data/random.complete.node.config.toml";

    #[test]
    fn verify_default_config() {
        // This test likely failed because there was a breaking change in the
        // NodeConfig. It may be desirable to reverse the change or to
        // change the test config and potentially documentation.
        let mut actual = NodeConfig::random();
        let mut expected =
            NodeConfig::load(DEFAULT).expect("Unable to load config");

        // These are randomly generated, so let's force them to be the same,
        // perhaps we can use a random seed so that these can be made
        // uniform...
        expected.consensus.consensus_keypair =
            actual.consensus.consensus_keypair.clone();
        expected.consensus.consensus_peers =
            actual.consensus.consensus_peers.clone();
        ...
        expected_network.advertised_address =
            actual_network.advertised_address.clone();
        expected_network.listen_address = actual_network.listen_address.clone();
        expected_network.network_keypairs =
            actual_network.network_keypairs.clone();
    ...
    fn verify_random_complete_config() {
        ...
        let root_dir =
            RootPath::new(expected.test.as_ref().unwrap().temp_dir().unwrap());
        let path = root_dir.full_path(&PathBuf::from("node.config.toml"));
        expected.save(&path).expect("Unable to save config");

        let actual =
            NodeConfig::load(RANDOM_COMPLETE).expect("Unable to load config");
        expected.set_data_dir(actual.data_dir().clone());
        compare_configs(&actual, &expected);
    }
    ...
    fn verify_random_default_config() {
        ...
        let actual =
            NodeConfig::load(RANDOM_DEFAULT).expect("Unable to load config");
        expected.set_data_dir(actual.data_dir().clone());
        compare_configs(&actual, &expected);
    }

    fn compare_configs(actual: &NodeConfig, expected: &NodeConfig) {
        // This is broken down first into smaller evaluations to improve
        // idenitfying what is broken. The output for a broken config
        // leveraging assert at the top level config is not readable.
    ...
    fn verify_all_configs() {
        let _ = vec![
            // This contains all the default fields written to disk, it
            // verifies that the default is consistent and can be
            // loaded without failure
            DEFAULT,
            // This config leverages default fields but uses the same PeerId
            // and secondary files as the random.complete.node.
            // config.toml. It verifies the assumptions about loading
            // files even if the paths aren't present
            RANDOM_DEFAULT,
            // This config explicitly writes all the default values for a
            // random peer to disk and verifies that it correctly
            // loads. It shares the same PeerId as random.default.
            // node.config.toml
            RANDOM_COMPLETE,
        ]
        .iter()
