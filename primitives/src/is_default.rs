...
mod tests{
  use crate::{Account, CodeInfo, DepositList, VoteStakeList, StorageValue};
  use cfx_types::{Address, U256, H160};
  use crate::is_default::IsDefault;
  use std::sync::Arc;

  #[test]
  fn test_is_default() {
    let account = Account::new_empty_with_balance(&H160([0x00;20]),
    &U256::zero(),&U256::zero()).unwrap();
    assert_eq!(account.is_default(),true);
    let code_info = CodeInfo{ code: Arc::new(vec![]), owner: Default::default() };
    ...
    let storage_value = StorageValue{ value: Default::default(), owner: None };
    assert_eq!(storage_value.is_default(),true);
    let storage_value1 = StorageValue{ value: Default::default(), owner: Some(Address::default()) };
