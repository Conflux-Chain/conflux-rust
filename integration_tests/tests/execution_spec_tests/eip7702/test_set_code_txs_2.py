...
@pytest.mark.valid_from("Prague")
def test_storage_key_allocation(state_test: StateTestFiller, pre: Alloc):
    """
    验证storage.store_next的键分配顺序
    """
    env = Environment()
    sender = pre.fund_eoa()
    ...
    # 按顺序分配存储键
    key1 = test_storage.store_next(1, "第一个存储项")
    key2 = test_storage.store_next(2, "第二个存储项")
    key3 = test_storage.store_next(3, "第三个存储项")
    key4 = test_storage.store_next(4, "第四个存储项")
    
    # 部署验证合约
    contract = pre.deploy_contract(
        code=Op.SSTORE(key1, 1)
        + Op.SSTORE(key2, 2)
        + Op.SSTORE(key3, 3)
        + Op.SSTORE(key4, 4)
        + Op.STOP
    ...
    post = {
        contract: Account(storage=expected_storage)
    }
        ...
        + Switch(
            cases=[
                Case(
                    ...
                    + Op.CALL(gas=1_000_000, address=proxy, args_offset=0, args_size=32 * 2)
                    + Op.STOP(),
                ),
                Case(
