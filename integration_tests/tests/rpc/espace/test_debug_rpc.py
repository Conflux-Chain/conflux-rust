...
# a temp util method, finally should be replaced by the util method provided by the framework
# load contract metadata from tests/test_contracts/artifacts
...
def test_trace_simple_cfx_transfer(ew3, evm_accounts, call_rpc):
    ...
    tx_trace = call_rpc('debug_traceTransaction', [tx_hash])
    assert tx_trace['failed'] == False
    assert tx_trace['gas'] == 21000
    assert tx_trace['returnValue'] == ''
    assert len(tx_trace['structLogs']) == 0

def test_trace_deploy_contract(ew3, call_rpc, evm_accounts):
    account = evm_accounts[0]
    contract_meta = _load_contract_metadata("MyToken")
    ...
    check_deploy_trace(call_rpc, tx_hash)
    check_transfer_trace(call_rpc, transfer_hash)
    check_noop_trace(call_rpc, transfer_hash)
    check_four_byte_trace(call_rpc, transfer_hash)
    check_call_trace(call_rpc, transfer_hash)
    check_opcode_trace_with_config(call_rpc, transfer_hash)

def check_deploy_trace(call_rpc, tx_hash):
    tx_trace = call_rpc('debug_traceTransaction', [tx_hash])
...
def check_transfer_trace(call_rpc, transfer_hash):
    transfer_trace = call_rpc('debug_traceTransaction', [transfer_hash])
...
def check_noop_trace(call_rpc, transfer_hash):
    noop_trace = call_rpc('debug_traceTransaction', [transfer_hash, {"tracer": "noopTracer"}])
    assert noop_trace == {}

def check_four_byte_trace(call_rpc, transfer_hash):
    four_byte_trace = call_rpc('debug_traceTransaction', [transfer_hash, {"tracer": "4byteTracer"}])
    assert four_byte_trace == {'0xa9059cbb-64': 1}

def check_call_trace(call_rpc, transfer_hash):
    call_trace = call_rpc('debug_traceTransaction', [transfer_hash, {"tracer": "callTracer"}])
...
def check_opcode_trace_with_config(call_rpc, tx_hash):
    trace = call_rpc('debug_traceTransaction', [tx_hash, {
        "enableMemory": True,
        "disableStack": False,
    ...
    limited_trace = call_rpc('debug_traceTransaction', [tx_hash, {
        "enableMemory": True,
        "disableStack": False,
        "disableStorage": False,
        "enableReturnData": True,
    ...
    no_stack_storage_trace = call_rpc('debug_traceTransaction', [tx_hash, {
        "enableMemory": True,
        "disableStack": True,
        "disableStorage": True,
        "enableReturnData": True
    ...
    disable_all_trace = call_rpc('debug_traceTransaction', [tx_hash, {
        "enableMemory": False,
        "disableStack": True,
        "disableStorage": True,
