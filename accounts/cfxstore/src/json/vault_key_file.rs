...
pub fn remove_vault_name_from_json_meta(
    meta: &str,
) -> Result<String, error::Error> {
    let mut meta = if meta.is_empty() {
        Value::Object(serde_json::Map::new())
...
mod test {
    ...
    fn vault_name_not_inserted_to_json_meta() {
        assert!(
            insert_vault_name_to_json_meta(r#"///3533"#, "MyVault").is_err()
        );
        assert!(
            insert_vault_name_to_json_meta(r#""string""#, "MyVault").is_err()
        );
    }
    ...
    fn vault_name_removed_from_json_meta() {
        ...
        assert_eq!(
            remove_vault_name_from_json_meta(
                r#"{"tags":["kalabala"],"vault":"MyVault"}"#
            )
            .unwrap(),
            r#"{"tags":["kalabala"]}"#
