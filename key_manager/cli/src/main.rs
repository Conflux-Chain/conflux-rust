...
use docopt::Docopt;
use keylib::{KeyPair, Random, Brain, BrainPrefix, Prefix, Error as KeyError, Generator, sign, verify_public, verify_address, brain_recover};
...
Commands:
    ...
    generate random    Generates new random key.
...
fn execute<S, I>(command: I) -> Result<String, Error> where I: IntoIterator<Item=S>, S: AsRef<str> {
	...
	return if args.cmd_info {
		...
		let result = if args.flag_brain {
			...
			let secret = args.arg_secret_or_phrase.parse().map_err(|_| KeyError::InvalidSecret)?;
		...
		let secret = args.arg_secret.parse().map_err(|_| KeyError::InvalidSecret)?;
		let message = args.arg_message.parse().map_err(|_| KeyError::InvalidMessage)?;
		let signature = sign(&secret, &message)?;
		Ok(format!("{}", signature))
	} else if args.cmd_verify {
		let signature = args.arg_signature.parse().map_err(|_| KeyError::InvalidSignature)?;
		let message = args.arg_message.parse().map_err(|_| KeyError::InvalidMessage)?;
		let ok = if args.cmd_public {
			let public = args.arg_public.parse().map_err(|_| KeyError::InvalidPublic)?;
			verify_public(&public, &signature, &message)?
		} else if args.cmd_address {
			let address = args.arg_address.parse().map_err(|_| KeyError::InvalidAddress)?;
		...
		let (phrase, keypair) = in_threads(move || {
			...
			move || {
				...
				Err(KeyError::Custom("Couldn't find any results.".into()))
			}
		})?;
...
fn in_threads<F, X, O>(prepare: F) -> Result<O, KeyError> where
	O: Send + 'static,
	X: Send + 'static,
	F: Fn() -> X,
	X: FnMut() -> Result<Option<O>, KeyError>,
{
	...
	Err(KeyError::Custom("No results found.".into()))
}
