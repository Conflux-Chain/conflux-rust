...
/// memory overlay.
///
/// The operations `insert()` and `remove()` take place on the memory overlay;
/// batches of such operations may be flushed to the disk-backed DB with
/// `commit()` or discarded with `revert()`.
///
/// `lookup()` and `contains()` maintain normal behaviour - all `insert()` and
/// `remove()` queries have an immediate effect in terms of these functions.
#[derive(Clone)]
...
impl Payload {
    fn new(count: u32, value: DBValue) -> Self { Payload { count, value } }
}
...
impl OverlayDB {
    ...
    pub fn commit_to_batch(
        &mut self, batch: &mut DBTransaction,
    ) -> io::Result<u32> {
        let mut ret = 0u32;
            ...
            if rc != 0 {
                match self.payload(&key) {
                    Some(x) => {
                        ...
                        deletes += if self
                            .put_payload_in_batch(batch, &key, &payload)
                        {
                            1
                        } else {
                            0
    ...
    /// Put the refs and value of the given key, possibly deleting it from the
    /// db.
    fn put_payload_in_batch(
        &self, batch: &mut DBTransaction, key: &H256, payload: &Payload,
    ) -> bool {
        if payload.count > 0 {
...
mod tests {
    ...
    fn overlaydb_overlay_insert_and_remove() {
        ...
        assert_eq!(
            trie.get(&h, EMPTY_PREFIX).unwrap(),
            DBValue::from_slice(b"hello world")
        );
    ...
    fn overlaydb_complex() {
        ...
        assert_eq!(
            trie.get(&hfoo, EMPTY_PREFIX).unwrap(),
            DBValue::from_slice(b"foo")
        );
        let hbar = trie.insert(EMPTY_PREFIX, b"bar");
        assert_eq!(
            trie.get(&hbar, EMPTY_PREFIX).unwrap(),
            DBValue::from_slice(b"bar")
        );
        trie.commit().unwrap();
        assert_eq!(
            trie.get(&hfoo, EMPTY_PREFIX).unwrap(),
