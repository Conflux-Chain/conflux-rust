...
/// ```
///
/// When we make a new commit, we make a journal of all blocks in the recent
/// history and record all keys that were inserted and deleted. The journal is
/// ordered by era; multiple commits can share the same era. This forms a data
/// structure similar to a queue but whose items are tuples. By the time comes
/// to remove a tuple from the queue (i.e. then the era passes from recent
/// history into ancient history) then only one commit from the tuple is
/// considered canonical. This commit is kept in the main backing database,
/// whereas any others from the same era are reverted.
///
/// It is possible that a key, properly available in the backing database be
/// deleted and re-inserted in the recent history queue, yet have both
/// operations in commits that are eventually non-canonical. To avoid the
/// original, and still required, key from being deleted, we maintain a
/// reference count which includes an original key, if any.
///
...
/// ```
///
/// Practically, this means that for each commit block turning from recent to
/// ancient we do the following:
/// ```text
/// is_canonical:
...
impl EarlyMergeDB {
    ...
    // The next three are valid only as long as there is an insert operation of
    // `key` in the journal.
    ...
    fn reset_already_in(
        batch: &mut DBTransaction, col: Option<u32>, key: &H256,
    ) {
        batch.delete(col, &Self::morph_key(key, 0));
    }

    fn is_already_in(
        backing: &dyn KeyValueDB, col: Option<u32>, key: &H256,
    ) -> bool {
        backing
    ...
    fn insert_keys(
        inserts: &[(H256, DBValue)], backing: &dyn KeyValueDB,
        col: Option<u32>, refs: &mut HashMap<H256, RefInfo>,
        ...
        for &(ref h, ref d) in inserts {
            match refs.entry(*h) {
                ...
                Entry::Vacant(entry) => {
                    ...
                    if in_archive {
                        // already in the backing DB. start counting, and
                        // remember it was already in.
                        Self::set_already_in(batch, col, h);
                        trace!(target: "jdb.fine", "    insert({}): New to queue, in DB: Recording and inserting into queue", h);
                    } else {
                        // Gets removed when a key leaves the journal, so should
                        // never be set when we're placing a new key.
    ...
    fn replay_keys(
        inserts: &[H256], backing: &dyn KeyValueDB, col: Option<u32>,
    ...
    fn remove_keys(
        deletes: &[H256], refs: &mut HashMap<H256, RefInfo>,
        batch: &mut DBTransaction, col: Option<u32>, from: RemoveFrom,
    )
    {
        // with a remove on {queue_refs: 1, in_archive: true}, we have two
        // options:
        // - convert to {queue_refs: 1, in_archive: false} (i.e. remove it from
        //   the conceptual archive)
        // - convert to {queue_refs: 0, in_archive: true} (i.e. remove it from
        //   the conceptual queue)
        // (the latter option would then mean removing the RefInfo, since it
        // would no longer be counted in the queue.) both are valid, but
        // we switch between them depending on context.     All inserts
        // in queue (i.e. those which may yet be reverted) have an entry in
        // refs.
        for h in deletes {
            match refs.entry(*h) {
                Entry::Occupied(mut entry) => {
                ...
                Entry::Vacant(_entry) => {
                    // Gets removed when moving from 1 to 0 additional refs.
                    // Should never be here at 0 additional refs.
    ...
    fn can_reconstruct_refs(&self) -> bool {
        let (latest_era, reconstructed) =
            Self::read_refs(&*self.backing, self.column);
    ...
    fn read_refs(
        db: &dyn KeyValueDB, col: Option<u32>,
    ) -> (Option<u64>, HashMap<H256, RefInfo>) {
...
impl JournalDB for EarlyMergeDB {
    ...
    fn journal_under(
...

        {
            ...
            // - we place into the backing DB or increment the counter if
            //   already in;
    ...
    fn mark_canonical(
        ...
        while let Some(rlp_data) = {
            ...
            if canon_id == &view.id().expect("rlp read from db; qed") {
                // Collect keys to be removed. Canon block - remove the
                // (enacted) deletes.
                ...
                for k in &inserts {
                    match refs.get(k).cloned() {
                        None => {
                            // [in archive] -> SHIFT remove -> SHIFT insert
                            // None->Some{queue_refs: 1, in_archive: true} ->
                            // TAKE remove Some{queue_refs: 1, in_archive:
                            // true}->None -> TAKE insert
                            // already expunged from the queue (which is allowed
                            // since the key is in the archive).
                            // leave well alone.
                        }
                        Some(RefInfo {
                            queue_refs: 1,
                ...
                // Collect keys to be removed. Non-canon block - remove the
                // (reverted) inserts.
...
mod tests {
    use super::*;
    use crate::{commit_batch, inject_batch};
