...
impl AtomicBitSet {
    ...
    pub fn add(&mut self, id: Index) -> bool {
        ...
        self.layer2[p2]
            .store(self.layer2[p2].load(Relaxed) | id.mask(SHIFT2), Relaxed);
        self.layer3
    ...
    pub fn clear(&mut self) {
        ...
        // that are already clear. In the best case when the set is already
        // cleared, this will only touch the highest layer.
        ...
        loop {
            if m2 != 0 {
                ...
                // layer 1 & 0 are cleared unconditionally. it's only 32-64
                // words and the extra logic to select the
                // correct works is slower then just clearing
                // them all.
...
impl AtomicBlock {
    ...
    fn contains(&self, id: Index) -> bool {
        self.atom
            .get()
            .map(|l0| {
                l0[id.row(SHIFT1)].load(Ordering::Relaxed) & id.mask(SHIFT0)
                    != 0
            })
            .unwrap_or(false)
    }
