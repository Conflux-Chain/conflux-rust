...
class Block:
    ...
    def __init__(self, hash: str, parent_hash: str, timestamp: float, height: int, referees: list, difficulty: int):
        self.hash = hash
    ...
    def __parse_block_header__(log_line: str):
        try:
            ...
            difficulty = int(parse_value(log_line, "difficulty: ", ","))
            ...
            for ref_hash in parse_value(log_line, "referee_hashes: [", "]").split(","):
                ...
                if len(ref_hash) > 0:
                    assert len(
                        ref_hash) == 66, "invalid block referee hash length, line = {}".format(log_line)
                    referees.append(ref_hash)
            return Block(block_hash, parent_hash, timestamp, height, referees, difficulty)
...
class NodeLogMapper:
    ...
    def __init__(self, log_file: str):
        assert os.path.exists(
            log_file), "log file not found: {}".format(log_file)
...
class LogAggregator:
    ...
    def generate_latency_stat(self):
        for b in self.blocks.values():
            for t in BlockLatencyType:
                self.block_latency_stats[t.name][
                    b.hash] = Statistics(b.get_latencies(t))
