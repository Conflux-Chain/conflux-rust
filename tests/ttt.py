...
class CommissionPrivilegeTest(ConfluxTestFramework):
    ...
    def run_test(self):
        ...
        test_contract = solc.get_contract_instance(
            abi_file = os.path.join(file_dir, "contracts/abi.json"),
            bytecode_file = os.path.join(file_dir, "contracts/bytecode.dat"),
        )
        ...
        client.send_tx(tx, True)
        assert_equal(client.get_balance(contract_addr), 1000 * 10 ** 18)
