...
class P2PTest(ConfluxTestFramework):
    ...
    def run_test(self):
        ...
        for i in range(tx_n):
            value = int(
                (balance_map[sender_key] - ((tx_n - i) * 21000 * gas_price))
                * random.random()
            )
            ...
            tx_data = decode_hex(
                erc20_contract.functions.transfer(
                    Web3.toChecksumAddress(encode_hex(privtoaddr(receiver_sk))), value
                ).buildTransaction(self.tx_conf)["data"]
            )
    ...
    def get_balance(self, contract, token_address, nonce):
        tx = contract.functions.balanceOf(
            Web3.toChecksumAddress(encode_hex(token_address))
        ).buildTransaction(self.tx_conf)
