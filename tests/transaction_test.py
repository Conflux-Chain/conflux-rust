...
class TransactionTest(DefaultConfluxTestFramework):
    ...
    def check_account(self, k, balance_map):
        ...
        try:
            ...
            staking_balance = parse_as_int(self.nodes[0].cfx_getBankBalance(addr))
        except Exception as e:
            self.log.info("Fail to get balance, error=%s", str(e))
            return False
        if balance + staking_balance == balance_map[k]:
            return True
        else:
            self.log.info("Remote balance:%d, local balance:%d", balance + staking_balance, balance_map[k])
            time.sleep(1)
            return False
