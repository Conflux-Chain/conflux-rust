...
class TxPoolGarbageCollectTest(ConfluxTestFrameworkForContract):
    ...
    def run_test(self):
        ...
        addr = accounts[0][0]
        for i in range(1, 11):
            self.cfx_transfer(addr, 1, (i+10)  * (10**9), priv_key=accounts[i][1], execute=False)

        assert_equal(self.client.txpool_status(), (5, 5))

        for i in range(0, 5):
            self.cfx_transfer(addr, 1, 21 * (10**9), priv_key=accounts[0][1], nonce = i, execute=False)
        
        assert_equal(self.client.txpool_status(), (5, 1))
        for i in range(1, 11):
            self.cfx_transfer(addr, 1, (i+30)  * (10**9), priv_key=accounts[i][1], execute=False)
