...
run_latency_exp () {
    ...
    ssh ubuntu@${master_ip} "cd ./conflux-rust/tests/scripts;python3 ./exp_latency.py --batch-config \"$exp_config\" --storage-memory-mb 16 --bandwidth 20 --tps $tps --enable-tx-propagation --send-tx-period-ms 50 --txgen-account-count $account_count --enable-flamegraph $flamegraph_option"
