...
run_latency_exp () {
    ...
    ssh ubuntu@${master_ip} "cd ./conflux-rust/tests/scripts;python3 ./exp_latency.py --vms $slave_count --batch-config \"$exp_config\" --storage-memory-mb 16 --bandwidth 20 --slave-count $slave_count --tps $tps --enable-tx-propagation --send-tx-period-ms 200 $flamegraph_option"
