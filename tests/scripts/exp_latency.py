...
def execute(cmd, retry, cmd_description):
    while True:
        ...
        print(
            "Failed to {}, return code = {}, retry = {} ...".format(
                cmd_description, ret, retry
            )
        )
        assert retry > 0
        retry -= 1
...
def pssh(ips_file: str, remote_cmd: str, retry=0, cmd_description=""):
    cmd = 'parallel-ssh -O "StrictHostKeyChecking no" -h {} -p 400 "{}" > /dev/null 2>&1'.format(
        ips_file, remote_cmd
    )
...
def pscp(ips_file: str, local: str, remote: str, retry=0, cmd_description=""):
    cmd = 'parallel-scp -O "StrictHostKeyChecking no" -h {} -p 400 {} {} > /dev/null 2>&1'.format(
        ips_file, local, remote
    )
    execute(cmd, retry, cmd_description)
        ...
        for config in batch_config.split(","):
            ...
            if len(fields) != 4 and len(fields) != 6:
                raise AssertionError(
                    "invalid config, format is <block_gen_interval_ms>:<txs_per_block>:<tx_size>:<num_blocks>:[<data_propagate_interval_ms>:<data_propagate_size>]"
                )
            config_groups.append(
                RemoteSimulateConfig(
...
class LatencyExperiment(ArgumentHolder):
    ...
    def run(self):
        for config in RemoteSimulateConfig.parse(self.batch_config):
            ...
            execute(
                "sh copy_metrics.sh {} > /dev/null".format(self.tag(config)),
                3,
                "collect metrics",
            )
    ...
    def run_remote_simulate(self, config: RemoteSimulateConfig):
        cmd = [
            ...
            "--nodes-per-host",
            str(self.nodes_per_host),
            "--generation-period-ms",
            str(config.block_gen_interval_ms),
            "--num-blocks",
            str(config.num_blocks),
            "--block-sync-step",
            str(self.block_sync_step),
            "--txs-per-block",
            str(config.txs_per_block),
            "--generate-tx-data-len",
            str(config.tx_size),
            "--connect-peers",
            str(self.connect_peers),
            "--ips-file",
            self.ips_file,
            "--throttling",
            self.throttling,
            "--storage-memory-mb",
            str(self.storage_memory_mb),
            "--tps",
            str(self.tps),
            "--bandwidth",
            str(self.bandwidth),
            "--metrics-report-interval-ms",
            str(self.metrics_report_interval_ms),
            "--send-tx-period-ms",
            str(self.send_tx_period_ms),
            "--txgen-account-count",
            str(self.txgen_account_count),
        ]
    ...
    def tag(self, config: RemoteSimulateConfig):
        ...
        return "{}ms_{}k_{}vms_{}nodes".format(
            config.block_gen_interval_ms, block_size_kb, self.vms, self.nodes_per_host
        )
    ...
    def stat_latency(self, config: RemoteSimulateConfig):
        ...
        if config.data_propagate_enabled:
            os.system(
                'echo "Data propagation enabled: interval = {}, size = {}" >> {}'.format(
                    config.data_propagate_interval_ms,
                    config.data_propagate_size,
                    self.stat_log_file,
                )
            )
        ...
        ret = os.system(
            "python3 stat_latency.py {0} logs {0}.csv >> {1}".format(
                self.tag(config), self.stat_log_file
            )
        )
        assert (
            ret == 0
