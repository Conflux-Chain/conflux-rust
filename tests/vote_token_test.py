...
class VoteTokenTest(SmartContractBenchBase):
    ...
    def setup_contract(self):
        ...
        transaction = self.call_contract_function(
            self.token_contract, "constructor", [], self.default_account_key
        )
        self.token_address = self.wait_for_tx([transaction], True)[0]["contractCreated"]
        transaction = self.call_contract_function(
            self.vote_contract, "constructor", [], self.default_account_key
        )
    ...
    def generate_transactions(self, i):
        self.call_contract_function(
            ...
            [
                ...
                [Web3.toChecksumAddress(privtoaddr(acc)) for acc in self.accounts],
                "v",
            ],
        ...
        for i in range(self.num_of_options):
            self.call_contract_function(
                self.vote_contract,
                "vote",
                [i, random.randint(0, self.num_of_options - 1)],
                self.default_account_key,
                self.vote_address,
                True,
                True,
