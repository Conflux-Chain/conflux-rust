...
// FIXME: MITM is possible. How bad is it? Faking message isn't too bad other
// FIXME: than ruining the reputation of one node. Flooding the network is more
// FIXME: serious problem. The worst among the flooding is tx prop. Actually,
// FIXME: how is tx prop working?
//
// FIXME: check if the handshake is designed properly.
pub struct Session {
    /// Session information
...
pub enum SessionData {
    ...
    // FIXME: it only seem to be useful in SessionDataWithDisconnectInfo.
    // Update the comment?
    /// Session has more data to be read.
    Continue,
}

// FIXME: to understand where is it used.
...
impl Session {
    ...
    fn prepare_packet(
        ...
        if protocol.is_some()
            // FIXME: why use emptiness check? Is it a must?
            // FIXME: We don't check if we have the protocol suggest that this check is optional.
            && (/* self.metadata.capabilities.is_empty()
                || */self.had_hello.is_none())
        // FIXME: make sure had_hello is set only after successful handshake.
        {
            debug!(
