...
use cfx_rpc_cfx_types::{
    pos::{
        ...
        NodeLockStatus, PoSEpochReward, Reward, RpcCommittee, RpcTermData,
...
use cfxcore::{
    ...
    consensus::pos_handler::PosVerifier, errors::Result as CoreResult,
    BlockDataManager, SharedConsensusGraph,
};
...
impl PosRpcServer for PosHandler {
    fn pos_status(&self) -> RpcResult<Status> { Ok(self.status_impl()) }

    fn pos_account(
        &self, address: H256, view: Option<U64>,
    ) -> RpcResult<Account> {
        self.account_impl(address, view).map_err(Into::into)
    }

    fn pos_account_by_pow_address(
        &self, address: RpcAddress, view: Option<U64>,
    ) -> RpcResult<Account> {
        self.account_by_pow_address_impl(address, view)
            .map_err(Into::into)
    }

    fn pos_committee(&self, view: Option<U64>) -> RpcResult<CommitteeState> {
        self.committee_by_block_number(view).map_err(Into::into)
    }
    ...
    fn pos_get_epoch_state(
        &self, epoch: U64,
    ) -> RpcResult<Option<RpcEpochState>> {
        Ok(self
    ...
    fn pos_get_ledger_info_by_epoch(
        &self, epoch: U64,
    ) -> RpcResult<Option<RpcLedgerInfoWithSignatures>> {
        Ok(self
    ...
    fn pos_get_ledger_infos_by_epoch(
        &self, start_epoch: U64, end_epoch: U64,
    ) -> RpcResult<Vec<RpcLedgerInfoWithSignatures>> {
        Ok(self
