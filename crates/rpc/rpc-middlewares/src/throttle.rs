        ...
        + 'static
{
        ...
        for entry in batch.iter_mut() {
            let (id, name) = match entry {
                Ok(BatchEntry::Call(req)) => {
                    (req.id.clone(), req.method_name().to_string())
                }
                Ok(BatchEntry::Notification(_)) => continue,
                Err(_) => continue,
            };

            let throlltle_result = self.before(&name);
            if let Err(e) = throlltle_result {
    ...
    fn notification<'a>(
        ...
        let throlltle_result = self.before(&n.method_name().to_string());
        match throlltle_result {
