...
impl From<TestAuthorization> for SignedAuthorization {
    fn from(auth: TestAuthorization) -> Self { auth.inner }
}

impl<'de> Deserialize<'de> for TestAuthorization {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where D: Deserializer<'de> {
        // This is a hack to remove duplicate yParity and v fields which can be
        // used by the test files for cross client compat
