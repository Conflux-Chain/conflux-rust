...
impl RawConfiguration {
    pub fn validate(&self) -> Result<(), String> {
        ...
        // timeout must be larger than the window size
        ...
        if tx_cache_timeout_ms < tx_cache_window_size {
            return Err(format!(
        ...
        if self.received_tx_index_maintain_timeout_ms
        ...
        if self.inflight_pending_tx_index_maintain_timeout_ms
            < inflight_window_size_ms
        {
            return Err(format!(
                "inflight_pending_tx_index_maintain_timeout_ms ({}) must be >= {}ms",
                self.inflight_pending_tx_index_maintain_timeout_ms, inflight_window_size_ms
            ));
        }
        if self.tx_maintained_for_peer_timeout_ms < self.send_tx_period_ms {
            return Err(
        ...
        // number.
        if self.pos_round_per_term % 2 != 0 {
            return Err("pos_round_per_term must be an even number".to_string());
        }
