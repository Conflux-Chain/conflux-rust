...
use blst::{
    blst_bendian_from_fp, blst_fp, blst_fp_from_bendian, blst_scalar,
...
pub(super) fn remove_padding(input: &[u8]) -> Result<&[u8; FP_LENGTH], String> {
...
/// Extracts a scalar from a 32 byte slice representation, decoding the input as
/// a big endian unsigned integer. If the input is not exactly 32 bytes long, an
/// error is returned.
///
/// From [EIP-2537](https://eips.ethereum.org/EIPS/eip-2537):
/// * A scalar for the multiplication operation is encoded as 32 bytes by
///   performing BigEndian encoding of the corresponding (unsigned) integer.
///
/// We do not check that the scalar is a canonical Fr element, because the EIP
/// specifies:
/// * The corresponding integer is not required to be less than or equal than
///   main subgroup order `q`.
pub(super) fn extract_scalar_input(
    input: &[u8],
) -> Result<blst_scalar, String> {
    ...
    unsafe {
        // Note: We do not use `blst_scalar_fr_check` here because, from
        // EIP-2537:
        //
        // * The corresponding integer is not required to be less than or equal
        //   than main subgroup
        // order `q`.
...
pub(super) fn fp_from_bendian(
    input: &[u8; 48],
) -> Result<blst_fp, &'static str> {
