...
impl PosState {
    ...
    pub fn decode_persisted(data: &[u8]) -> Result<Self> {
        // Safe in both directions because complete BCS encodings are
        // prefix-free: old bytes run out under the current layout, current
        // bytes leave the trailing map unread under the old one.
...
impl PosState {
    ...
    pub fn validate_dispute(
        ...
        match self.node_map.get(&dispute_payload.address) {
            None => {
                bail!("Unknown dispute node: {:?}", dispute_payload.address)
            }
            Some(node_status)
                if node_status.lock_status.exempt_from_forfeit().is_some() =>
            {
                bail!("Dispute a forfeited node: {:?}", dispute_payload.address)
            }
            Some(_) => {}
        }
...
mod tests {
    ...
    fn a_dispute_never_shortens_a_withdrawal_already_in_flight() {
        ...
        let exits: Vec<View> = lock_status_of(&state, &alice)
            .out_queue
            .iter()
            .map(|item| item.view)
            .collect();
        ...
        let relocked: Vec<View> = lock_status_of(&state, &alice)
            .out_queue
            .iter()
            .map(|item| item.view)
            .collect();
        assert_eq!(relocked, exits);
    }
