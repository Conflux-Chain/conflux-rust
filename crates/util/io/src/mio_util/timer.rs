...
impl<T> Timer<T> {
    fn new(
        tick_ms: u64, num_slots: usize, capacity: usize, start: Instant,
    ) -> Timer<T> {
    ...
    pub fn set_timeout(
        &mut self, delay_from_now: Duration, state: T,
    ) -> Timeout {
    ...
    fn set_timeout_at(
        &mut self, delay_from_start: Duration, state: T,
    ) -> Timeout {
        ...
        while self.tick <= target_tick {
            ...
            if curr == EMPTY {
                ...
                if links.tick <= self.tick {
                    ...
                    self.wheel[slot].next_tick =
                        cmp::min(next_tick, links.tick);
    ...
    fn schedule_readiness(&self, tick: Tick) {
        if let Some(inner) = self.inner.borrow() {
            ...
            loop {
                ...
                trace!(
                    "advancing the wakeup time; target={}; curr={}",
                    tick,
                    curr
                );
    ...
    fn slot_for(&self, tick: Tick) -> usize { (self.mask & tick) as usize }
}
...
mod test {
    ...
    fn count<T>(timer: &Timer<T>) -> usize { timer.entries.len() }

    fn timer() -> Timer<&'static str> {
        Timer::new(TICK, SLOTS, CAPACITY, Instant::now())
    }

    fn ms_to_tick<T>(timer: &Timer<T>, ms: u64) -> u64 { ms / timer.tick_ms }
}
