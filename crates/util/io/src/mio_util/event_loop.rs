...
impl EventLoopBuilder {
    ...
    /// values.
    pub fn new() -> EventLoopBuilder { EventLoopBuilder::default() }
...
impl<H: Handler> EventLoop<H> {
    ...
    /// Indicates whether the event loop is currently running. If it's not it
    /// has either stopped or is scheduled to stop on the next tick.
    ...
    /// whenever any of the registered handles are ready.
    ...
    /// during that time.
    pub fn run_once(
        &mut self, handler: &mut H, timeout: Option<Duration>,
    ) -> io::Result<()> {
