...
impl<T> BitParIter<T> {
    ...
    /// bitset.par_iter().layers_split(2).count();
    /// # }
    /// ```
    ///
        ...
        let other = {
            let mut handle_level = |level: usize| {
                if self.0.masks[level] == 0 {
                    ...
                    // Top levels prefix is zero because there is nothing before
                    // it
                    let level_prefix =
                        self.0.prefix.get(level).cloned().unwrap_or(0);
                    ...
                    average_ones(self.0.masks[level])
                        .and_then(|average_bit| {
                            ...
                            // The `other` is the more significant half of the
                            // mask
                            other.0.masks[level] = self.0.masks[level] & !mask;
                            other.0.prefix[level - 1] =
                                (level_prefix | average_bit as u32) << BITS;
                            // The upper portion of the prefix is maintained,
                            // because the `other`
                            // will iterate the same subtree as the `self` does
                            other.0.prefix[level..]
                                .copy_from_slice(&self.0.prefix[level..]);
                            // And the `self` is the less significant one
                            self.0.masks[level] &= mask;
                            self.0.prefix[level - 1] =
                                (level_prefix | first_bit) << BITS;
                            Some(other)
                        })
                        .or_else(|| {
                            // Because there is only one bit left we descend to
                            // it
                            let idx =
                                level_prefix as usize | first_bit as usize;
                            self.0.prefix[level - 1] = (idx as u32) << BITS;
                            // The level that is descended from doesn't have
                            // anything interesting
                            // so it can be skipped in the future.
                            self.0.masks[level] = 0;
                            self.0.masks[level - 1] =
                                self.0.set.get_from_layer(level - 1, idx);
                            None
                        })
                }
            };
...
mod test_bit_producer {
    ...
    fn test_splitting(split_levels: u8) {
        fn visit<T>(
            mut us: BitProducer<T>, d: usize, i: usize, mut trail: String,
            c: &mut usize,
        ) where
