    ...
    pub async fn process_proposal_msg(
            ...
            .context("[RoundManager] Process proposal")?
        {
            if self
                ...
                .await?
            {
                // process_proposal returns Ok(true) only when the
                // proposal's VRF number strictly beats the current best
                // candidate's (`receive_proposal_candidate`); duplicates
                // tie on VRF number and return Ok(false). Each node
                // therefore relays at most a strictly-improving sequence
                // of candidates per round, so there is no broadcast
                // storm.
