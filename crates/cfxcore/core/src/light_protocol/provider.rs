...
impl Provider {
    ...
    fn tx_info_by_hash(&self, hash: H256) -> Result<TxInfo> {
        let (tx, tx_index, receipt) =
            match self.consensus.get_transaction_info_by_hash(&hash) {
                None => {
                    return Err(Error::UnableToProduceTxInfo {
    ...
    fn on_status_v2(
        ...
        self.send_status(io, peer)
            .map_err(|_| Error::SendStatusFailed { peer: *peer })?;
        ...
        let it = req
            ...
            .map::<Result<_>, _>(|h| {
                self.graph
                    ...
                    .ok_or_else(|| {
                        Error::Msg(format!("Block {:?} not found", h))
                            .into()
                    })
            });
    ...
    fn throttle<T: Message>(&self, peer: &NodeId, msg: &T) -> Result<()> {
        ...
        match result {
            ...
            ThrottleResult::Throttled(wait_time) => {
                ...
                return Err(Error::Throttled(msg.msg_name(), throttled))
            }
            ThrottleResult::AlreadyThrottled => {
                return Err(Error::AlreadyThrottled(msg.msg_name()))
            }
        }
    }
