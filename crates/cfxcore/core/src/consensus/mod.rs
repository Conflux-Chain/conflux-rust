...
use crate::{
    ...
    errors::{self, invalid_params, invalid_params_check, Result as CoreResult},
...
impl ConsensusGraph {
            ...
            .block_execution_results_by_hash(&block_hash, false)
        {
            ...
            None => {
                match self.data_man.local_block_info_by_hash(&block_hash) {
                    ...
                    Some(_) => {
                        return Err(FilterError::BlockAlreadyPruned { block_hash })
                    }
                }
            }
        };
    ...
    fn filter_logs_by_block_numbers(
        ...
        {
            Some(h) => h,
            None => return Err(FilterError::Custom(format!(
    ...
    pub fn call_virtual(
            ...
            self.get_block_hashes_by_epoch(epoch)
        {
            ...
            return Err(errors::Error::Msg("cannot get block hashes in the specified epoch, maybe it does not exist?".to_string()));
        };
        self.executor
    ...
    fn get_storage_state_by_height_and_hash(
        ...
        if !state_availability_boundary.check_availability(height, &hash) {
            ...
            return Err(errors::Error::Msg(format!(
                "State for epoch (number={:?} hash={:?}) does not exist: out-of-bound {:?}",
                height, hash, state_availability_boundary
            )));
        }
