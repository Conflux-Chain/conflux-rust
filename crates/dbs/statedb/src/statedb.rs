...
use cfx_types::{address_util::AddressUtil, Address, AddressWithSpace, Space};
...
impl StateDb {
    ...
    pub fn new_for_unit_test_with_epoch(epoch_id: &EpochId) -> Self {
        ...
        Self::new(Box::new(InmemoryStorage::from_epoch_id(epoch_id).unwrap()))
    }
    ...
    fn modify_single_value(
        ...
        match &mut entry {
            Occupied(o) => {
                ...
                Some(std::mem::replace(&mut o.get_mut().current_value, value))
            }
            ...
            &mut Vacant(_) => {
                ...
                entry
                    .or_insert(EntryValue::new_modified(original_value, value));
    ...
    pub fn delete_all<AM: access_mode::AccessMode>(
        ...
        let iter_range_upper_bound = to_key_prefix_iter_upper_bound(&key_bytes);
    ...
    fn load_storage_layout(
        ...
        storage: &dyn StorageStateTrait, accessed_entries: &AccessedEntries,
    ) -> Result<()> {
        if storage_layouts_to_rewrite.contains_key(&(address.to_vec(), space)) {
            return Ok(());
        }
        let storage_layout_key =
            StorageKey::StorageRootKey(address).with_space(space);
        let current_storage_layout =
            match accessed_entries.get(&storage_layout_key.to_key_bytes()) {
    ...
    pub fn set_storage_layout(
        &mut self, address: &AddressWithSpace, storage_layout: StorageLayout,
        debug_record: Option<&mut ComputeEpochDebugRecord>,
    ) -> Result<()> {
        self.set_raw(
