...
use cfx_types::{Address, H256 as B256, U256, U64};
...
pub enum TraceType {
    /// Default trace
    Trace,
    /// Provides a full trace of the VM’s state throughout the execution of the
    /// transaction, including for any subcalls.
    VmTrace,
    /// Provides information detailing all altered portions of the Ethereum
    /// state made due to the execution of the transaction.
    StateDiff,
}
...
impl TraceResults {
    ...
    ///
    /// The root trace's gasUsed should mirror the actual gas used by the
    /// transaction.
    ///
...
impl<T> Delta<T> {
    ...
    pub const fn is_added(&self) -> bool { matches!(self, Delta::Added(_)) }

    /// Returns true if the value is removed
    pub const fn is_removed(&self) -> bool { matches!(self, Delta::Removed(_)) }

    /// Returns true if the value is changed
    pub const fn is_changed(&self) -> bool { matches!(self, Delta::Changed(_)) }
}
...
impl Action {
    ...
    pub const fn is_call(&self) -> bool { matches!(self, Action::Call(_)) }

    /// Returns true if this is a create action
    pub const fn is_create(&self) -> bool { matches!(self, Action::Create(_)) }
    ...
    pub const fn is_reward(&self) -> bool { matches!(self, Action::Reward(_)) }
...
/// Represents a _create_ action, either a `CREATE` operation or a CREATE
/// transaction.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl TraceOutput {
    ...
    pub fn set_gas_used(&mut self, gas_used: u64) {
        match self {
            ...
            TraceOutput::Create(create) => {
                create.gas_used = U64::from(gas_used)
            }
        }
    }
}
...
/// A wrapper for [TransactionTrace] that includes additional information about
/// the transaction.
...
// Implement Serialize manually to ensure consistent ordering of fields to match
// other client's format
impl Serialize for LocalizedTransactionTrace {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where S: Serializer {
        let mut s =
            serializer.serialize_struct("LocalizedTransactionTrace", 9)?;
...
mod tests {
                ...
                trace: LocalizedTransactionTrace {
                    trace: TransactionTrace {
                        action: Action::Call(CallAction {
                            from: "4f4495243837681061c4743b74b3eedf548d56a5".parse::<Address>().unwrap(),
                            ...
                            to:  "99b5fa03a5ea4315725c43346e55a6a6fbd94098".parse::<Address>().unwrap(),
                            value: U256::from(0),
                        }),
                        error: None,
                    ...
                    transaction_hash: Some(B256::from_str("54160ddcdbfaf98a43a43c328ebd44aa99faa765e0daa93e61145b06815a4071").unwrap()),
  ...
  "type": "call"
        ...
        let trace: LocalizedTransactionTrace =
            serde_json::from_str(reference_data).unwrap();
