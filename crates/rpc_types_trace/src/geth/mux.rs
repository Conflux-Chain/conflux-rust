...
use super::{GethDebugBuiltInTracerType, GethDebugTracerConfig, GethTrace};
use serde::{Deserialize, Serialize};
use std::collections::HashMap;

/// A `muxTracer` config that contains the configuration for running multiple
/// tracers in one go.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct MuxConfig(
    pub HashMap<GethDebugBuiltInTracerType, Option<GethDebugTracerConfig>>,
);
...
mod tests {
    ...
    const CALL_FRAME_WITH_LOG: &str =
        include_str!("../../test_data/call_tracer/with_log.json");
    ...
    fn test_serialize_mux_tracer_config() {
        ...
        opts.tracing_options.tracer_config =
            serde_json::to_value(MuxConfig(HashMap::from([
                (GethDebugBuiltInTracerType::FourByteTracer, None),
                (
                    GethDebugBuiltInTracerType::CallTracer,
                    Some(GethDebugTracerConfig(
                        serde_json::to_value(call_config).unwrap(),
                    )),
                ),
                (
                    GethDebugBuiltInTracerType::PreStateTracer,
                    Some(GethDebugTracerConfig(
                        serde_json::to_value(prestate_config).unwrap(),
                    )),
                ),
            ])))
            .unwrap()
    ...
    fn test_deserialize_mux_frame() {
        let expected = HashMap::from([
            (
                ...
                GethTrace::FourByteTracer(
                    serde_json::from_str(FOUR_BYTE_FRAME).unwrap(),
                ),
            ),
            (
                GethDebugBuiltInTracerType::CallTracer,
                GethTrace::CallTracer(
                    serde_json::from_str(CALL_FRAME_WITH_LOG).unwrap(),
                ),
            ),
            (
                GethDebugBuiltInTracerType::PreStateTracer,
                GethTrace::PreStateTracer(
                    serde_json::from_str(PRESTATE_FRAME).unwrap(),
                ),
            ),
        ]);
