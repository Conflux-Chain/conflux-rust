...
use cfx_serde::num::from_int_or_hex_opt;
use cfx_types::{Address, H256 as B256, U256};
...
pub enum PreStateFrame {
    /// The default mode returns the accounts necessary to execute a given
    /// transaction.
    ///
    /// It re-executes the given transaction and tracks every part of state
    /// that is touched.
    Default(PreStateMode),
    /// Diff mode returns the differences between the transaction's pre and
...
impl PreStateFrame {
    ...
    /// Returns the account states after the transaction is executed if this
    /// trace was requested without diffmode.
    ...
    /// Returns the account states before and after the transaction is executed
    /// if this trace was requested with diffmode.
...
///
/// This will only contain changed [AccountState]s, created accounts will not be
/// included in the pre state and selfdestructed accounts will not be included
/// in the post state.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
impl DiffMode {
    ...
    ///
    /// In other words it removes entries that are equal (unchanged) in both the
    /// pre and post sets.
...
impl DiffStateKind {
    ...
    pub const fn is_pre(&self) -> bool { matches!(self, DiffStateKind::Pre) }

    /// Returns true if this is the post state of the [DiffMode]
    pub const fn is_post(&self) -> bool { matches!(self, DiffStateKind::Post) }
}
...
impl AccountState {
    ...
    pub fn from_account_info(
        nonce: u64, balance: U256, code: Option<Bytes>,
    ) -> Self {
        Self {
    ...
    ///
    /// This is useful for comparing pre vs post state and only keep changed
...
#[derive(
    Debug, Copy, Clone, Default, PartialEq, Eq, Serialize, Deserialize,
)]
#[serde(rename_all = "camelCase")]
pub struct PreStateConfig {
    /// If `diffMode` is set to true, the response frame includes all the
    /// account and storage diffs for the transaction. If it's missing or
    /// set to false it only returns the accounts and storage necessary to
...
impl PreStateConfig {
    ...
    #[inline]
    pub fn is_default_mode(&self) -> bool { !self.is_diff_mode() }
}
...
mod tests {
    ...
    const DEFAULT: &str =
        include_str!("../../test_data/pre_state_tracer/default.json");
    const LEGACY: &str =
        include_str!("../../test_data/pre_state_tracer/legacy.json");
    const DIFF_MODE: &str =
        include_str!("../../test_data/pre_state_tracer/diff_mode.json");
        ...
        let s = r#"{
            ...
            "0xc8ba32cab1757528daf49033e3673fae77dcf05d": {
                ...
                "storage": {
                    ...
                    "0x59fb7853eb21f604d010b94c123acbeae621f09ce15ee5d7616485b1e78a72e9":
                    "0x00000000000000c42b56a52aedf18667c8ae258a0280a8912641c80c48cd9548",
                    "0x8d8ebb65ec00cb973d4fe086a607728fd1b9de14aa48208381eed9592f0dee9a":
                    "0x00000000000000784ae4881e40b1f5ebb4437905fbb8a5914454123b0293b35f",
                    "0xff896b09014882056009dedb136458f017fcef9a4729467d0d00b4fd413fb1f1":
                    "0x000000000000000e78ac39cb1c20e9edc753623b153705d0ccc487e31f9d6749"
                }
            }
        }"#;
