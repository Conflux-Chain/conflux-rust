...
mod tests{
    ...
    fn test_epoch_number_serialize() {
        ...
        assert_eq!(serde_json::to_string(&e3).unwrap(), r#""latest_checkpoint""#);
        assert_eq!(serde_json::to_string(&e4).unwrap(), r#""latest_confirmed""#);
    ...
    fn test_epoch_number_into_primitive() {
        ...
        let latest_mined = EpochNumber::into_primitive(EpochNumber::LatestMined);
        let latest_checkpoint = EpochNumber::into_primitive(EpochNumber::LatestCheckpoint);
        let latest_confirm = EpochNumber::into_primitive(EpochNumber::LatestConfirmed);
        let latest_state = EpochNumber::into_primitive(EpochNumber::LatestState);
    ...
    fn test_block_hash_or_epoch_number_serialize() {
        ...
        let epoch_number1 = BlockHashOrEpochNumber::EpochNumber(EpochNumber::Num(U64::one()));
        let epoch_number2 = BlockHashOrEpochNumber::EpochNumber(EpochNumber::Earliest);
        let epoch_number3 = BlockHashOrEpochNumber::EpochNumber(EpochNumber::LatestCheckpoint);
        let epoch_number4 = BlockHashOrEpochNumber::EpochNumber(EpochNumber::LatestConfirmed);
        let epoch_number5 = BlockHashOrEpochNumber::EpochNumber(EpochNumber::LatestState);
        let epoch_number6 = BlockHashOrEpochNumber::EpochNumber(EpochNumber::LatestMined);
    ...
    fn test_block_hash_or_epoch_number_deserialize() {
        ...
        let deserialize:BlockHashOrEpochNumber = serde_json::from_str(serialize).unwrap();
        let block_hash = BlockHashOrEpochNumber::BlockHash(H256::default());
        ...
        let deserialize1:BlockHashOrEpochNumber = serde_json::from_str(serialize1).unwrap();
        let deserialize2:BlockHashOrEpochNumber = serde_json::from_str(serialize2).unwrap();
        let deserialize3:BlockHashOrEpochNumber = serde_json::from_str(serialize3).unwrap();
        let deserialize4:BlockHashOrEpochNumber = serde_json::from_str(serialize4).unwrap();
        let deserialize5:BlockHashOrEpochNumber = serde_json::from_str(serialize5).unwrap();
        let deserialize6:BlockHashOrEpochNumber = serde_json::from_str(serialize6).unwrap();
        assert_eq!(deserialize1,BlockHashOrEpochNumber::EpochNumber(EpochNumber::Num(U64::one())));
        assert_eq!(deserialize2,BlockHashOrEpochNumber::EpochNumber(EpochNumber::Earliest));
        assert_eq!(deserialize3,BlockHashOrEpochNumber::EpochNumber(EpochNumber::LatestCheckpoint));
        assert_eq!(deserialize4,BlockHashOrEpochNumber::EpochNumber(EpochNumber::LatestConfirmed));
        assert_eq!(deserialize5,BlockHashOrEpochNumber::EpochNumber(EpochNumber::LatestState));
        assert_eq!(deserialize6,BlockHashOrEpochNumber::EpochNumber(EpochNumber::LatestMined));
    }
    #[test]
    fn test_block_hash_or_epoch_number_into_primitive() {
        let block_hash =
            BlockHashOrEpochNumber::into_primitive(BlockHashOrEpochNumber::BlockHash(H256::default()));
        let epoch_number =
            BlockHashOrEpochNumber::into_primitive(BlockHashOrEpochNumber::EpochNumber(EpochNumber::Earliest));
        assert_eq!(block_hash,PrimitiveBlockHashOrEpochNumber::BlockHash(H256::default()));
        assert_eq!(epoch_number,PrimitiveBlockHashOrEpochNumber::EpochNumber(PrimitiveEpochNumber::Earliest));
    }
}