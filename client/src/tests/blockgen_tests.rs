...
fn test_mining_10_epochs_inner(handle: &ClientHandle) {
    ...
    let best_block_hash = sync_graph.consensus.get_best_info().as_ref().best_block_hash;
    let start_height =
    ...
    while instant.elapsed() < max_timeout {
        let new_best_block_hash =
            sync_graph.consensus.get_best_info().as_ref().best_block_hash;
...
fn test_mining_10_epochs() {
    ...
    // NOTE: we do not propagate blocks in catch-up
    // mode so it would break the test
    conf.raw_conf.start_as_catch_up_mode = false;
